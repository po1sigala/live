...
| 1:00PM  | 16  | Instructor Demo: Session Storage   | 0:05     |
| 1:05PM  | 17  | Student Do: Session Storage        | 0:15     |
| 1:20PM  | 18  | Instructor Review: Session Storage | 0:10     |
...
### 16. Instructor Demo: Session Storage (5 min)

* Navigate to `17-Ins_Session-Storage` and run `npm install` and `npm start` from your command line.
...
* Open `17-Ins_Session-Storage/server.js` in your IDE to demonstrate the following:
  ...
  * The `maxAge` attribute is set to `24 * 60 * 60 * 1000` milliseconds, meaning that the user has approximately one day until their cookie is expired:
    ...
    > **Note**: In this demo, we specify a `maxAge`, but it is not necessary to do so. When no `maxAge` is added, the cookie expires when the user ends the session by closing out all tabs open to the page in the browser.
...
### 17. Student Do: Session Storage (15 min)

* Direct students to the activity instructions found in `18-Stu_Session-Storage/README.md`.

* Break your students into pairs that will work together on this activity.

  ```md
  # 📐 Add Comments to Implementation of the Sequelize Session Storage and Cookies
...
* Open `18-Stu_Session-Storage/Solved/server.js` in your IDE and explain the following:
  ...
  * 🔑 Then we set up an Express.js session like before, but this time we connect to the Sequelize database. We include the `secret`, `resave`, and `saveUninitialized` options as before. We can set additional options on the cookie, like a maximum age, as shown in the following example:
  ...
  * We set additional options on the cookie, like a `maxAge`, `secure`, `sameSite`, and `httpOnly`:
