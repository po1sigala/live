...
* Open `10-Stu_Timers-Intervals/Solved/script.js` in your IDE and explain the following: 
  ...
  * We use `appendChild()` to add the element that will hold the words of the poem to the HTML document, as follows:

    ```js
    mainEl.appendChild(bodyEl);
    ```

  * 🔑 We set a variable when creating the `setInterval()` method. The variable name is used as the parameter when we call `clearInterval()` to stop the execution. See the following code for an example:

    ```js
    var poemInterval = setInterval(function() {
    ```

  * 🔑 We set the first argument of the `setInterval()` method to be the function that we want performed at each interval. 

  * 🔑 If all the words have printed to the screen at the given interval, we use `clearInterval()` to stop the action, as follows:
  ...
  * Else, we use the `textContent` property to set the text of the `<div>` to the current word. The process repeats until no words remain at the set interval. See the following code for an example:
...
* Ask the students if there is anything they would like to review from Unit 04 so far.
