...
* Use the prompts and talking points (🔑) below to review the following key points:
  ...
  * ✔️ Using `<Routes>` and `<Route>` components we are able to construct routes for our applications. All `<Route>` component(s) should always be wrapped within a single `<Routes>` component. Additionally, each `<Route>` should also have `path` and `element` properties that give the location as well as which components to render respectively.
  ...
  * ✔️ We are able to create declaritive and accessible navigation around our application using the `Link to={}` component. We've shown that you can add a string URL but other datatypes such as objects can be passed in as arguments.
  
    ```js
    `<Link to={`/thoughts/${thought._id}`}>`
    ```

  * ✔️  For dynamic routes, we're able to capture data stored in the URL by using a react hook that comes with react router called `useParams()` to do so.
