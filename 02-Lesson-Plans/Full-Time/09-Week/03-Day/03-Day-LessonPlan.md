...
* In the open MongoDB shell, do the following:
  ...
  * Notice that to return the id, we must wrap the generated id value in `ObjectId()`! In Compass, the `ObjectId()` is automatically available. However, notice in the activity's `server.js` file, we have to require the `ObjectId` before we can return a value! 
