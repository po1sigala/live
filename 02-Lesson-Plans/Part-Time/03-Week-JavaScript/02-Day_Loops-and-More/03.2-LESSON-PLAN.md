...
* Take a few minutes before class to review the activities of the day and think about questions students with a limited exposure to JavaScript might ask. 

* In `19-Stu_Scope`, students are asked to work with a partner to debug some broken code to learn more about scope and shadowing. This is a challenging activity especially for learners new to programming languages. Please make sure to look through the solution code prior to class and try to anticipate questions that might arise. If need be, review the [MDN Web Docs on scope](https://developer.mozilla.org/en-US/docs/Glossary/Scope) so that you can confidently answer students' questions. 
...
* Open `11-Ins_Array/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 To store multiple values in a variable, we also use `var` and `=`. To hold multiple values, we put them in brackets `[]` separated by commas, like in the following example: 
  ...
  * 🔑 To access a single element in an array, we use the array's name plus the item's index or place in the array in brackets`[]`. The element with the index of `1` in the following example is `"Karl"`:

    ```js
    console.log(names[1]);
    ```

  * 🔑 Arrays are zero-indexed. So the first element in an array has an index of `0`, not `1`. Using `0` in the following example will log `Andre`, and as we saw previously, using `1` will actually log the second item in the array, `Karl`:
...
* Open `11-Ins_Array/index.html` in your browser and navigate to the console to demonstrate the following:
  ...
  * When we click on the arrow to the left of the data, the indices of each element in the array are shown. This is a handy way to inspect an array and find the index of an element we want to work with in code. 
...
* As an instructor, I want to create a list of students from which I can add and remove students.

## Acceptance Criteria 

* It's done when the total number of elements in the array is logged to the console.  

* It's done when "Welcome to the class STUDENT_NAME" is logged using each element in the array. 

* It's done when the first element in the array is removed and that element is 
replaced with the name of a new student.

* It's done when "REPLACED_NAME is in class" is logged when an `if statement` is used 
...
* Open `12-Stu_Arrays/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 When creating an array, the elements are placed in brackets `[]`. Each element is separated by a comma, as follows:
  ...
  * We test if the first value has been replaced using an `if` statement that evaluates if `students[0]` equals `"Bob"`. If the condition evaluates to true in the following example, `"Bob is in class!"` logs to the console. 
...
* Open `13-Ins_Iteration/script.js` in your IDE and demonstrate the following:
  ...
  * The second expression is a condition that determines if the loop will execute. If the following expression evaluates to `true`, we execute the code, and if it's false, the loop is complete:
  ...
  * 🔑 Next, we place the block of code that we want to execute each time the loop runs in curly brackets, as follows:
  ...
  * 🔑 We can also use `i` to access the elements of the array. We can do this using something called Bracket Notation. By placing the counter inside `[]`, as follows, we can access elements:
...
* Open `13-Ins_Iteration/index.html` in your browser and navigate to the console to demonstrate the following:
  ...
  * In the first loop, the counter variable `i` is initially assigned the value `0`. So, the first log shows the value of `i` is `0`. Then, after the block of code is executed, `i` is incremented. So, when it runs again, `i` is 1 and so on until the condition `i < 5` is no longer `true`. 

  * In the next loop, instead of just console logging `i`, `i` is used to determine the index of the element. So the first time it is run, `zooAnimals[i]` is read as `zooAnimals[0]` and `"Bears"` is logged. When the loop runs again, `i` is equal to 1, so `"Giraffes"` is logged. Doing this, we can use a `for` loop to quickly iterate over an entire array&mdash;even a huge one&mdash;with just a few lines of code. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How would iterating over an array be useful in your own code?  
...
* Open `15-Ins_Functions/script.js` in your IDE and demonstrate the following: 
  ...
  * 🔑 We can also use `var` to declare a function expression. After the assignment operator `=`, we use `function` followed by parentheses `()`. Just like before, in the following example the code block goes in curly brackets `{}`, and `return` is used:
...
* It's done if "They are equal in type and value" is logged if the values are strictly equal. 

* It's done if "They are equal in value" is logged if the two values are equal but not strictly equal.

* It's done if "The values are not equal" if the values are not equal or strictly equal. 
...
* Open `16-Stu_Functions/Solved/script.js` in your IDE and explain the following:
  ...
  * We use an `if` statement to evaluate if the values are strictly equal. If so, the messange `"They are equal in type and value"` will log, as shown in the following statement:
  ...
  * If the two values are not strictly equal, we check if they are equal in value only with `==`. So we add an `else if` statement that will log `"They are equal in value"` if `x == y` evaluates to `true`, as shown in the following example: 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How can using functions help improve code? 

  * 🙋 Functions are reusable blocks of code that we can execute again and again. Using functions makes code more modular and help us write cleaner code.  
...
* 🔑 When we declare a variable inside a function it changes the `scope` of the variable. The variable can only be accessed inside that function. This is called **local scope**. See the following code for an example:
...
* If we try to use a locally scoped variable another function, like in the following example, we will get an error:
...
* 🔑 **Shadowing** happens when we use the same variable name in both the local and global scope, as shown in the following example:
  ...
  ```
   ...
   We should avoid shadowing by giving the local and global variables unique names. Keeping the scope clean and organized allows us an easier time developing and working in the codebase.
...
* Open `18-Stu_Scope/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 We declare the variable `shout` in the global scope so is available to all functions including `justShout` and `shoutItAllOut`, as shown in the following example:
...
* Open `19-Ins_Methods/index.html` in your browser and demonstrate the following:
  ...
  * 🔑 Array methods can also return a new array. The `slice()` method returns a slice of an array as a new array. The arguments determine what indexes to use to start and end the slice. You can see the following statement for an example:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 They give us access to a wide range of functionality to use when writing code.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Array methods are a quick and easy way to add elements, remove elements, join two arrays and much, much more. String methods are also a powerful way to manipulate strings. With both array and string methods, we can implement powerful functionality in just a single line of code.
