...
* Open `13-Ins_Iteration/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 We use for loops to execute a block of code more than once. The complete for loop expression dictates how many times our code will be executed.
  ...
  * The second expression is a condition that determines if the loop will execute. If the expression evaluates to `true`, we execute our code. If it's false, the loop is complete.

  ```js
  i < 5;
  ```

  * After the loop executes our code block, we increment the counter.
  ...
  * 🔑 When iterating over an array, we use the array's `length` property to determine when the loop should stop. In this case, as long as `i` is less than the length of `zooAnimals`, the loop will execute our code.
  
  ```js
  for (var i = 0; i < zooAnimals.length; i++) 
  ```

  * 🔑 We can also use `i` to access the elements of the array. We can do this using something called Bracket Notation. By placing our counter inside `[]`, we can access elements.
...
* Open `14-Stu_Iteration/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 We write our for loop. We set our counter to 0 and dictate the loop will run till the counter equals the length of our array.
  ...
  * 🔑 We set the conditions under which our loop will run. Because we want to loop over our entire array, we set the condition such that our loop will stop when our counter equals the length of our array.

  ```js
  i < students.length;
  ```

  * Finally, after the block of code is executed, we increment our counter by `1`.
...
* Open `16-Stu_Functions/Solved/script.js` in your IDE and explain the following:
  ...
  * We use an if statement to evaluate if the values are strictly equal. If so, then `"They are equal in type and value"` will log.
...
* Open `17-Ins_Scope/index.html` in your IDE and demonstrate the following:
  ...
  * 🔑 We have been declaring variables outside of our functions. Variables declared outside of a function are `globally scoped`.
...
* 🔑 When we declare a variable inside a function it changes the `scope` of the variable. The variable can only be accessed inside that function. This is called local scope.
...
* If we try to use a locally-scoped variable another function, we will get an error.
...
* 🔑 Shadowing happens we we use the same variable name in both the local and global scope. We should avoid shadowing by giving our local and global variables unique names. Keeping our scope clean and organized allows us an easier time developing and working in our codebase.
...
* 🔑 To avoid shadowing, or declaring a variable with the same name in the global and local scope, we remove the `bears` variable in the local scope so that `sayBears` function will log `"Bears! OH MY!"`
...
* We declare the variable `sing` inside the outer function so the value of `sing` is available to everything inside the outer function, including the inner function. This is an example of `nested functions`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 They give us access to a wide range of functionality to utilize when writing code.
