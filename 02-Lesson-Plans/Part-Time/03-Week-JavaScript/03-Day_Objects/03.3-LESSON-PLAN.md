...
In today's class, students will learn about objects, object methods, and the keyword `this`. Local Git branching, checkout, and merging will also be introduced. The class also includes a group mini-project aimed at reinforcing and building on foundational JavaScript skills. 
...
* The last hour of the class will be devoted to the Rock, Paper, Scissors mini-project. In addition to the fundamentals covered in class, the students will be asked to research window methods `alert()`, `confirm()`, and `prompt()` on their own as well as find a way to randomize the computer's selection. This might prove challenging to some learners. If the students feel overwhelmed, encourage them to use pseudocoding as a problem solving tool. They can also refer to the documentation provided in `28-Stu_Mini-Project\README.md` for guidance.  
...
* Inform the students that at the end of today's class they will work together to build an interactive game with JavaScript.

* Tell the students that coding games is a great way to hone their skills as developers. However, writing code for games was not always as simple as it is today!

* Back in the early '60s, it took Stephen Russell, Peter Samson, Dan Edwards, and Martin Graetz over 200 hours and 40 pages of code to create the game credited as the first true digital, interactive computer game.
...
* We can still see the game run&mdash;as originally coded&mdash;using an emulator written in JavaScript.

* Open [SpaceWars!](https://spacewar.oversigma.com/) in your browser to show the students the original game.

* We can also play a remake of the game online. 
...
* Open [Play SpaceWars!](https://www.masswerk.at/spacewar/) in your browser and demonstrate the following:
  ...
  * Different actions are performed based on the user input. If a user enters W, the ship spins one way. If A is entered, the ship spins the other way.

  * The game is ended when a specific condition is met, in this case, one of the ships is destroyed. 

* Inform the students that while technologies have certainly changed, the logic pioneered by the MIT scientists is much the same as the logic we will use to build the interactive Rock, Paper, Scissors game at the end of class. 
...
* Open `21-Ins_Objects/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 To create the `planet` object, we use object literal notation and use curly brackets `{}` to hold the values, as follows:
  ...
  * 🔑 We can also use bracket notation to access a value. Don't forget that when we use bracket notation, the key goes in quotes, as shown in the following example:
...
* It's done when the `customerOrder` object has three properties that store the drink's name, number of sugars and a boolean indicating if the order is ready. 

* It's done when the drink name and number of sugars is logged to the console. 

* It's done when "Ready for pick-up" logs if the order is ready. 

* It's done when "Still in order queue" logs if order is not ready.  
...
* Open `22-Objects/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 We declare a `customerDrink` object using object literal notation and give it three properties, as follows:
...
* Open `25-Ins_Object-This/script.js` in your IDE and demonstrate the following: 
  ...
  * When we use `this` inside of an object like `planet`, the keyword `this` belongs to that object and is used to refer to the object's properties, as shown in the following example:
  ...
  * When we call an object's method, `this` refers to the object the method belongs to. Because `logPopulation` belongs to the `planet` object in the following example, `this` refers to `planet`:
...
* Open `26-Stu_Object_This/Solved/script.js` in your IDE and explain the following: 
  ...
  * When we use `this` inside of a function, `this` refers to the global object, as follows:
  ...
  * 🔑 When we use `this` inside of an object, `this` is local and belongs to that object, as shown in the following example:
...
* Open the [Atlassian guide on using branches](https://www.atlassian.com/git/tutorials/using-branches) in your browser and explain the following:

  * When we work on new features, bug fixes, or in a collaborative environment, it's a good idea that we don't work directly in the `main` branch.

  * Using Git, we can create a copy of what's in a branch and work on it in a different branch, allowing us to work in isolation and not worry about messing up the codebase.

  * This activity is all about learning the `git branch`, `git checkout`, and `git merge` commands and how we can use them to work in different branches and merge them locally. 
...
* Open `26-Stu_Mini-Project/index.html` in your browser and demonstrate the following:
  ...
  * 🔑 When we have entered the letter and press OK, we are alerted to the computer's choice. 

  * 🔑 After we press OK, we are alerted to if we won, tied, or lost. 
  ...
  * 🔑 If we press OK, the game starts again. If not, the game is exited. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How would we build this?

  * 🙋 We will start with a plan! Pseudocoding will help us figure out what the code will need to look like. Then, using the JavaScript skills learned in class and a bit of research, we will write the actual code. 
...
* Open `26-Stu_Mini-Project/Main/script.js` in your IDE and explain the following: 
  ...
  * 🔑 To play the game, the user has to enter a choice. We use a window prompt to grab the choice and store it in a variable, as follows:
  ...
  * 🔑 We use `window.alert()` to alert the computer's pick in the browser, as shown in the following example:
  ...
  * If it is not a tie or a win, then we alert that it is a loss and increment `losses`, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 The users interact through a series of pop-up boxes that prompt them to provide a choice, share a message, or confirm whether they want to play again. The window methods used to create these pop-up boxes are `window.alert()`, `window.confirm()`, and `window.prompt()`. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We will use the HTML and CSS we learned for structure and style and use JavaScript to add interactivity. 

  * ☝️ How does this project relate to your career goals?

  * 🙋 Creating interactive pages and apps is an essential skill in web development. This project will help enable you to demonstrate your JavaScript skills to a future employer. 
