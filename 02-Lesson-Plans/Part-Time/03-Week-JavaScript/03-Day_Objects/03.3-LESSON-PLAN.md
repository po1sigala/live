...
* We can still see the game run -- as originally coded -- using an emulator written in JavaScript.
...
* Open [Play SpaceWars!](https://www.masswerk.at/spacewar/) in your browser and demonstrate the following:
  ...
  * The game uses letters to input users' choices. The `w`, `a`, `s`, `d` keys control one of the spaceships. The `i`, `j`, `k`, `l` keys control the other. 
...
* Inform the students that while technologies have certainly changed, the logic pioneered by the MIT scientists is much the same as the logic we will use to build our interactive Rock, Paper, Scissors game at the end of class. 
...
* Open [Atlassian guide on using branches](https://www.atlassian.com/git/tutorials/using-branches) in your browser and explain the following:

  * When we work on new features, bug fixes, or in a collaborative environment, it's a good idea that we don't do our work directly in the `main` branch.

  * Using Git, we can create a copy of what's in a branch and work on it in a different branch, allowing us to work in isolation and not worry about messing up our codebase.
...
* Open your command line and demonstrate the following:
  ...
  * 🔑 We can create a new branch to do our work in, but it doesn't immediately take us to that new branch: 

  ```bash
  git branch <name-of-branch>
  ```

  * 🔑 If we want to move into that branch to do work, we use the "checkout command:

  ```bash
  git checkout <name-of-branch>
  ```

  * Alternatively, we may create a branch and checkout into it simultaneously:
  ...
  * 🔑 When we finish making our changes in our branch, we commit our work to it and checkout back to the branch we want this new code to be merged to, then we merge it:
  ...
  * When we're done, we can keep the feature branch or delete it:
...
* Open `26-Stu_Mini-Project/index.html` in your browser and demonstrate the following:
  ...
  * 🔑 When we have entered our choice and hit `OK`, we are alerted to the computer's choice. 

  * 🔑 After we hit `OK`, we are alerted to if we won, tied, or lost. 

  * 🔑 When we hit `OK` again, we are alerted to our stats.

  * 🔑 When we hit `OK` a final time, we are presented with the prompt "Play Again?". 
  
  * 🔑 If we hit `OK` the game starts again. If not, the game is exited. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 The users interact through a series of pop-up boxes that prompt them to provide a choice, share a message or ask them to confirm if they want to play again. The window methods used to create these pop-up boxes are `window.alert()`, `window.confirm()`, and `window.prompt`. 
