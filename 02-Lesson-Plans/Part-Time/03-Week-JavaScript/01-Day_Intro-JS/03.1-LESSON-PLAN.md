...
* Open `28-Stu_Mini-Project/Solved/script.js` in your IDE.
   ...
   *  Assure students that it is okay if the code does not look familiar just yet. This is just a preview of the decision-making code they will use to make a game by the end of the week. 

   * Point out that the Rock, Paper, Scissors game code is made up of a series of conditions that are tested to determine if a user wins, loses or ties.  
   
   * For example, if `userChoice` equals `computerChoice`, then `"It's a tie"` alerts. The tie count also increases by `1`. 
   ...
   * If instead, the `userChoice` is one that beats the `computerChoice`, then `"You win!"` alerts and the win count increases by `1`.  
  ...
  ```
  * Finally, if no tie or no win can be determined, then `You lost!` alerts and the losses count is increased by `1`.
...
* Assure students that we will delve into this JavaScript code in more detail during the week.
