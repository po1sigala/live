...
* We are dealing with client and web caching so make sure to clear your cache regularly before each activity starts or take advantage of the [incognito window in Google Chrome](https://support.google.com/chrome/answer/95464?hl=en&co=GENIE.Platform%3DDesktop#:~:text=You%20can%20also%20use%20a,Press%20%E2%8C%98%20%2B%20Shift%20%2B%20n.).
...
* Make it fun, because after this unit, students will begin the very challenging task of learning React.
...
* Remind students that the React unit is on the horizon and much of what they've learned in this unit can be applied to learning React.
...
* Run `npm run start` travel to the Application tab in Chrome DevTools and demonstrate the following:
...
* Run `npm run dev` and open up the `localhost:8080` to demonstrate the following:
  ...
  * Add an item to the TODO list and open the Application tab in Chrome DevTools. Under the `Cache Storage` section, inspect the contents of the object store.
...
* Open `28-Stu_Mini-Project/Main/client/.npmrc` in your IDE and explain the following:
  ...
  * 🔑 This tells Heroku to not purge our dev dependencies when we deploy to Heroku. We need webpack for our application to work properly, and webpack is a dev dependency.
