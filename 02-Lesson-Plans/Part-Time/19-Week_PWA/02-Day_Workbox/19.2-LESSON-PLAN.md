...
* Open `webpack.config.js` in your IDE to demonstrate the following:
  ...
  * In this file, we have set the mode to `development`. Webpack has three modes: `development`, `production` or `none`. By setting the mode, we enable webpack's built-in optimizations that correspond to each environment. 
