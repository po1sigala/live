...
* Open a new incognito window in your browser and then open the developer console in the new window. Once you have the window open with the developer console in view, navigate to `http://localhost:8080`.
  ...
  * 🔑  On the initial load, we see messages in the console that tell there was no cached response found in `static-resources`. This is in part because of the caching strategy we are using, as shown in the following snippet:
...
* Navigate to `17-Ins_Caching-Images` in your terminal and run `npm i && npm run dev`. This will install the dependencies and start the development server.
  ...
  * 🔑 This strategy is very straight forward and does what the name suggests. The service worker will always look first to the cache and fall back to the network if a cached version is not available.
...
* Open `Solved/package.json` in your IDE to demonstrate the following:
  ...
  * 🔑  We already set up an Express.js server in the `server` directory, that will serve the static HTML file found in the `client/dist` directory, so we don't need to do anything else.
