...
* Open the [slide deck](https://docs.google.com/presentation/d/1mkVX8q7pIQM6giW6ArxB2pjAYFCp2BpjIu1x7MDR6dE/edit?usp=sharing) and follow these prompts on their corresponding slides:
  ...
  * **Are PWAs dependent on a specific technology?**: No, PWAs are focused on optimizations, not specific tech
    ...
    * PWAs are not reliant on a single, specific technology. Instead, PWAs use a variety of tools to build and optimize web applications to take advantage of the features already available on the user’s device and browser.
  ...
  * **What makes an app a PWA?**: PWA design pattern optimizations.
    ...
    * PWAs are installable by users directly on their devices. A PWA uses a special set of instructions for the browser called a `manifest.json` that adds a splash screen and makes the app installable directly on the user’s device.
  ...
  * **What are the key benefits of PWAs?**
    ...
    * PWAs enhance web applications by taking advantage of user’s browser capability to provide a native app experience on any device.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We are learning Webpack, bundlers, Workbox, service workers, IndexedDB, PWAs, offline functionality to name a few.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [docs on Lighthouse performance scoring from Google](https://developers.google.com/web/tools/lighthouse), and attend Office Hours to ask for help.
...
* Navigate to `06-Stu_Webpack/Unsolved` and run `npm i && npm run watch` to create the error in the terminal. Examine the error to reveal the first bug.
  ...
  * 🔑 Open the `06-Stu_Webpack-Bundle/Unsolved/webpack.config.js` file and notice the error is in the `output` property. The output file name `entry` must be `name` in order to create multiple file names.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [webpack documentation on output file names](https://webpack.js.org/configuration/output/), and attend Office Hours to ask for help.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Babel documentation regarding plugins](https://babeljs.io/docs/en/plugins), and attend Office Hours to ask for help.
...
* Open `10-Stu_Webpack-Plugin/Solved/webpack.config.js` in your IDE and explain the following:
  ...
  * 🔑 We must add the `plugin` property to the `config` object as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [webpack documentation on HtmlWebpackPlugin](https://webpack.js.org/plugins/html-webpack-plugin/), and attend Office Hours to ask for help.
