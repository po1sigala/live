...
* Open `03-Ins_Webpack-Intro/src` in your IDE and demonstrate the following:
  ...
  * Webpack is a module bundler that will take our raw code and bundle it to optimize performance. 

  * Before we use webpack, we first have to make a change to our code. Previously, we have stored our images, CSS, and JavaScript in an `asset` folder. Now, we will place all our raw code into a source or `src` file.  Source files are used to store code that will be minified. Right now, this `src` folder only includes a JavaScript file, but we will be adding both images and CSS soon. 
...
* Open `03-Ins_Webpack-Intro/src/index.js` in your IDE and demonstrate the following:
  ...
  * For now, our app only has one JavaScript file imported. 
