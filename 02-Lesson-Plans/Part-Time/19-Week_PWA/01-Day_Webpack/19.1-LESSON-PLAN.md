...
* Open `03-Stu_Webpack-Intro/index.html` in the browser and explain the markup is using the bundle to handle user interactivity for the webpage.
...
* In the terminal, navigate to `08-Stu_Webpack/Unsolved` and run `npm i && npm run watch` to create the error. Examine the error to reveal the first bug.
...
* 🔑 Navigate to the `08-Stu_Webpack-Bundle/Solved/webpack.config.js` file and open in your IDE:
...
* 🔑 Navigate to `08-Stu_Webpack-Bundle/Solved/` in the terminal and run `npm install` and `npm run watch` to demonstrate that the error is now gone and the app is bundling as expected. 
