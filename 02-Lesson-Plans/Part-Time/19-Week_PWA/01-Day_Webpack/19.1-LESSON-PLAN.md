...
In this class, you will introduce the concept of bundling an application using webpack and its impact on webpage performance. You'll also demonstrate how to configure webpack to incorporate more performance improvements and lazy loading, as well as improve the development processes.

## Instructor Notes

* In this lesson, students will complete activities `01-Ins_Lighthouse` through `10-Stu_Webpack-Plugin` in `19-PWA`.

* You will use webpack for the majority of today's activities. Most of the activities only use the front end; there is no back end in any of the activities.
...
* In your browswer, open the [Wikipedia page on information technology](https://en.wikipedia.org/wiki/Information_technology).
...
* Open the [Lighthouse audit report for weather.com](./Images/weather-report.png) and note that the score can vary from report to report because the measurements are calculated in milliseconds and often depend on external factors such as third-party APIs, server load capacity, and internet connectivity.
...
* Open the `03-Ins_Webpack-Intro/index.html` file in your IDE to show the following script element at the bottom of the `<body>`:
  ...
  * Open `04-Stu_Webpack-Intro/index.html` in the browser to show that the markup is using the bundle to handle user interactivity for the webpage.
...
* Break your students into pairs that will work together on this activity.
  ...
  * As a developer, I should be able to create multiple bundles from the command line.
  ...
  3. Instead of a success message that states the bundles are created, an error message indicates that the bundle process failed.
...
* In the terminal, navigate to `06-Stu_Webpack/Unsolved` and run `npm i && npm run watch` to create the error. Examine the error to reveal the first bug.

  * The error message in the terminal indicates that the error is in the `webpack.config.js` file.
  ...
  * The next error in the terminal alerts that multiple bundles are being output to the same file name.

    ```sh
    [webpack-cli] Error Conflict: Multiple chunks emit assets to the same filename [entry].bundle.js (chunks main and cost)
    ```

  * 🔑 Open the `06-Stu_Webpack-Bundle/Unsolved/webpack.config.js` file and notice that the error is in the `output` property. The output file name `entry` must be `name` in order to create multiple file names.
...
* Click the "Get the latest interest rate" button and observe the following error message.

  ```sh
  Uncaught ReferenceError: regeneratorRuntime is not defined
  ```

* Open the `08-Stu_Webpack-Loader/assets/js/app.js` file in the IDE in search of the "Get latest interest rate" button click handler function that contains the runtime error.
...
* Open the `08-Stu_Webpack-Loader/webpack.config.js` file in the IDE to see if Babel was properly configured due to the async/await dependency.
  ...
  * In the terminal, navigate to `08-Stu_Webpack-Loader` and run the command `npm run watch` to create a new bundle.

  * Open `08-Stu_Webpack-Loader/index.html` in the browser and click the "Get latest interest rate" button to reveal that the function is now operational.
...
* Direct students to the activity instructions found in `10-Stu_Webpack-Plugin`, which are also shown below.
...
* Break your students into pairs that will work together on this activity.
  ...
  * It is done when the generated HTML file has a generated page title, Loan Calculator.

  * It is done when I can generate an HTML file that contains a reference to the webpack bundle.
