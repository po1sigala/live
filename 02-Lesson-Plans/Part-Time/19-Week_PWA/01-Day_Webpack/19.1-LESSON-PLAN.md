...
* Today's activities take students through setting up a webpack app and bundling files step-by-step.  As a result, for the majority of activities, you will be required to manually insert an `index.html` into the generated `dist` file and update the script's src attribute to the output filename to run the build code. 
...
* Open the `10-Stu_Webpack-Plugin/Solved/dist/index.html` in your browser using Live Server to demonstrate the following: 
  ...
  * DevTools is just one way of using Lighthouse. Lighthouse is also available as a Chrome extension and as a node module for use during development. During the first activity, we will use another Lighthouse tool, the Chrome extension, to explore metrics. 
