...
* Explain the use case npm package concurrently in the development enviroment.
...
* In the terminal, navigate to `11-Ins_Webpack-HMR` and run `npm install` and `npm run dev`. This will install the dependencies, run the development server, and open the `index.html` in the browser. 
  ...
  * 🔑 Previously when working with webpack, we ran `npm run build` to output our files to a `dist` file and manually opened the `index.html` using Live Server. When we run this application, however, the files are built and sent directly to the browser! This is because we are using a powerful development tool available with webpack: Hot Module Replacement. 
  
  * 🔑 Hot Module Replacement or HMR is used during development that allow modules to be updated at runtime without the need for a full refresh. This saves development time because we can see our changes as we make them. 
...
* Open `11-Ins_Webpack-HMR/package.json` in your IDE to demonstrate the following:
  ...
  * 🔑 When using webpack's development server, we do not *NOT* need to manually install the Hot Module Replacement plugin. Instead, we simply install `webpack-dev-server` and add the `hot` option to the `devServer` object in your webpack config.

  * 🔑 To install `webpack-dev-server` as a dev dependency, we use `npm install webpack-dev-server --save-dev`. Once installed, `webpack-dev-server` will appear the `devDependencies` section of the `package.json` file, as seen below:
...
* Navigate to `13-Ins_Client-Server` in your terminal and run `npm install` and `npm run` to demonstrate the following:
  ...
  * The concurrently npm package is a great tool that will allow us to easily spin up a Express.js server and serve up our bundled files in the `dist` folder. 

* Navigate to `13-Ins_Client-Server` in your terminal and run `npm run start:dev` to demonstrate the following:

  * When we execute the `npm run start:dev` script, concurrently runs the client app's build script to create a `dist` directory that contains an `index.html` and starts an Express.js server to serve up our `index.html` and the assets found in the `dist` directory.
...
* Open to `localhost:3001/` in your browser to explain the following:
  ...
  * Using concurrently, instead of manually opening the `index.html` in the dist directory with Live Server like we did in earlier activities or relying on webpack's dev server to serve our fles directly to the browser, we can use our own Express server to serve up the bundled app on the `localhost:3001` or whatever port we assign in our `server.js` file. 

  * For concurrently to work, however, we have to make some changes to how we organize our app. 
...
* Open `13-Ins_Client-Server` in your IDE to demonstrate the following:
  ...
  * The client directory contains the front-end files. In addition, when a `dist` file is generated during a build, it will also be located inside the client directory. Since we have been working with a front-end app, all of our current app files will be located in the client! 
  ...
  * Both the client directory and server directory are actually separate apps that function together thanks to the package.json in the root folder. This package.json contain our `npm run start:dev` script and the concurrently de dependency that allows the client and server to be spun up at the same time.  

  * 🔑 Using the client-server model allows us to separate our front-end code from our back-end code in these two directories. This creates a more modular and reusable codebase and is especially important to understand when we start working with React or other front-end framework that use bundlers like webpack! 

  * During the next activity, you will check out the client and server directories own your own. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How would we use the client-server model to modularize our app?

  * 🙋 We would start by modularizing our front-end and back-end into two directories, the client and server folders. Since our app is a front-end app, all of our existing files will go in the client directory. The server directory contains a server and HTML route that allows the `dist/index.html` to be served up easily. Finally, we can start both our client and server apps at the same time using an npm module callled `concurrently`. 
...
* Open `14-Stu_Client-Server/Solved/server/README.md` in your IDE to explain the following:
  ...
  * The `server.js` file is also at the root level. This JavaScript file contains the code needed for our basic Express.js server. At the root level is also directory containing the logic needed for our HTML route. 
...
* Open `14-Stu_Client-Server/Solved/server/routes/htmlRoutes.js` in your IDE to explain the following:
  ...
  * We access our app through the `index.html` file. Previously, we had to do this manually by opening up the dist's `index.html` using the Live Server tool. Now, using a simple Express `GET` route, we can will match requests to the root route `/` and serve up the index.html file in the client's `dist` folder.
...
* Navigate to `14-Stu_Client-Server/Solved/client/README.md` in your IDE to explain the following: 
  ...
  * At the root-level of the client directory, we typically have the client's package.json that contains the scripts and dependencies needed to run the client app, the `index.html` and configuration files such as `webpack.confj.js`. 
...
* Open `15-Ins_Workbox-Service-Workers/src/sw.js` in your IDE to demonstrate the following:
  ...
  * In the browser, we can navigate to ChromeDevTools and click on the Application tab and see that the service worker is running and what status it is in.
...
* To get a better understanding of how to create a service worker with a framework, we will be using the [GenerateSW plugin](https://developers.google.com/web/tools/workbox/modules/workbox-webpack-plugin) for webpack.
...
* Open `14-Stu_Workbox-Service-Workers/Solved/client/src/js/register-sw.js` in your IDE to demonstrate the following:
  ...
  * 🔑 To register, we need to check to make sure service workers are supported in the browser and the use the window load event to register the service worker. When working with workbox, it is important to check the docs for this code!
...
* Navigate to 17-Ins_Caching-Images and run `npm install` and `npm run start:dev` to demonstrate the following:
  ...
  * In the console, we see an log from GenerateSW indicating that our files have been precached similar to the following:
...
* Navigate to `localhost:3001` in an incognito window and open DevTools to demonstrate the following:
   ...
   * Working with a cache can be tricky especially during development. It is a good idea to do a hard refresh by holding down the refresh icon on the left side browser toolbar and clicking on "Empty Cache and Hard Reload" on a Mac or pressing `Ctrl + Shift + r` or `F5` on Windows. This will make sure you are seeing the latest changes to your app!
   ...
   * The images will have names that a long string of numbers and letters due to the bundling process. To see our cached images and determine which images are cached, we can click on the Preview mode below the table. 
   ...
   * In our `index.html`, it seems two of our images are actually external URL links. Remember, when we use GenerateSW only the images that are stored locally and bundled are included!

   * By default, when we use GenerateSW all images that have been bundled will be automatically precached when the service worker is installing. However, this is often not the behavior we want and it may even slow your app's performance down. 
   
   * That is when adding runtime caching strategy will help make your app more performant. In the next activity, you will be adding a runtime strategy to an existing app so that images are only saved once they are used on the page. As usual, don't forget to consult the documents!
...
* Open `18-Stu_Caching-Images/Solved/webpack.config.js` in your IDE and explain the following:
  ...
  * To use runtime caching, we need to add paramenters to our GenerateSW class in our `webpack.config.js`.

  * First we need to exclude our images from precaching
  ...
  * We also need to add a custom cache name and can select how many images we want to hold in the cache at the same time. This allows us to control the size of our cache. 
...
* Navigate to `18-Stu_Caching-Images/Solved/` in your terminal and run `npm install` and `npm run start` to demonstrate the following:
  ...
  * When we click on the network tab and then click on the `Change Box Image`, we can see that a network call is being used the first time our images are shown on the page. 
...
** Note:** The `dist` folder for both of these examples has been pre-generated for you. 
...
* Open `19-Ins_InjectManifest/GenerateSWexample/client/dist/service-worker.js` in your IDE to demonstrate the following:
  ...
  * 🔑 When we need more control over our service worker, we can use another webpack workbox plugin class, `InjectManifest`.

* Open `19-Ins_InjectManifest/InjectManifestExample/client/src/sw.js` in your IDE to demonstrate the following:

  * We can build our own custom service worker with more advanced scripts and caching strategies and place the file in our `src` directory.

* Open `19-Ins_InjectManifest/InjectManifestExample/client/dist/service-worker.js` in your IDE to demonstrate the following:

  * When the build script is executed, InjectManifest creates a list -- or manifest -- of URLs that are to be preacached and injects them into your existing service worker. 
...
* Open `19-Ins_InjectManifest/InjectManifestExample/client/dist/sw.js` in your IDE to demonstrate the following:
  ...
  * When we use InjectManifest, our custom service worker is otherwise bundled but unchanged. This allows us to add thec custom scripts -- and control -- we need to our service worker. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 When we want to quickly generate a service worker and don't need custom option, GenerateSW is a great and easier choice. However for more complex configurations, or when you need to interact with an API like WebPush, InjectManifest is a better option. 

* Answer any questions before proceeding to the next activity.

* In preparation for the activity, ask TAs to start directing students to the activity instructions found in `19-Stu_InjectManifest/README.md`.

### 16. Student Do: InjectManifest (15 min)

* Direct students to the activity instructions found in `19-Stu_InjectManifest/README.md`., which are also shown below.
...
### 17. Instructor Review: Caching Images (10 min)
...
* Open `20-Stu_InjectManifest/Solved/client/src/sw.js` in your IDE and explain the following:
  ...
  *  When we use GenerateSW, a complete service worker is generated for us. When we use InjectManifest, we start by creating our own service worker in the `src` directory. This file was provided for us, but is a great resource for some of the tools you can use to customize your own service worker. 

* Open `20-Stu_InjectManifest/Solved/client/src/webpack.config.js` in your IDE and explain the following:
  ...
  * 🔑 Next, we add InjectManifast last on our list of plugins and make sure to add the service worker source file, or `swSrc`. While we have added only `swSrc` here, InjectManifest has a list of customizable options that allow you to control exactly what you want to include and exclude from the cache as well as other features. Workbox has a list of these configurations in the docs! 
    ...
    ```
  * Since we have already installed `workbox-webpack-plugin` as a dependency, that is all we need to do.

* Navigate to `20-Stu_InjectManifest/Solved` and run `npm install` and `npm run start:dev`.

* Open `20-Stu_InjectManifest/Solved/client/dist/service-worker.js` in your IDE to demonstrate the following:

  * A list of URLS to be precached is generated during the build and injected into the existing service worker.  
  
* Open `20-Stu_InjectManifest/Solved/client/dist/sw.js` in your IDE to demonstrate the following:

  * The existing, custom service worker, as well as any custom configurations, is otherwise unchanged.
