...
* In the terminal, navigate to `11-Ins_Webpack-HMR` and run `npm install` and `npm run dev`. This will install the dependencies, run the development server, and open the `index.html` in the browser. 
  ...
  * 🔑 Hot Module Replacement or HMR is a valuable tool used during development that allow modules to be updated at runtime without the need for a full refresh. 
...
* Open `11-Ins_Webpack-HMR/package.json` in your IDE to demonstrate the following:
  ...
  * To use Hot Module Reloading in a development environment, we have to make sure that the script has been added to our package.json file to start our dev server and open up our app in the browser. 
...
* Open `11-Ins_Webpack-HMR/webpack.config.js` in your IDE to demonstrate the following:
  ...
  * You can also see that we have set up our webpack config to use Hot Module Replacement by adding a `devServer` options. In our `devServer` object, we have added an attribute of `hot` and set it to `only`. This will reload the module without reloading the entire page.
...
* Open `11-Ins_Webpack-HMR/package.json` in your IDE to demonstrate the following:
  ...
  * 🔑 When using webpack's development server, we do not *NOT* need to manually install the Hot Module Replacement plugin. Instead, we can simply install `webpack-dev-server` and add the `hot` option to the `devServer` object in your webpack config.

  * Under the hood, webpack calls `webpack-dev-server` when it parses the `devServer` object in the `webpack.config.js` file. We must install `webpack-dev-server` as a dev dependency using `npm install webpack-dev-server --save-dev`. Once installed, `webpack-dev-server` will appear the `devDependencies` section of the `package.json` file, as seen below:
...
* Open `12-Stu_Webpack-HMR/Solved/src/index.js` in your IDE to demonstrate the following:
  ...
  * 🔑 Even though we have set up HMR, it is by default an opt-in feature. In order to take advantage of it, we need to update the `index.js` file to accept hot modules as shown in the docs! 
  ...
  * We don't need to accept hot modules in every file. All the files that are imported into `index.js` will be hot reloaded. 
