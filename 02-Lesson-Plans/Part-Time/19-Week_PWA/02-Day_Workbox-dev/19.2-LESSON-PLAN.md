...
* During Stoking Curiosity, you will play the following short video in YouTube: ["One Last Thing" by Steve Jobs at Worldwide Developer Conference 07'](https://youtu.be/ZlE7dzoD6GA). Please remember to turn the volume up and click on the closed caption icon located on the lower left of the screen so that the most students can enjoy the 3 minute clip. 

* In `14-Stu_Client-Server` students are asked to convert an existing app to use the client-server model. This is an important concept and will be used throughout the rest of the activities! To avoid frustration with over syntax and to allow students to focus on the 'big picture', feel free to provide the students with the demo code for use during the activity time. Otherwise, you can also point the students to the remaining activities to the day to help the students the the syntax needed. 
...
* Add Hot Module Reloading to a webpack app to improve the development experience.
...
* First, though, we will learn about Hot Module Replacement, an easy-to-use tool that can help speed up development.
...
* In the terminal, navigate to `11-Ins_Webpack-HMR` and run `npm install` and `npm run dev`. This will install the dependencies, run the development server, and open the `index.html` in the browser. 
  ...
  * 🔑 Previously when working with webpack, we ran `npm run build` to output our files to a `dist` file and manually opened the `index.html` using Live Server. When we run this application, however, the files are built and sent directly to the browser and not to a `dist` folder. This is because we are using Hot Module Replacement. 
  
  * 🔑 Hot Module Replacement or HMR is a valuable tool used during development that allow modules to be updated at runtime without the need for a full refresh. This saves development time! 
...
* Open `11-Ins_Webpack-HMR/src/js/module1.js` in your IDE to demonstrate the following:
    ...
    * 🔑 The `module1.js` file is providing the content to our live page. To change the content, we can update the appended HTML so that inside the span element, the text reads: `"THIS IS NEW CONTENT"`.
    ...
    * We can then save the `module1.js` file and navigate back to our opened browser page to see the changed content. Sometimes, working with HMR can be tricky, so if this doesn't work for you right away, try refreshing or opening your app in an incognito window. 

* Open `11-Ins_Webpack-HMR/package.json` in your IDE to demonstrate the following:

  * To add Hot Module Reloading to our webpack app, we first need to make sure that the script has been added to our package.json file to start our dev server and open up our app in the browser. We add the `--open` tag so that our app will open up in our browser automatically. 
...
* Open `11-Ins_Webpack-HMR/package.json` in your IDE to demonstrate the following:
  ...
  * 🔑 We install `webpack-dev-server` as a dev dependency using `npm install webpack-dev-server --save-dev`. Once installed, `webpack-dev-server` will appear the `devDependencies` section of the `package.json` file, as seen below:
...
* Open `12-Stu_Webpack-HMR/Solved/package.json` in your IDE and explain the following:
  ...
  * 🔑 In our scripts, we've also added a `dev` command to run `webpack-dev-server --open`. This command will run the `webpack-dev-server` and open the project in the browser. Optionally, if you don't like this behavior, you can use `--no-open` to prevent the browser from opening automatically. 
...
* Open `12-Stu_Webpack-HMR/Solved/webpack.config.js` in your IDE to demonstrate the following:
  ...
  * Now that we have our script to start the server and have installed the `webpack-dev-server` package, we need to add a `devServer` object to our `webpack.config.js` file. This object contains all of the configuration options for our development server, which gets invoked by the `dev` command.
...
* Open `12-Stu_Webpack-HMR/Solved/src/index.js` in your IDE to demonstrate the following:
  ...
  * 🔑 Even though we have set up HMR, it is by default an opt-in feature. In order to take advantage of it, we need to update the `index.js` file to accept hot modules. We found the syntax for the `module.hot.accept()` method in the docs! 
...
* Navigate to `13-Ins_Client-Server` in your terminal and run `npm install` to install all dependencies and demonstrate the following:

  * When we enter `npm run` we can see list of all the scripts defined in the root package.json file including `npm run start:dev`. This script uses an npm module called `concurrently` to start the client and server directory at the same time. This will allow us to use our own server to easily serve up our `index.html` file. 

   ```json
   "start:dev": "concurrently \"cd client && npm run build\" \"cd server && npm run server\" "
   ```

  * When we enter `npm run start:dev`, using the concurrently npm module, we navigate into the client directory and run the `npm run build` from the client's `package.json` file. Then, we start the server to serve up the html route by navigating into the server directory and running `npm run server`.

* Navigate to `localhost:3001` to explain the following:

  * In our last activity, we served up the files directly to the browser using webpack dev server. In this app, when we run `npm run start:dev` we are creating a `dist` directory that contains an `index.html` and then -- instead of manually opening the `index.html` with Live Server like we did in earlier activities -- we are using our own Express server to serve up the app on `localhost: 3001`. 

* Open `13-Ins_Client-Server` in your IDE to demonstrate the following:

  * 🔑 When we take a look inside the `13-Ins_Client-Server`, we see we see that we have two main directories: a client directory and a server directory. 

   * 🔑 Using the client-server model, allows us to separate our front-end code from our back-end code in these two directories. This separation will allow us to create a more modular and reusable codebase, especially when we start working with React or any other front-end framework that uses a bundler such as webpack.
...
* Navigate to `13-Ins_Workbox-Service-Workers` in your terminal, run `npm install` and `npm run dev` to demonstrate the following:
