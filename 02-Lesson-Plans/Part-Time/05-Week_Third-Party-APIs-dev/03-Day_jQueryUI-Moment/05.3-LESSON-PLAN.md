...
* Complete activities `21-Ins_jQuery-UI-Widgets` through `28-Stu_Mini-Project`.
...
* Open the [jQuery UI Docs on Dialog](https://jqueryui.com/dialog/) in your browser and demonstrate the following:
...
* Open `21-Ins_jQuery-UI-Widgets/dialog-docs.html` in your IDE and demonstrate the following:
  ...
  * Here we see the code for the dialog widget. Let's go through the code and tweak it a bit.
...
* Open `22-Stu_jQuery-UI-Widgets/Solved/assets/js/script.js` in your IDE and explain the following: 
  ...
  * 🔑 If we look at the [jQuery UI Docs on Autocomplete](https://jqueryui.com/autocomplete/), we see that we need to create an array for the `.autocomplete()` function to find and select from. Everyone's array of skills will be different but will look something like the following example:
  ...
  * 🔑 If we look at the [jQuery UI Docs on Datepicker](https://jqueryui.com/datepicker/), we see that we have a lot of choices in terms of customizing it. For the purposes of this activity, we want to show the month and year in drop-downs in place of the static month/year header. 
  
  * So we look for the ["Display month & year menus"](https://jqueryui.com/datepicker/#dropdown-month-year) option and apply the additional Boolean options, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, review the [jQuery UI Demos](https:\//jqueryui.com/demos/), and stick around for office hours to ask for help.

* Answer any questions before proceeding to the next activity.

### 5. Instructor Demo: jQuery UI Interactions (5 min) 

* Open the [jQuery UI Docs on Sortable](https://jqueryui.com/sortable/) in your browser and demonstrate the following:
...
* Open `23-Ins_jQuery_UI_Interactions/sortable-docs.html` in your IDE and demonstrate the following:
  ...
  * Again, let's clean up the `<head>` section by importing a custom stylesheet, moving the `<style>` to that stylesheet, and moving the `<script>` tags down to the bottom of the page, as follows:
  ...
  * Lastly, notice what the sortable function is doing&mdash;it is looking for an element with an id of `sortable` and applying the `.sortable()` function to it. Right afterwards, it will `.disableSelect()`, as follows:
...
* Open `24-Stu_jQuery-UI-Interactions/Solved/assets/js/script.js` in your IDE and explain the following: 
  ...
  * According to the [jQuery UI Docs on Sortable](https://jqueryui.com/sortable), to allow the user to drag and reorder the skills listed under List of Skills, we need to use the sortable interaction.
  ...
  * 🔑 However, we also want to add the option of seeing the whitespace between the items shifting. According to the ["Drop placeholder"](https://jqueryui.com/sortable/#placeholder) option, we can do just that, like in the following example:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Moment.js Documentation](https://momentjs.com/docs/), and stick around for office hours to ask for help.
...
* Open [Git Revert Docs](https://git-scm.com/docs/git-revert) in your browser and explain the following:
  ...
  * Let's say that you are working with your classmates on a project and you are trying to fix a bug. You find out that the bug was caused by a single commit. Then you can use `git revert` to go fix it and commit a new snapshot. 
...
* If necessary, you can review this activity. Open your command line and demonstrate the following:
    ...
    * Let's see this in action! I'll create a new directory for us to test this in and `cd` into it. Then we will initialize an empty Git repo in that directory to practice in.
...
* Open the newly created `git_revert_demo` directory in your IDE and copy the `27-Evr_Git-Revert/script.js` file into it.
  ...
  * `git revert` requires a commit reference and will not execute without one. Here we pass in the `HEAD`, which will revert the latest commit, as follows:

    ```bash
    git revert HEAD
    ```

  * This will create a new commit, which will open the commit message editor prompting for a new commit message. When Git is finished, we can use `git log` to see that there is a new commit added, which reverted the `added Sortable interaction` commit.
  ...
  * If we look at the [Git Revert Docs](https://git-scm.com/docs/git-revert#_options), we see various options that we can also use. The `--no-edit` option is a useful one that will not open the commit message editor and will automatically write a commit message for you.

  * We can try this out! Let's say we want to revert the `added Autocomplete widget` commit. After we run `git log` we need to get the commit number and use it in the `git revert`, as follows:

    ```bash
    git revert --no-edit <add commit number>
    ```

  * When we run `git log`, we can see the automatically generated commit message `Revert "added Autocomplete widget"`. When we check the `script.js` file, the widget is no longer there.
...
* Create a Bootstrap card component explaining the instructions of how to use the app and a button to open a [Bootstrap modal dialog](https://getbootstrap.com/docs/4.5/components/modal/).
...
* While you build, remember the following:
...
* Open `28-Stu_Mini-Project/Solved/index.html` in your IDE and explain the following: 
  ...
  * 🔑 We can find the general code for the modal in the official docs for Bootstrap modals. In the modal, we will put in a form. Notice the id for the input fields, shown in the following example:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the official documentation (like the [MDN web docs on the HTML input element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input) or the [Bootstrap documentation on components](https://getbootstrap.com/docs/4.5/components/modal/)), and stick around for office hours to ask for help.
