...
* Open `23-Ins_jQuery_UI_Interactions/sortable-docs.html` in your IDE and demonstrate the following:
  ...
  * Here we see the code for the sortable interaction taken straight from the documentation. Let's go through the code and tweak it a bit.
...
* Refer to the official [Moment.js documentation on parsing](https://momentjs.com/docs/#/parsing/) to format ISO 8601 strings.
...
Read the [Moment.js documentation](https://momentjs.com/docs/) to research this.
```
...
* Open the newly created `git_revert_demo` directory in your IDE and copy the `27-Evr_Git-Revert/script.js` file into it.
  ...
  * If we look at the [Git documentation on git revert](https://git-scm.com/docs/git-revert#_options), we see various options that we can also use. The `--no-edit` option is a useful one that will not open the commit message editor and will automatically write a commit message for you.
