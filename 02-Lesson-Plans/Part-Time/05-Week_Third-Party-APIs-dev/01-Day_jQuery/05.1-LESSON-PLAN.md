...
* Open the [slide deck](https://docs.google.com/presentation/d/1-TxVbjIGv8jMD4pcEtjonVPs8qItpUNHUCfIGMAErxg/edit?usp=sharing) and follow these prompts on their corresponding slides:
  ...
  * **Are there APIs we can use that aren’t provided by the browser?**
    ...
    * The web is filled with tools that other developers have built and made available for us to use in our projects! These third-party APIs exist to help us carry out specific tasks that we can do ourselves with CSS or JavaScript, but may be too complex or time consuming for us to do by hand.

  * **How can we learn to use and implement these APIs?:** Since these APIs are provided by developers (like us!) of all different backgrounds, there is no one correct way to learn how to use them. 

    * What’s important is that you identify what problem they solve for you and how to properly integrate it into your existing application.

    * Some ways to help learn about these APIs is to read the official documentation and practice with the provided examples;
...
* In this unit, we will learn how to integrate these third-party APIs with our applications. Specifically, we'll learn how to work with jQuery, Bootstrap, Google Fonts, jQuery UI, and MomentJS!
...
* Open `28-Stu_Mini-Project/Solved/index.html` in your browser.
  ...
  * We can see how much money we may make on the project based on our hourly rate and potential project completion date.
...
🙋 Most applications developers work on rely heavily on third-party tools and libraries such as these, so it's important to learn how to use them.
...
* Open `04-Stu_Click-Events/Solved/assets/js/script.js` in your IDE and explain the following: 
    ...
    * We create our variables first, then our functionality, then our event listener bindings. This to ensure everything exists before we put it to use in our JavaScript.
...
* Open `05-Ins_Form-Elements/assets/js/script.js` in your IDE and show the following:
    ...
    * Since there can be more than one checked item, we need to select the checked elements and store them in an array using the jQuery `$.each()` method.
...
* Open `09-Ins_Event-Delegation/assets/js/script.js` in your IDE and demonstrate the following:  
    ...
    * 🔑 We must delegate the event listener to the parent element, `<div id="buttons">`, because the letters are not available when the `document` loads.
