...
* Open [Wireframe example from Wikipedia](https://en.wikipedia.org/wiki/Website_wireframe#/media/File:Profilewireframe.png) in your browser and explain the following:
  ...
  * 🔑 Wireframes are created early on in the development process for developers to have a rough idea of how the page layout will look and feel like to the user.

  * 🔑 Wireframes do not need to be fancy. Like the example, they can be sketched by hand or if you want to take a digital approach you can use a tool like Figma or Photoshop.

* Open `21-Ins_Wireframing/index.html` in your browser and demonstrate the following:

  * 🔑 Our application is nearly finished but we can reverse engineer it to see how we could have used a wireframe to help us achieve the same result. 

  * Each element now has border with a corresponding color to indicate how our element is positioned on the page.
