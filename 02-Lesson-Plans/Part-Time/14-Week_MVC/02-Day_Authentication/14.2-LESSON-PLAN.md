...
* Open <http://localhost:3001/> in your browser to demonstrate the following:
  ...
  * **Important:** Adding cookie settings allows us to change some settings associated with the cookie that gets sent to the client immediately, despite adding any additional cookie options to the session object. With some changes however, we can change how our session is identified on the server whenever sessions are used. Cookies allow us to store information about the session on the user's client.
