...
| 1:55  | 12  | Instructor Demo: Session Storage  | 0:05     |
| 2:00  | 13  | Student Do: Session Storage       | 0:15     |
| 2:15  | 14  | Instructor Review: Session Storage| 0:10     |
...
### 12. Instructor Demo: Session Storage (5 min)
...
### 13. Student Do: Session Storage (15 min)
...
### 14. Instructor Review: Session Storage (10 min)
...
* Open `18-Stu_Session-Storage/Solved/server.js` in your IDE and explain the following:
  ...
  * 🔑 We first need to initialize Sequelize with the session store which, by default, uses cookies. To do that, we need to install and import `connect-session-sequelize`. The `connect-session-sequelize` library automatically stores the sessions created by `express-session` into the database! See the following code for an example:
