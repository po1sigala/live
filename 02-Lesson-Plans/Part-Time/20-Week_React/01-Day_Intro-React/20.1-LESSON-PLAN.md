...
* Run `npm start` from the command line and demonstrate the following:
  ...
  * 🔑 Components are independent and reusable bits of code. They are simply JavaScript functions that return JSX expression that render as HTML. Components come in two types, Class components and Function components.
  ...
  * 🔑 Every function has access to a `props` argument. A prop is always an object that contains all of the values passed to the function.
