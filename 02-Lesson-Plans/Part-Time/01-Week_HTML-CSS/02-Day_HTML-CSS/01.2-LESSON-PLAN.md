...
* Open `07-Ins_Attributes/index.html` in your IDE and demonstrate the following:
  ...
  * Now let's set up the HTML document to use CSS so that we can apply the rules written in the style sheet to the webpage. We will add a `<link>` element to connect the CSS to the HTML and add global attributes to identify elements for styling. 

  * The `<link>` element connects the HTML page to an external CSS style sheet. Within that element, the `href` attribute specifies the relative path, and the `rel` attribute describes the relationship: 
  ...
  * The `id` attribute can also be used to select and style an element, by providing that element with a unique identifier, but it should only be used on a single element: 
...
* Open `09-Ins-CSS-Color/assets/style.css` in your browser and demonstrate the following:
  ...
  * 🔑 A style sheet is a list of CSS properties that defines how HTML elements will be displayed.
