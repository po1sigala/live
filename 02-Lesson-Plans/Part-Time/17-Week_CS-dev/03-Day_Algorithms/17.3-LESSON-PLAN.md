...
* Since [Big O notation](https://www.bigocheatsheet.com/) is difficult to explain in an activity, spend some time at the beginning of class as well as after each algorithm review to help students visualize the differences in time complexities and performance.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Regex is a powerful tool we can implement, just like an algorithm. Knowledge in it will make us more confident in ourselves both on the job and during a technical interview.
