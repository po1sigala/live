...
* Open `02-Stu_Global-Execution-Context/Solved/assets/js/script.js` in your IDE and explain the following: 
  ...
  * A conditional statement evaluates the `confirm()` to execute `goFast()`.  Since `confirm()` is a Web API, once it has resolved, the function `goFast()` is then placed on the callback queue, then is allowed to get pushed onto the call stack to be executed by the event loop.

  1.  When is `speed` assigned a value? Where is this value stored?
