...
* Open `13-Ins_Closures/index.js` in your browser and demonstrate the following:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We could conceal sensitive variable that shouldn't be mutated outside of its function's scope.
...
* Open `15-Ins_Factory-Functions/index.js` in your IDE and demonstrate the following:
  ...
  * 🔑 Factory functions use closures to encapsulate its variable, making it difficult to mutate the data from outside of its function scope.
  ...
  * 🔑 Inside both our newly created functions we return `console.log` statements using template literals.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 @TODO We can refer to supplemental material, read the [MDN Web Docs on Closures](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Closures), and stick around for office hours to ask for help.
