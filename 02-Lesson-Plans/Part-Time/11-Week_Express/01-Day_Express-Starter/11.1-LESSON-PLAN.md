...
This week, students will learn how to create server-side APIs using the Express.js back-end framework for Node.js. In this unit, they will learn how to create an API, define the routes, perform back-end logic, and save data using the built-in Node.js `fs` module.

## Instructor Notes

* In this lesson, students will complete activities `01-Ins_Setup` through `28-Stu_Mini-Project`.
...
* Open `01-Ins_Setup/server.js` in your IDE to demonstrate the following:
  ...
  * Additionally, a root path sends the client a string that instructs them to visit `/send` or `/routes`:
    ...
    **Note**: Because we set up middleware for the `public` folder, Express.js automatically looks for an `index.html` in the `public` folder to send when a user visits the root path. Since we do have an `index.html` in the `public` folder, we're seeing that in our browser now. If we didn't have an `index.html` file in the `public` folder, we'd see the text message of the response, `Navigate to /send or /routes`.
...
* Open `localhost:3001` in your browser to demonstrate the following:
  ...
  * 🔑 When we run the application and visit the `localhost:3001/` page, we are presented with a definition for an API.

  * 🔑 This example also includes another endpoint, `localhost:3001/api`.
...
* Open `localhost:3001` in your browser to demonstrate the following:
  ...
  * 🔑 When we run this application and visit `localhost:3001`, we find a single button that reads **Fetch terms**.
...
* Break your students into pairs that will work together on this activity.
  ...
  * As a user, I want to be able to view pet data on the page by pressing a **Fetch pets** button.
  ...
  1. Open the page by visiting <http://localhost:3000>.
...
* Open `localhost:3001` in your browser to demonstrate the following:
  ...
  * When we run this application and visit `localhost:3001`, we notice that there is a button that doesn't seem to do anything.
...
* Open `09-Ins_Static-Assets/server.js` in your IDE to demonstrate the following:
  ...
  * To use middleware in your Express.js application, use the `app.use()` function to mount the `express.static` middleware at the `public` directory.
    ...
    > **Important:** You may have noticed that some of our previous activities used `express.static` without being properly introduced. Use of `express.static` in prior activities was necessary to serve HTML and CSS required for the activity. This demo and the following activity will give some context as to what this middleware does, and why it is required.
...
* Open `localhost:3001/images/cat.jpg` in your browser to demonstrate the following:
