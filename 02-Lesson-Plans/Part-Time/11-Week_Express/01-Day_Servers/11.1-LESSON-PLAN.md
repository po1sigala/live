...
* Briefly run through the code and explain that this app is going to log the type of request it receives, along with any information that was sent with the request. We will test it out using a new application called Insomnia Core.

* Have students download the application [Insomnia Core](https://insomnia.rest/). Go to the [Insomnia Core website](https://insomnia.rest/) and follow the instructions for downloading the application for your operating system. The site mentions pricing, but we're going to use the free tier of the application, as those features will really cover all of our needs!
...
> **Important:** You can refer the students to the [Getting Started with Insomnia page](https://support.insomnia.rest/article/11-getting-started).
>
> Don't forget to bookmark the [Insomnia Core documentation](https://support.insomnia.rest/) as well!

* Let's start by clicking New Request button. You can also click the plus sign (+) in the left column and select New Request.
...
Below that is a subnavigation that allows us to enter more information about the request, as shown below:

![Insomnia Core with dropdown menu showing subnavigation](./Images/04-Insomnia-request.png)

Under the Body dropdown menu, choose Other. Let's send a POST request with the body text "Hello World".

> **Important:** Make sure you turn on your server using `npm start` before testing.

* We should get a `200 OK` status returned but no body. However if you check the console, you will see the following message:
...
* Let's do a GET request. Change the type of request method to GET and remove the body text. Click Send and you should see the following message in your console:
