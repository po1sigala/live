...
* In this lesson, students will complete activities `21-Ins_Local-Storage` through `27-Stu_Timer-App`.
...
* Use JSON methods to convert an object to and from a JSON string
...
* 🔑 To retrieve the string and convert it back into an object, we use `JSON.parse()`
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can use an object and then convert it to a string to be stored.
...
* Open `25-Ins_Local-Storage-Todos/assets/js/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 We want the data retrieved and rendered when we load the page. So, we put it in a function called `init()` that holds the code we want executed when the page first loads. If data has previously been saved using `localStorage`, the data will appear on the page. 
...
* Open `26-Stu_Local-Storage-Todos/Solved/assets/js/script.js` in your IDE and explain the following: 
  ...
  * 🔑 To store an array using `localStorage`, we first stringify it using `JSON.stringify()`. Then we set it to storage using `.setItem()`.
  ...
  * 🔑 To retrieve the data from storage, we use `.getItem()` and convert the data from a string to an array using `JSON.parse()`. To use the array in our code, we store it in a variable.

  ```js
  var storedTodos = JSON.parse(localStorage.getItem("todos"));
  ```

  * We use `.createElement()` and `.appendChild()` to render our todo list. Since we are going to reuse this code, we use a function.
  ...
  *  When a user enters a todo, the data is stored using `localStorage`. The todos are rendered to the page by calling the `renderTodos()` function.
  ...
  * We also call the `storeTodos()` and `renderTodos()` functions when the `complete` button is clicked. 
  ...
  * 🔑 We declare an `init` function that will fire when the browser window is open. Inside of that function, we put our code to retrieve the data. We also use `renderTodos()` to have the retrieved data, if it exists, render on the screen. 
...
* In preparation for the activity, ask TAs to start directing students to the activity instructions found in `@TODO/folder/file`. {TODO NEXT Sprint}
...
* Open [Git docs](https://git-scm.com/docs/GitHub-Contributing-to-a-Project) in your browser and explain the following:
  ...
  *  In this activity, we'll practice creating a new repository on Github, pulling down the repo to our local machine using `git clone`, and pushing up changes using `git push`. We will also explore pull requests by opening a pull request,  reviewing the request, and merging the changes.
...
* Open your terminal and demonstrate the following:
  ...
  * 🔑 We create a new branch. This is the branch where we will make our changes. 
...
* Open your IDE and demonstrate the following:
  ...
  * We open up VSCode and navigate to the README to make a change. 
...
* Open your terminal and demonstrate the following:
  ...
  * 🔑  In the terminal, we add and commit our change. Then, we push it up to the remote branch making sure we use the name of the branch we just created. 
...
* Open your browser and navigate to [Github](https://github.com/) and demonstrate the following:
  ...
  * 🔑 We pick the branch we want to compare the changes against as our `base` branch. We pick `main`.

  * 🔑 We pick the branch we want to compare. That is the new branch we just created or `01-my-new-feature`/

  * 🔑 We will in the form and click on `Create Pull Request` to create the request.
...
* Open `27-Stu_Mini-Project/main/index.html` in your browser and demonstrate the following:
  ...
  * 🔑 The users wins and losses appear on the screen. 
...
* Open `27-Stu_Mini-Project/assets/js/script.js` in your IDE and explain the following: 
...
* In the `checkLetters` function, we use conditonal statements to test if the letter is a match. 
...
* We also use condiitonal statements to check if there is a win.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 The ability to manipulate the DOM and build an interactive app is an essential skillset for developers. The quiz app is also a great way to showcase our growing JavaScript skills in our portfolios. 
