...
* Open `13-Ins/Preventing-Default/assets/js/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 The action we want peformed when a user clicks on an element has been separated into its own, named function. We add it as the second parameter of the `.addEventListener()` method by using the name of function.
...
* Open `14-Stu_Prevent-Default/Solved/assets/js/script.js` in your IDE and explain the following: 
  ...
  * 🔑 Inside the `addTip` function, we add `event.preventDefault()` to the top of the code block. This will stop the browser from clearing the information in the text fields by default. All the code we wrote in the function will still fire as usual. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We use `event.preventDefault()` to stop the default behavior. This can be useful for blocking default behavior in our code.
