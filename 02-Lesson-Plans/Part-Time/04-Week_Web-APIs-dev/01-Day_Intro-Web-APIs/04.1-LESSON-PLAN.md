...
* Open `09-Ins_Timers-Intervals/index.html` in your browser and demonstrate the following:
  ...
  * 🔑 The timer will evaluate an expression at a set interval until it is cleared. 
...
* Open `09-Ins_Timers-Intervals/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 We call`sendMessage()` after the interval is cleared, so the image will pop up after the countdown is complete. The `sendMessage` function creates and appends the image and only executes after the countdowm is done.
...
* Open `10-Stu_Timers-Intervals/Solved/script.js` in your IDE and explain the following: 
  ...
  * We set a variable when creating the `.setInterval()` method. The variable name will be used as the parameter when we call `.clearInterval()` to stop the execution. 
  ...
  * Else, we use the `.textContent` property to set the text of the `<div>` to the current word. The process repeats until no words remain.
