...
* The last hour of the class will be devoted to the Rock, Paper, Scisssors  mini-project. In addition to the fundamentals covered in class, the students will be asked to research window methods `alert()`, `confirm()`, and `prompt()` on their own as well as find a way to randomize the computer's selection. This may prove challenging to some learners. If the students feel overwhelmed, encourage them to use pseudocoding as a problem solving tool. They can also refer to the documentation provided in the README {TODO NEED FILE} for guidance.  
...
* Ask the class the following question (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We learned about adding JavaScript to HTML files, console logs, variables, primitive types, logical operators, comparison operators, and conditional statements on the first day. We also learned arrays, iteration, array methods, functions and scope.

* As the students answer, make a list to visually reinforce how much they have learned in just one week.  

* Inform the students that using the concepts learned, they are ready to start building fully functional apps that use HTML, CSS and JavaScript to create an interactive experience for the user. 

* Remind the students that at the beginning of the week, we took a look at the JavaScript used to create a simple Rock, Paper, Scissors game. 

* Open `28_Stu_Mini-Project/Solved/script.js` in your IDE.

* Tell the class this is the same code as they saw at the beginning of the week. 

* Ask the class the following question (☝️) and call on students for the answers (🙋):

  * ☝️ Does the code look more familiar? What JavaScript concepts are used in the code? 

  * 🙋 The code uses logical and comparison operators, variables and arrays. 

* As the students name the concepts used, take a moment to point out their locations in the code to the class.  

* Assure the students that it is okay if they still don't recognize everything in the code. At the end of the class, they will have a chance to get hands-on and work in a group to create their own Rock, Paper, Scissors game to solidify the concepts.   

* Transition to the first activity by telling students that before tackling the mini-project, we are first going to learn about something that is found nearly everywhere in JavaScript: objects. 
...
* Open `21-Ins_Objects/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 To create our `planet` object, we use object literal notation. We start with `var` and the assignment operator `=` and use curly brackets `{}`to hold the values.

  ```js
  var planet = {...};
  ```
  * 🔑We use key-value pairs to give the characteristics a name and a value.   

  ```js
  name: "Earth"
  ```
  * 🔑 The association between the key and value is known as a property. Objects usually have many related properties that help to define the object. 
...
* 🔑 To access the value stored in a key-value pair, we use dot notation. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MDN Web Docs: Objects](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object), and stick around for Office Hours to ask for help.
...
* Open `23-Ins_Object-Methods/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 To access an element in an array contained in an object, use the object's name and key plus the index of the element. 
  ...
  * 🔑 To call a object's method, we use the object's name followed by the assigned key. Don't forget the `()`! 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 A method is a function associated with an object. It is also a property of the object and just like other properties, follows the key-value pair format. 
...
### 7. Instructor Review: Activity Methods (10 min)  
...
* Use the prompts and talking points below to review the following key (🔑) points:
  ...
  * ✔️ Access array within object 

  * ✔️ Access array element within object

* Open `24-Stu_Object-Methods/Solved/script.js` in your IDE and explain the following: 

  * The chosenPet variable holds the name of of the chosen pet. 

  ```js
  var chosenPet = "Lulu";
  ```

  * The `shelter` object contains two arrays and one method. 
  ...
  * We access a variable by using its name, not dot notation. Removing `.Lulu` logs the message.  
  ```js
  function dogMessage() {
  ...
  * 🔑 When we access a method without `()` it returns the function definition but no code is run!  We add `()` to execute the `apptMessage` function and log the message.  
  ...
  ```
  * 🔑 We access an entire array in an object by using the object's name and the key.
  ...
  * 🔑 We access an element in the array using the object's name, key and the element's index.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MDN Web Docs: Working With Objects](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Working_with_Objects), and stick around for Office Hours to ask for help.
...
* When we call an object's method, `this` refers back to the object the method belongs to. Since `logPopulation` belongs to the `planet` object, `this` refers to `planet`. 
...
* Open `26-Stu_Object_This/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 When we use `this` inside of an object, `this` belongs to that object. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MDN Web Docs: This](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this), and stick around for Office Hours to ask for help.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋We can refer to supplemental material, read the [MDN Web Docs: Window Prompt](https://developer.mozilla.org/en-US/docs/Web/API/Window/prompt), [MDN Web Docs: Window Confirm](https://developer.mozilla.org/en-US/docs/Web/API/Window/confirm), [MDN Web Docsl Window Alert](https://developer.mozilla.org/en-US/docs/Web/API/Window/alert) and stick around for Office Hours to ask for help.
