...
* Open `21-Ins_Objects/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 In JavaScript, objects are like objects in real life. They are 'things' that have distinct characteristics or properties. For our first JavaScript object, the 'thing' we are going to create is a planet. 

  * 🔑 To create our `planet` object, we use an object literal. We start with `var` and the assignment operator `=` and use curly brackets `{}`to hold the values.
...
* 🔑 We can also use bracket notation. Don't forget when we use bracket notation, the key needs to go in quotes! 
...
* Open `22-Objects/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 Dot notation is one way to use the object's name and associated key to access a value stored in an object.
  ...
  * 🔑 Another way is by using bracket notation. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What is unique about how objects store values? 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋We are going to use the JavaScript skills we learned this week.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How can we use `window.prompt()`, `window.alert()` and `window.confirm()` to add interactivity to our code? 
...
* Open `02-Homework/Master/index.html` in your browser and demonstrate the following:
  ...
  * This password generator takes criteria selected by the user and utilizes it to generate a random string. 
