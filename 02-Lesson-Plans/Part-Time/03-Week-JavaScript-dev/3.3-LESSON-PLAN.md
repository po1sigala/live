...
* Open `@TODO/folder/file` in your IDE and explain the following: 
  ...
  * 🔑 To play the game, the user has to enter their choice. We use a window prompt and store it in a variable. 

  ```js
  var userChoice = window.prompt("Enter R, P, or S:");
  ```

  * We only want the game to run if the user has made a choice and does not hit cancel. This block of code is stored in a function.
  ...
  * To make a match, the `userChoice` must be capital letters. So, we use a string method to make sure all choices are uppercase. 
 ...
 * 🔑 We use Math.floor() combined with Math.Random() to select a random index from our `options` array. 
  ...
  * 🔑 This index is then used to assign the computer a choice. 

  ```js
  var computerChoice = options[index];
  ```

  * 🔑 We use window.alert() to alert the choices in the browser. 
