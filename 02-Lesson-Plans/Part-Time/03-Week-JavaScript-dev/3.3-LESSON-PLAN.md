...
* Ask the class the following question (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We learned about adding JavaScript to HTML files, console logs, variables, primitive types, logical operators, comparison operators, and conditional statements. We also learned arrays, iteration, array methods, functions and scope.

* As the students answer, make a list to visually reinforce how much they have learned in just one week and congratulate them on their achievement.   

* Inform the students that using the concepts learned, they are ready to start building interactive apps that use HTML, CSS and JavaScript.

* Remind the students that at the beginning of the week, we took a look an interactive Rock, Paper, Scissors game. 
...
* Take a moment to point out the concepts named in the code to the class.  

* Assure the students that it is okay if they don't recognize everything in the code just yet. At the end of the class, they will have a chance to dig into the code and work in a group to create their own Rock, Paper, Scissors game.
...
* Use the prompts and talking points below to review the following key (🔑) points:
  ...
  * ✔️ Accessing array within object 

  * ✔️ Accessing array element within object
...
* Open `24-Stu_Object-Methods/Solved/script.js` in your IDE and explain the following: 
  ...
  * We access value store in a avariable by using just its name, not dot notation. Removing `.Lulu` logs the message.
  ...
  * 🔑 When we access a method without `()` it returns the function definition but the code is not executed!  We add `()` to execute the `apptMessage` function and log the message.  
  ...
  ```
  * 🔑 We access an entire array within an object by using the object's name and the key.
  ...
  * 🔑 We access an element using the object's name, key and the element's index.
