...
* Open `13-Ins_Iteration/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 A for statement starts with the keyword `for` followed by parentheses`()`. Inside the parentheses, there are three expressions: an initializing expression, a conditional expression, and an incrementing expression. 

  ```js
  for (var i = 0; i < 5; i++) 
  ```
  * The initializing expression declares a variable. That variable is used as the counter for the loop. Since we want to start at `0`, we assign the value to be `0`. 

  ```js
  var i = 0;
  ```

  * The second expression is the conditional expression. The conditional expression determines if the loop will execute. If the expression evaluates to `true`, the loop will execute. If not, it won't.

  ```js
  i < 5;
  ```

  * The third expression is the incrementing expression. After the block of code executes one time, we want `i` to increase by `1`. To do this, we use the increment operator `++`.

  ```js
  i++
  ```

  * 🔑 Next, the block of code we want to execute each time the loop runs goes in curly brackets `{}`.
  ...
  * 🔑 For loops are useful for iterating over arrays! 
  
  * 🔑 We use the array's `length` property to determine when the loop should stop. In this case, as long as `i` is less than the length of `zooAnimals`, the loop will continue. However, as soon as the loop reaches the end of the array and `i` is equal to the length of `zooAnimals`, the loop will stop.  
  
  ```js
  for (var i = 0; i < zooAnimals.length; i++) 
  ```
  * 🔑 We can also `i` to access the elements of the array by placing `i` in brackets following the array's name. Let's see this in action in the console. 
...
* Open `13-Ins_Iteration/index.html` in your browser and navigate to the console to demonstrate the following:
  ...
  * In this loop, our counter variable `i` is initially assigned the value `0`. So, the first log shows the value of `i` is `0`. Then, after the block of code is executed, `i` is incremented. So, when it runs again, `i` is 1 and so on until the condition `i < 5` is no longer true. 
...
* Open `14-Stu_Iteration/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 The second statement is a condition that uses the `students` array's `length` to determina when to stop. If this condition evaluates to `true`, the loop will continue executing. If not, the loop will stop. 

  ```js
  i < students.length;
  ```

 * Finally, after the block of code is executed, we want `i` to increment by `1` so we use `++` to incremeent `i` by `1`.
