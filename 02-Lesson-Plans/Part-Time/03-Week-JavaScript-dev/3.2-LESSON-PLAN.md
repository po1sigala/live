...
* Open `13-Ins_Iteration/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 A for loop starts with the keyword `for` followed by parentheses `()`. Inside the parentheses are three statements. The first assigns `i` a starting value. The second provides the condition when the loop will run. The third statement either increments or decrements `i`.

  ```js
  for (var i = 0; i < 5; i++) 
  ```

  * 🔑 The block of code that will be repeated goes in curly brackets `{}`. This block of code will be executed every time the loop runs. 
  ...
  ```
  * 🔑 For loops are an excellent way of performing an action on all the elements in an array. 
  
  * 🔑 Since we want the loop to stop when al
...
* Open `14-Stu_Iteration/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 The first statement sets a starting value for `i`.

  ```js
  var i =0;
  ```

  * 🔑 The second statement is a condition. If this condition evaluates to `true`, the loop will continue executing. If not, the loop will stop. 
