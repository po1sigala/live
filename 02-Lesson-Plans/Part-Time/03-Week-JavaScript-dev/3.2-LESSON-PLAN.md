...
* Open `13-Ins_Iteration/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 A for statement starts with the keyword `for` followed by parentheses`()`. Inside the parentheses, there are three expressions: an initializing expression, a conditional expression, and an incrementing expression. 
  ...
  for (var i = 0; i < 5; i++) 
  ```
  * The initializing expression declares a variable. That variable is used as the counter for the loop. Since we want to start at `0`, we assign the value of `i` to be `0`. 
  ...
  * The final expression is the incrementing expression. After the block of code executes one time, we want `i` to increase by `1`. To do this, we use the increment operator `++`.
  ...
  * 🔑 For loops will execute the block of code again and again until the conditional expression evaluates to `false` and each time it does, `i` will increase by one. This makes for loops great for iterating over an array.

  
  * 🔑 When iterating over an array, we use the array's `length` property to determine when the loop should stop. In this case, as long as `i` is less than the length of `zooAnimals`, the loop will continue to execute the code block. 
