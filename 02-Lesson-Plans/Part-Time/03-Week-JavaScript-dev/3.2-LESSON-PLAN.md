...
* Open `11-Ins_Array/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 To store multiple values, we again use `var` and `=`. But this time, we use brackets `[]` to hold our data. The data is separated by a comma `,`.
  ...
  * 🔑 To access a single value or element in an array, we use the item's index or place in the array. Using the index `1`, will log `Karl`. 

  ```js
  console.log(names[1]);
  ```

  * 🔑 Arrays are zero-indexed! So the first value in an array has an index of `0`, not `1`. Using `0` will log `Andre`.
