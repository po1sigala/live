...
* Open `13-Ins_Iteration/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 A for loop starts with the keyword `for` followed by parentheses `()`. Inside the parentheses are three statements. The first assigns `i` a starting value. The second provides on the condition on when the loop should run. The third statement either increments or decrements `i`.

  ```js
  for (var i = 0; i < 5; i++) 
  ```

  * 🔑 The block of code to be executed again and again goes in curly brackets `{}`.
  ...
  ```
  * 🔑 Loops are also great to iterate over arrays. 
  
  * 🔑 To determine how many times the block of code should repeat, we use the array's length to set the condition.  
...
* Open `13-Ins_Iteration/index.html` in your browser and navigate to the console to demonstrate the following:
  ...
  * So when we use `i` to access an array element, the first time the block of code executes the element at index `0` will log, the next time the element at index `1` will log and so on. 
