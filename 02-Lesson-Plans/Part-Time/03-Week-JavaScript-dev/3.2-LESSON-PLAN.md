...
* Open `15-Ins_Functions/script.js` in your IDE and demonstrate the following: 
  ...
  * 🔑 One way we can define a function is to use a function declaration.

  * 🔑 A function declaration starts with the keyword `function` followed by a name and parentheses `{}`.
  ...
  * 🔑 Adding parameters inside the `()`, only gives them names, not values! The values are assigned when the function is called. So in this case `x` is `7`, `y` is `"Hello" and `z` is `true`. 
