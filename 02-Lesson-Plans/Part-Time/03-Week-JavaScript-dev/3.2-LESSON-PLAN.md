...
* Open `13-Ins_Iteration/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 We use a for loop to execute a block of code more than once.

  * 🔑 A for loop starts with the keyword `for` followed by parentheses `()`. Inside the parentheses are three statements. The first assigns `i` a starting value. The second provides on the condition on how many times the the loop should run. The third statement either increments or decrements `i`.

  ```js
  for (var i = 0; i < 5; i++) 
  ```

  * 🔑 The action that we want executed for each loop goes in curly brackets `{}`.
  ...
  ```
  * 🔑 We often use a for loop to iterate over an array. To determine how many times the loop should execute, use the array's length.
