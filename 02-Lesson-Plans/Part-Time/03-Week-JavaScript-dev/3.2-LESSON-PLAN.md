...
* Open `11-Ins_Array/index.html` in your browser and navigate to the console to demonstrate the following:
  ...
  * When we click on the arrow to the left of the data, the indices of each element in the array is seen. The first element has an index of `0`. The last element has an index of `3`.
...
* It is done when "REPLACED_NAME is in class" is logged when an `if statement` is used to test if the first element in the array has been replaced. 
