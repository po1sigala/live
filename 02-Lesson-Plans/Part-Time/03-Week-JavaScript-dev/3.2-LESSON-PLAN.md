...
* Open `11-Ins_Array/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 Since arrays are variables that hold multiples values, we also declare arrays using `var` and `=`. The data is contained in brackets `[]`. Inside the brackets, the data is separated by a comma `,`.
  ...
  * 🔑 To access a single value or element in an array, we use the item's index or place in the array. This will log `Karl`. 
