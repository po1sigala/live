...
* Open `19-Ins_Arrays/index.html` in your browser and demonstrate the following:

  * 🔑 Arrays have built-in methods that allow us to manipulate arrays and the elements they hold. 

  * 🔑 Array methods are easy to use. We use the name of the array followed by a dot `.` and the name of the method with parentheses `()`. This will call the `sort()` method on the `comparisonOperators` array and sort the elements. 

  ```js
  comparisonOperators.sort();
  ```

  * 🔑 Some array methods need parameters. The `push()` method attaches the value `%` to the end of the `arithmaticOperators` array. 
  ...
  * 🔑 Methods are not limited to arrays. Strings also have methods. This will replaces "String" with "World" and returns new string named `myNewString`.

  ```js
  var myNewString = myString.replace("String", "World");
  ```

   * 🔑 These are just a few of the array and string methods available to use in our code. The best way to learn about array and string methods is to look at the documentation. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ What does an array method do? 

  * 🙋 An array method is a built in way for us to easily manipulate arrays and the elements it contains.

  * ☝️ If we want to figure out how to use a specific array method, what should we do? 

  * 🙋 There are a lot of array methods. So we should look at the documentation to learn to apply the right documentation to our code.
