...
* Open `O3-Ins_Hello-Variable/script.js` in your IDE and demonstrate the following:
  ...
  * To access the value held in a variable, we use the variable name. 
  ...
  * Using the variable name is useful in our code when we want to access the value held in the variable, for example in console logs.  
  ...
  * When we want to use a value stored in a variable, we do things a little bit differently. Instead of putting the value directly in between the `()`, we put the variable name -- without quotes -- instead. Since the value of `studentName` is currently `Tonya`, the console log below will log `Tonya`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * ☝️ How do you access a value stored in a variable? 
