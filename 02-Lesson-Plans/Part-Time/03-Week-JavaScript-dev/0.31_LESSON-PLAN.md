...
* Open `08-Stu_Logical-Comparison-Operators/Solved/index.html` in your IDE and explain the following: 
    ...
    * In this example, the variable `a` has the type of string while variable `b` has a type of number. As a result, they are not strictly equal (===). However, since they have the same value, in this case `50`, double equals (==) returns true.
