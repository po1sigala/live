...
* There is a lot of information in today's lesson that may be new to many students. If you find that the students are feeling overwhelmed, take a moment to point out that that these are the building blocks of JavaScript development and each activity builds on one another so the concepts will be revisited again and again. They can look back at previous activities for guidance or reach out during office hours for help.
...
* Open `10-Stu_Conditional-Statements/Solved` in your IDE. Note there is a new file type, the `script.js`.

* Open `10-Stu_Conditional-Statements/Solved/index.html` in your browser and quickly point out that the `index.html` file is just an HTML file. Then navigate to the console to show them the Javascript at work. Stress that in the first lesson, we will be learning how to add JavaScript to our HTML files and write messages to the console like those shown.
...
* Run `01-Ins_Script-ConsoleLog\index.html` in the browser and show the students how to navigate to the console in Chrome DevTools. Point out that both the console log from the inline JavaScript and external file appear.  
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We used variables in our CSS to store CSS values that we wanted to reuse in our code.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel with creating a JavaScript variable using `var`? (Poll via Fist to Five, Slack, or Zoom)
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel with primitive types boolean, string, number and undefined? (Poll via Fist to Five, Slack, or Zoom)
...
* Open `08-Stu_Logical-Comparison-Operators/Solved/index.html` in your IDE and explain the following: 
    ...
    * In this example, the variable `a` has the type of string while variable `b` has a type of number. As a result, they are not strictly equal ``===`` and using `===` returns false.  However, since they have the same value, in this case `50`, since double equals ``==`` only evaluates value, using `==` returns true.
...
* Open `09-Ins_Conditional-Statements/script.js` in your IDE and make comments on the code to point out the following:
  ...
  * 🔑 With an if/else statement, if the statement evaluates to true, the action after the curly brackets is performed. Else, the second action is performed.
   ...
   * 🔑 If there are more than one statement to be evaluated, `else if` is used. Unlike `else`, `else if` takes a statment of its own.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 The action after the curly bracket is performed.
