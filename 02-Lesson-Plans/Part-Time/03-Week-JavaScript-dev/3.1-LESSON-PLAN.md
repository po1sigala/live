...
* Open `01-Ins_Script-ConsoleLog\index.html` in your IDE and note the following: 
  ...
  * 🔑 We add an src attribute to the opening `<script>` tag allows you to link to an external JavaScript file.

  ```html
  <script src="script.js"></script>
  ```

  * 🔑 We write `<script>` tags above the closing `</body>` tag to make sure your HTML and CSS load first.
...
* Open `O3-Ins_Hello-Variable/script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 When we declare a variable, it only gives it a name. To assign a value, we use the assignment operator `=` followed by the value you want the variable to hold. 
...
* Open `04-Stu_Hello-Variable/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 The variable's values are accessed using the variable name. Since we have assigned the variables the values `"Sakura"`, `3` , `"I like pineapple on my pizza."`, those are the values that are used in the console logs below. 
