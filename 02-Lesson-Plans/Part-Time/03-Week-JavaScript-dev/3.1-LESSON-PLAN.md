...
* Tell the students that today, they will start learning the tools needed to implement one of JavaScript's key uses: decision-making using conditional statements. 

* Open `28-Stu_Mini-Project/Solved/script.js` in your IDE.

   *  Assure students that it is okay if the code does not look familiar just yet. This is just a preview of the decision-making code they will be able to use to make a game by the end of the week. 

   * Point out that the Rock, Paper, Scissors game code is made up of a series of conditions. If those conditions evaluate to `true`, then something happens. 
   
   * For example, in the code below, if `userChoice` equals `computerChoice`, then the number of `ties` is increased by one and an alert reading `It's a tie!` appears.
   ...
   * If not, another conditions is tested.
  ...
  ```
  * If none of the conditions are met, then `You lost!` alerts.
...
* Open `28-Stu_Mini-Project/Solved/index.html` and follow the prompts to enter `R, P, or S` to show the students the functionality of the game.

* Assure students that during class we will delve into this JavaScript code in more detail during the week.
...
* I am done when I open the `index.html`, navigate to the console, and see the message contained in the `script.js` file logged to the console. 
...
* I am done when I store a fun fact in a variable called `funFact` and this line is logged to the console: "Fun fact: `VALUE_STORED_IN_VARIABLE_FUNFACT`."

* I am done when I re-assign the values of `name`, `pets`, and `funFact` with my new partner's information the logs in the console reflect the new values.
