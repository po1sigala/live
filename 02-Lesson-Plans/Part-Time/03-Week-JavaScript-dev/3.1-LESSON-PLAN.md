...
* Open `09-Stu-Logical-Comparison-Operators/Solved/script.js` in your IDE and explain the following: 
  ...
  * Since the `||` operator requires only one expression or the other to evaluate to `true`, this returns `true`.

  ```js
  console.log(expression1 || expression2);
  ```
  
  * Alternately, it is possible to negate `expression2` so that it returns `true`. Since, both expressions now evaluate to true thanks to the `!` operator, this returns `true`.

  ```js
  console.log(expression1 && !expression2);
  ```

  * Remove `!` negation from the first expression so that it returns `true`. Since one of the expressions, now evaluates to `true`, this returns `true`. 
...
* Open `09-Ins_Conditional-Statements\script.js` in your IDE and demonstrate the following:
  ...
  * 🔑 The `!` logical operator can also be used to negate the expression. Since `!Lunchtime` evaluates to false when `!` is added, `"It's Lunchtime Already !!` is logged.
...
* Open `10-Stu_Conditional-Statements/Solved/script.js` in your IDE and explain the following: 
  ...
  * 🔑 The action performed depends on the values inputted for the variables `x`, `expression1` and `expression2`. Since `x` has a value of `50`, both `expression1` and`expression2` currently evaluate to `false`. So since none of the conditions are `true`, the expected action is `False ❌ False ❌` is logged. 
