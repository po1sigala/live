...
* Open the [slide deck]() and follow these prompts on their corresponding slides:
  ...
  * **Mini-Project**: The mini-project for this unit will be a contact directory app that uses service workers, IndexedDB, webpack, and has PWA functionality. The app will also deploy the app to Heroku.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Yes, we will be using CRUD operations to persist and delete data. We will set up an Express.js server to serve the presentation layer.

  * ☝️ How does this project relate to your career goals?

  * 🙋 Not only do apps need to look impressive, they must also be fast and load quickly. In the real world connectivity can be unstable, so provisions must be made to preserve the app's functionality to retain users. 

* Answer any questions before proceeding to the next activity.

### 2. Instructor Demo: Lighthouse (5 min) 

* Open [weather.com](https://weather.com/) in your browser.

* Open the DevTools/Lighthouse tab.

* Generate a report and explain the following:

  * To get started, we'll focus on the main driver for high bounce rates, which are users that leave the web site without any page interaction - slow page loads.

  * Scroll down to the Performance section and note the overall score is rated very bad.

  * 🔑 Select the slide button to display the longer descriptions of the performance metrics. 

    ![The metrics long descriptions are displayed along with the link that describes the solution.](./Images/describe-metrics.png) 

  * Examine First Contentful Paint, Time to Interactive, Total Blocking Time, and Speed Index.

  * Follow the link for [Time To Interactive](https://web.dev/interactive/?utm_source=lighthouse&utm_medium=devtools) to obtain more details and the corrective methods to improve performance.

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How do we generate an audit report?

  * 🙋 Open the Lighthouse tab in DevTools and select the generate report button.
...
* Break your students into pairs that will work together on this activity.
  ...
  4. Add comments to the markdown file in the [audit.md] for which metrics are most problematic in regards to performance and how to fix them.(./Unsolved/audit.md)
...
* Open `02-Stu_Lighthouse/Solved/audit.md` in your IDE and explain the following: 

  * 🔑 Time to interactive was incredibly slow
