...
* Open a new incognito window in your browser and then open the developer console in the new window. Once you have the window open with the developer console in view, navigate to `http://localhost:8080`.
  ...
  * 🔑  On subsequent visits, we will see a message in the console that tells us that we have a cached response in `static-resources`. This is a result of the cached response being returned, as shown in the following screenshot:
    ...
    ![Chrome based browser console showing output from workbox](./Images/cache_response.png)
...
* Open `18-Stu_Caching-Images/Solved` in your IDE and explain the following:

  * 🔑  In order to cache images, we need to use the `CacheFirst` caching strategy. This strategy needs to be imported from the `workbox-strategies` package. We added this on to our existing import for the `staleWhileRevalidate` strategy, as shown in the following code:

    ```js
    import { StaleWhileRevalidate, CacheFirst } from 'workbox-strategies';
    ```

  * 🔑  The `CacheFirst` strategy is a good choice for images since the same images are often requested multiple times in a short period of time.
  ...
  * Any requests that have a destination of `images` will be handled by the `CacheFirst` strategy, as shown below:
  ...
  * 🔑  The `ExpirationPlugin` is a plugin that will automatically remove stale assets from the cache. This plugin needs to be imported from the `workbox-expiration` package, as shown in the following snippet:
  ...
  * We can also use the `maxEntries` option to set the maximum number of assets that can be stored in the cache. This will prevent the cache from growing too large, as shown below:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What are some challenges you encountered with caching images?
  ...
  * 🙋 We can refer to supplemental material, read the [Workbox Docs on caching images](https://developers.google.com/web/tools/workbox/guides/common-recipes), and stick around for office hours to ask for help.
...
* Navigate to `19-Ins_Client-Server` in your terminal and run `npm i` to install all dependencies and demonstrate the following:
  ...
  * Instead of opening the browser right away, run `npm run` in the terminal to list the available commands.
  ...
  * 🔑  The reason we have two directories is because we want to separate our front-end code from our back-end code in order to prepare for the future. This separation will allow us to create a more modular and reusable codebase especially when we start working with React or any other front-end framework that uses a bundler like Webpack.

  * 🔑  One thing we need to think about is deployment. Let's say we had an application that had an express server and a front end client. We would need some way to spin up the server and the client on the same machine using a single command, while also have the server and client running on different ports.
...
* Direct students to the activity instructions found in `20-Stu_Client-Server/README.md`.
...
* Navigate to `20-Stu_Client-Server` in your terminal and run `npm i && npm run start`. This will install the dependencies and start the development server.
  ...
  * 🔑  When we run this command, we notice that our terminal beings to start both the client's start command and the backend Express.js server.
    ...
    ```console
    concurrently "cd server && npm run server" "cd client && npm run start"
    ```

  * 🔑 We can see that because of our root level `start` command, we invoke `concurrently` to run both the client and the backend server at the same time.
...
* Open `Solved/server/server.js` in your IDE to demonstrate the following:
  ...
  * Notice that the static HTML route is configured to serve the contents if tge `client/dist` directory, as shown in the following code:
  ...
  * There is also a single HTML route that serves the contents of the `client/dist/index.html` file, as shown in the following snippet:
...
* Open `Solved/package.json` in your IDE to demonstrate the following:
  ...
  * 🔑  We already set up an express server in the `server` directory, that will serve the static HTML file found in the `client/dist` directory, so we don't need to do anything else.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Open-CLI Docs on Concurrently](https://github.com/open-cli-tools/concurrently), and stick around for office hours to ask for help.
