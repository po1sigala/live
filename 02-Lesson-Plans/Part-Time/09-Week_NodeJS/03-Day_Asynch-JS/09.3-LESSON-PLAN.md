...
* ECMAScript itself is a programming language, but as far as we are concerned, it is just a language from which syntax rules are inherited. We can find a more detailed history in the [Wikipedia article on ECMAScript](https://en.wikipedia.org/wiki/ECMAScript).
...
* Use the prompts and talking points (🔑) below to review the following key points:
  ...
  * ✔️ The syntax for the `for...of` loop reads very similar to plain english which helps us conceptualize what is happening in the program.
...
* Open [Ins_Rest_and_Spread](../../../../01-Class-Content/09-NodeJS/01-Activities/23-Ins_Rest_and_Spread/index.js) in your browser.

* Run `node index.js` from the command line and demonstrate the following: 

  * 🔑 When we run the file, we get output for a few different operations: without the rest operator, with the rest operator, without spread, and with spread.
  ...
  * One way to think of this is like pouring out items from a cup. The only difference is that the items are variables and the cup is an iterable.
...
* Use the prompts and talking points (🔑) below to review the following key points:
  ...
  * ✔️ During this exercise we also used something called `reduce`, which is used in this case to calculate the sum of an array. Let's look at the [MDN Web Docs on reduce](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/Reduce) briefly before we review.
...
* Next we modify the addition function to make use of the rest operator. This allows us to pass in as many arguments as we need. This is particularly useful in this case where we want to add as many numbers as necessary. The code should resemble the following example:
  ...
  * 🔑  There is a good chance that you might be confused on where spread and operators go, because they are very similar and can be used in similar situations. This is just an introduction and they will begin to pick up fluency as they get more practice.
...
* Open [25-Ins-ObjDestructuring](../../../../01-Class-Content/09-NodeJS/01-Activities/25-Ins_ObjDestructuring/index.js) in your browser.
...
* Run `node index.js` from the command line and demonstrate the following: 
  ...
  * 🔑 We also notice that in the file, we are using dot notation to access variables inside an object, as we have in the past. We are also plucking off certain variables and setting them equal to the value of the object. This is done with syntax that uses curly braces on the left side of the equals sign, as shown in the following example:
...
* Use the prompts and talking points (🔑) below to review the following key points:
  ...
  * ✔️ We can also destructure function parameters. This allows us to name them directly and again save a few steps.  
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋  We can refer to supplemental material, look for examples on Google, and stick around for office hours to ask for help.
...
* Open [28-Stu_Mini-project](../../../../01-Class-Content/09-NodeJS/01-Activities/28-Stu_Mini-project/README.md) in your browser and demonstrate the following:
...
* Use the prompts and talking points (🔑) below to review the following key points:
  ...
  * ✔️ We can use the `fs.writeFile` method to create and write to the needed `index.html` file.

* Open [28-Stu_Mini_project/Basic](../../../../01-Class-Content/09-NodeJS/01-Activities/28-Stu_Mini-project/Solved/Basic/index.js) in your IDE and explain the following: 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [{ DOCS }]({ URL }), and stick around for office hours to ask for help.

* Answer any questions before proceeding to the next activity.

### 16. Instructor Demo: Introduce Homework (5 min)

* Open [02-Homework](../../../../01-Class-Content/09-NodeJS/02-Homework/README.md) in your browser and demonstrate the following:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 This will help us learn how to use Node's `fs` module and also an added benefit of keeping readme files looking fresh.

* Ask TAs to direct students to the Homework Requirements found in [02-Homework/README.md](../../../../01-Class-Content/09-NodeJS/02-Homework/README.md).
