...
[09.2 ES6 Time Tracker](https://docs.google.com/spreadsheets/d/1B7WLtwYw1hkHrrkwf3mavXbSnh3tiyGaIsGKgxg49x8/edit?usp=sharing)
...
```
  ...
  * Then work on explaining the differences between each of these methods:
    ...
    * `filter` is similar to `forEach`, but it returns a brand new array using elements in the original array. If an element's callback function returns a truthy value, it is added to the newly returned array, otherwise it is _filtered_ out and doesn't get added to the new array.
...
* Students will now be introduced to template literals, a new feature in ES6.
...
* There are many ways to iterate in JS but the real power of for...of is the flexibility it provides.
...
* Explain that the for...of loop is very flexible and we will continue to learn about it's use cases as we progress through the course.
...
* The syntax is relatively straightforward, the key takeaway is knowing when to use and and what to use it for.
...
* Open [27-Ins_Spread_and_Rest/index.js](../../../../01-Class-Content/09-NodeJS/01-Activities/27-Ins_Rest_and_Spread/index.js) and step through the code.
...
* Explain that although it is possible to call a function with any number of arguments, only the first two arguments will be counted.
...
* This first part is relatively straightforward, but check for understanding to see if there are any hangups for students.
...
* This one might have been a little tricky for students. There is a chance that they have not seen `reduce` before, so take a second to explain what it does.
...
* Explain that object destructuring is a way for us to pull properties off of an object and onto separate variables for easier access. We can achieve this without ES6, but now we have a newer shorthand for doing so.
