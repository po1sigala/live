...
* Use the prompts and talking points below to review the following key (🔑) points:
  ...
  * ✔️ Point out that even thought we converted a few of the examples over to use arrow function, the code works exactly as it did before
  ...
  * ✔️ Note that we can remove parenthesis around the arrow function if the function only accepts one parameter. This is optional however. if students feel more comfortable with the older syntax using curly braces and the return keyword is completely valid and acceptable as well.
  ...
  * ✔️ Point out that when working with objects, we want to avoid using arrow functions while writing methods. The two takeaways here:
    * Most cases arrow functions can replace regular functions.
    * Arrow functions shouldn't be used in object methods.
