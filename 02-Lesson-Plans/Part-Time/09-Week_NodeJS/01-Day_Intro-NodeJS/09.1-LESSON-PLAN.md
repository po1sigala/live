...
### 7. Arrow function practice (10 mins) 
...
### 11. Instructor Review: Functional Loops (10 mins) 
...
* Use the prompts and talking points below to review the following key (🔑) points:
  ...
  * ✔️ In this demo we will introduce some new array methods:
    * `forEach`
    * `filter`
    * `map`

  * ✔️ Explain that `filter` and `map` are used heavily in React when rendering elements from array data. Students have probably seen `forEach` already, but it's a good stepping stone for understanding `filter` and `map`.

* Open [index.js](../../../../01-Class-Content/09-NodeJS/01-Activities/21-Ins_FunctionalLoops/index.js) in your IDE and explain the following: 

  * In the following example we have a new type of for loop called a `forEach`. This loop runs a callback on each element in the array.
  ...
  * `filter` is very similar to `forEach`, but it leaves the original array untouched. Instead, `filter` returns a brand new array using elements in the original array. If an element's callback function returns a truthy value, it is added to the newly returned array, otherwise it is _filtered_ out and doesn't get added to the new array. Consider this example:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ Which of these array methods mutates the original array, and which return a new array?

  * 🙋 A `forEach` is the only method of the three that mutates data inside the array. `filter` and `map` will return a new array.
