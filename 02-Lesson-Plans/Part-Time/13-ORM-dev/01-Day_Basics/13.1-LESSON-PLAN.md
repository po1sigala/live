...
* Run `npm start` from the command line and demonstrate the following: 
  ...
  * 🔑 In the PUT route we specify the endpoint to match the `isbn` of a book inside of our SQL database.
  ```js
    router.put('/:isbn', (req, res) => {
  ```

  * 🔑 Inside the PUT route we call the `Book` model's `update()`. The firstarguement is a list of fields in your database and the coordinating data from the request body.
  ```js
  Book.update(
    {
      title: req.body.title,
  ...
  * 🔑 The `update()` method takes a second arguement using the `where` to specify which book should be updated. The `isbn` is being retrieved from the request parameters.
  ```js
  {
    where: {
   ...
   * 🔑 TSequelize is Promise-based, so we add a then() and catch() to return the newly created book or an error message.
  ...
  * 🔑 In the DELETE route we specify the `isbn` endpoint. Call the `Book` model's method `destroy` which accepts an object. Use the `WHERE` clause to find the book that needs to e deleted based on the request parameters.
  ...
  * 🔑 Open Insomnia Core and make a `DELETE` request to the `localhost:3001/api/isbn` with following data in JSON format.
  ```json
  {
      "isbn": "9780137043293"
    },
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋  `UPDATE book SET column1 = value1, column2 = value2, ... WHERE isbn`
...
* Open `08-Stu_Update-Delete/Solved/routes/api/bookRoute.js` in your IDE and explain the following: 
  ...
  *🔑 We use the `update()` method on the `Book` model mapping our request body to the proper fields in our database.
...
* 🔑 We add optional parameters to the `update()` method to specify which book we want to update with the `WHERE` clause. We use our request parameters to retrieve the `book_id` of the desired book.
...
* We then send our response with our updaed book or catch any errors that may occure.
  ...
  * 🔑 Inside the DELETE route use the `destroy()` method on `Book` model. We use the `WHERE` clause to filter the book we want to delete from the `library-db`.
 ...
 * 🔑 Point out how express uses the `delete` keyword for DELETE but Sequelize uses the `destroy` keyword. This can be an easy mistake that can cause your route to not properly work.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 e can refer to supplemental material, read the [Sequelize docs on querying](https://sequelize.org/master/manual/model-querying-basics.html), and stick around for Office Hours to ask for help.
...
* Open `09-Ins_Async-Await/routes/api/bookRoutes.js` in your browser and demonstrate the following:
  ...
  * 🔑 To make a function asynchronus we have to add the `async` keyword infront of our callback function. This tells JavaScript to return a `promise` instead of returning the value. 
...
router.get('/', async (req, res) => {
  ...
  * 🔑 Inside of our function's body we need to store our `Book` model's `findAll()`method inside of a variable named ``bookData`. Infront of the `Book` model you place the `await` keyword. The `await` keyword can only be placed an `async` function. It tells JavaScript to wait untill the promise fullfills itself. Lastly, we return our response with the `bookData`.
...
* 🔑 This makes our asynchronus code read and run more like synchronus code. We have the added benefit of not having to deal with long `then()` and `catch()` chains.
...
* Direct students to the activity instructions found in `0-Stu_Async-Await/README.md`.
 ...
 * 🔑  Travel took the `seeds` folder and into the `bookSeeds.js` file. Explain that this the proper way to seed a database versuses creating a `/seed` route like we did earlier.

 * We have to import our data from the JSON files inside the `seeds` folder to use inside our new asynchronus function.
 ...
 * 🔑 We first need to add the `async` keyword to our `seedDatabase` function. This tells the function to return a promise.
   ...
   * 🔑 Since we are not running this function everytime our Express server starts we need to sync our servers before we seed our database. We don't need to store it in a variable because we have no reason to return it.
