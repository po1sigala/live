...
* Run `npm start` from the command line and demonstrate the following: 
  ...
  * 🔑 In the DELETE route we specify the `isbn` endpoint. Call the `Book` model's method `destroy` which accepts an object. Use the `where` keyword to find the book that needs to e deleted based on the request parameters.
