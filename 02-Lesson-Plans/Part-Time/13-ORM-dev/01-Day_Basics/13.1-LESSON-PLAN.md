...
* Run `npm start` from the command line and demonstrate the following: 
  ...
  * 🔑 Rename the `.env.EXAMPLE` file and fill in your credentials. Run `npm start` from the `07-Ins_Update-Delete` directory. Open Insomnia Core and make a PUT request to `localhost:3001/api/isbn` with following data in JSON format.
  ```json
  {
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋  `UPDATE book SET title = Pragmatic Prgrammer WHERE isbn`
...
router.get('/', async (req, res) => {
 ...
 ```
   ...
   * 🔑 `await` does not prevent the code running while it waits for the promise. It does the contrary, it allows other parts of the code to run but once the promise is fulfilled it will return its value.
...
* 🔑 Rename the `.env.EXAMPLE` file and fill in your credentials. Run `npm start` from the `09-Ins_Update-Delete` directory. Open Insomnia Core and show all request to `localhost:3001/api/` still work.
