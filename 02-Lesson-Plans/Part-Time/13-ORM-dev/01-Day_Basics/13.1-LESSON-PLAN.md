...
* Run `npm start` in the `01-Ins_Sequelize-Setup` directory from the command line and demonstrate the following: 
  ...
  * 🔑 Sequelize executes a simple test query to verify the database connection.
...
* Open `02-Stu_Sequelize-Setup/Solved/config/connection.js` in your IDE and explain the following: 
  ...
  * 🔑 We imported the `dotenv` module and immediately executed its `config()` method, which allows our `.env` variables to be read by the Node.js script.
...
* Open `03-Ins_Models/models/Book.js` in your IDE and explain the following:
  ...
  * 🔑 We did not need to specify an `id` field, because Sequelize will generate a primary key automatically. In the future, however, it would be better to define it ourselves so we can retain more control over our model.

  * 🔑 We pass a second object to the `init()` method to link the model to the `sequelize` connection object, which is required for the model to work. We can optionally define other settings in this same object.
...
* Open `07-Ins_Update-Delete/routes/api/bookRoutes` in your IDE and demonstrate the following: 
  ...
  * 🔑 For the PUT route we specify the endpoint to match the `isbn` of a book inside of the database.
  ```js
    router.put('/:isbn', (req, res) => {
  ```

  * 🔑 Inside the PUT route we call the `Book` model's `update()` method. The first parameters it takes are list of fields in the database and the coordinating data from the request body.
  ```js
  Book.update(
    {
      title: req.body.title,
  ...
  * 🔑 The `update()` method takes a second parameter using the `WHERE` clause to specify which book should be updated. The `isbn` is being retrieved from the request parameters.
  ```js
   {
    where: {
   ...
   * 🔑 Sequelize is Promise-based, so we add `then()` and `catch()` to return the newly updated book or an error message.
  ...
  * 🔑 Rename the `.env.EXAMPLE` file and fill in your credentials. Run `npm start` from the `07-Ins_Update-Delete` directory. Open Insomnia Core and make a PUT request to `localhost:3001/api/books/:isbn` with following data in JSON format.
  ```json
  {
  ...
  * 🔑 Run the GET route in Insomnia to show students the update.

  * 🔑 In the DELETE route we specify the `isbn` endpoint. Call the `Book` model's method `destroy` which accepts an object as one of its parameters. Use the `WHERE` clause inside the object to find the book that needs to be deleted based on the request parameters.
  ...
  * 🔑 Open Insomnia Core and make a DELETE request to `localhost:3001/api/books/:isbn` with following data in JSON format.
  ```json
   {
      "isbn": "9780137043293"
    },
  ...
  🔑 Run the GET route in Insomnia to show students the book 'Essential Scrum: A Practical Guide to the Most Popular Agile Process' was deleted

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ What is the plain SQL equivelent to `update()`?
...
* Open `08-Stu_Update-Delete/Solved/routes/api/bookRoute.js` in your IDE and explain the following: 
  ...
  *🔑 Use the `update()` method on the `Book` model mapping our request body to the proper fields in the database.
...
* 🔑 We add optional parameters to the `update()` method to specify which book we want to update with the `WHERE` clause. We use the request parameters to retrieve the `book_id` of the desired book.
...
* We then send our response with our updated book or catch any errors that may occur.
  ...
  * 🔑 Inside the DELETE route use the `destroy()` method on the `Book` model. We use the `WHERE` clause to filter the book we want to delete from the database.
 ...
 * 🔑 Point out how express uses the `delete` keyword for DELETE routes but Sequelize uses the `destroy` keyword. This can be an easy mistake that can cause your route to not properly work.
...
* Open `09-Ins_Async-Await/routes/api/bookRoutes.js` in your browser and demonstrate the following:
  ...
  * 🔑 To make a function asynchronous we have to add the `async` keyword in front of our callback function. This tells JavaScript to return a promise instead of returning a value. 
...
router.get('/', async (req, res) => {
 ...
 ```
   ...
   * 🔑 We call the `update()` method on the `Book` model with a objact mapping the request body to fields in the database. The `await` keyword does not prevent the code from running while it waits for the promise to fufill. It does the contrary, it allows other parts of the code to run but once the promise is fulfilled it will return its value.
 ...
 * 🔑 This makes our asynchronous code read and run more like synchronous code. We have the added benefit of not having to deal with long `then()` and `catch()` chains.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What are some of the benefits of writing your code with `async / await`?

  * 🙋 Look for: Cleaner code, less `then()` chains, easier to read code. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel with `async` / `await`? (Poll via Fist to Five, Slack, or Zoom)
...
* Use the prompts and talking points below to review the following key (🔑) points:
  ...
  * ✔️Seeding with Sequelize

* Open `10-Stu_Async-Await/Solved/seeds/bookSeeds.js` in your IDE and explain the following: 

 * 🔑  Navigate to the `seeds` folder and into the `bookSeeds.js` file. This the proper way to seed a database with Sequelize versuses creating a `/seed` route like we did earlier.
 ...
 * 🔑 We first need to add the `async` keyword to our `seedDatabase` function. This tells the function to return a promise.
   ...
   * 🔑 Since we are not running this function everytime our Express server starts we need to sync the database before we start to seed the database. We don't need to store it in a variable because we have no reason to return it.
   ...
   * 🔑 Take the `Book` and use the `bulkCreate()` method passing in the `bookSeedData` we imported. Add the `await` keyword in front of the statement inside the `async` function body.
  ...
  * Once the promises have been fufilled and no errors have been thrown we want to end this procees with `exit(0)`.
  ...
  * 🔑 In the command line travel to this directory `10-Stu_Async-Await/Solved/seeds/bookSeeds.js` and run `npm start` to have this function trigger. This way the database does not get seeded everytime th Express server starts.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MDN documentation on Async functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function), and stick around for Office Hours to ask for help.
