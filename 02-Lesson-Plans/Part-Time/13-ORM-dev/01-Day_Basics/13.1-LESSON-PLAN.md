...
* Run `npm start` from the command line and demonstrate the following: 
  ...
  * 🔑 Rename the `.env.EXAMPLE` file and fill in your credentials. Run `npm start` from the `05-Ins_Update-Delete` directory. Open Insomnia Core and make a `PUT` request to `localhost:3001/api/isbn` with following data in JSON format.
  ```json
  {
  ...
  * 🔑 Open Insomnia Core and make a `DELETE` request to `localhost:3001/api/isbn` with following data in JSON format.
  ```json
  {
      "isbn": "9780137043293"
    },
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋  `UPDATE book SET title = vPragmatic Prgrammer WHERE isbn`
...
* Direct students to the activity instructions found in `8-Stu_Update-Delete/README.md`.
...
Use Google or another search engine to research the above.
```
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Helps us specify where in our database we want to look.
...
router.get('/', async (req, res) => {
  ...
  * 🔑 Inside of our function's body we need to store our `Book` model's `findAll()` method inside of a variable named `bookData`. In front of the `Book` model you place the `await` keyword. The `await` keyword can only be placed inside of an `async` function. It tells JavaScript to wait untill the promise fullfills itself or throws an error. Lastly, we return our response with the `bookData`.
  ...
  * 🔑 `await` does not prevent your code running while it waits for the promise. It does the contrary, it allows other parts of your code to run but once the promise is fulfilled it will return its value.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Answers may vary but look for: Cleaner code, less `then()` chains, easier to read code.

  * ☝️ What happend to our error handling?

  * 🙋 Students probably do not know but it may be a good early heads-up about `try / catch`. 
