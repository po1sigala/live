...
* Do a quick Google search of "data breaches this year" to look up a list of businesses that were affected this current year. You can refer to this list as you are doing the `Stoke Curiosity` section.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 The ideal authentication platform should use both hashing and salting, and we can use a node package called `bcrypt` to accomplish this.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Sometimes, the promise may take some time before it rejects; so in that case, there will be a delay before `await` throws an error. Also we can wrap multiple lines in the `try` block, so that we can catch any and all errors in the `catch` block. 

  * ☝️ How can these status codes be used to enhance the user experience on the front end?

  * 🙋 We can use the status codes to know exactly what went wrong with the server request and how we can correct it.
...
* Navigate to `13-Ins_Validation` from the command line and run `npm start` to demonstrate the following in Insomnia Core: 
  ...
  * 🔑 Let's try to create a new user with the email address in the wrong email format by making a POST request to `localhost:3001/api/users/`:
...
* This activity uses unit tests to guide the students in creating the right validators and constraints. If needed, explain what the `npm run test` script does and demonstrate how to run `npm run test` to run the unit tests.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How can we use JavaScript to test if a string is alphanumeric?

  * 🙋 We can break it down by saying we will only allow letters between a to z as well as numbers 0 to 9. That kind of thinking is what RegEx, or regular expressions, uses when finding patterns in text.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ Can we use hooks to automate `bcrypt` password hashing for users?
