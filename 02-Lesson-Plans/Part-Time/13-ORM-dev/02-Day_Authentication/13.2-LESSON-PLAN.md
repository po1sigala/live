...
* Run `npm start` from the command line and demonstrate the following in Insomnia Core: 
  ...
  * Okay, so we created our users, but how do we check to see if a user has pets or not without returning ALL of their information? We need to create an instance method.
