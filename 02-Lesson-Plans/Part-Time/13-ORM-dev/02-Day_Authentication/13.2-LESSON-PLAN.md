...
* Open `17-Ins_Hooks/models/User.js` in your browser and demonstrate the following:
...
* Run `npm start` from the command line and demonstrate the following in Insomnia Core: 
  ...
  * 🔑 There is one other small difference, located in `userRoutes.js`. Open `17-Ins_Hooks/routes/api/userRotes.js` and scroll down to the `PUT` route. Notice that we've added an option, `individualHooks: true`, to the query call. This option will select the record that is about to be updated and emit `beforeUpdate` on the instance.
