...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ Is there any way we can make this code easier to read in our route's callback function?
...
* Open `28-Stu_Mini-Project/Solved` in your IDE and explain the following: 
  ...
  * 🔑 In the `models` folder we create a `Trip` model to be our joining, or "through" (in Sequelize terms), table between locations and travellers, so we create foreign key columns for both:
