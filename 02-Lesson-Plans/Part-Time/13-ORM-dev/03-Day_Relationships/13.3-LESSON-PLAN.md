...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Sequelize provides a lot, but sometimes it's easier to just use plain SQL when it comes to a more complicated query. @TODO Expand on this point.
...
* Open `27-Evr_EsLint/Unsolved` in your IDE to demonstrate the following:
  ...
  * In `package.json`, we use `npm run test` to run `npm run lint`, a convention used in the development field so that we have more meaningfully named commands to run. @TODO Clarify meaning here.
...
* Open `02-Homework/Develop` in your IDE and explain the following:
  ...
  * Use the comments in the files to help provide context as to what's needed. @TODO Is this meant as guidance for the instructor or for students?
