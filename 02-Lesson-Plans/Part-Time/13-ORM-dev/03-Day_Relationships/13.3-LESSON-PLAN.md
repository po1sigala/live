...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ Why would we want to represent these real-world relationships in our applications?

  * 🙋 These types of relationships are what make up our everyday lives, both in person and on the web. Think about all of the social networks we belong to and online stores that know our personal preferences, these are all digital representations of characteristics and relationships that exist in real life.
...
* Run `npm start` from the command line and walk through the following in the command line console: 
  ...
  * 🔑 By default, Sequelize logs what commands it performs on the SQL server, even on load:
...
* Open `21-Ins_One-to-One/models` in your IDE and explain the following:
  ...
  * 🔑 In `Driver.js`, there's nothing that indicates ownership of a License.

  * 🔑 In `License.js`, however, there is a column created that specifically references the `driver` model:
  ...
  * 🔑 In `index.js`, however, we instruct Sequelize to create a relationship between the models and details of that relationship:
...
* Use the prompts and talking points below to review the following key (🔑) points:
  ...
  * ✔️ Create a foreign key column in the `models/LibraryCard.js`.

  * ✔️ Association methods used in `models/index.js`

  * ✔️ The use of `include` in the routes to perform a SQL JOIN.

* Open `22-Stu_One-to-One/Solved` in your IDE and explain the following: 

  * 🔑 In `models/LibraryCard.js`, we create a foreign key column to reference the library card's `Reader`
  ...
  * 🔑 In `models/index.js`, we import both models, associate them to one another, and export them as an object:
  ...
  * 🔑 Also in our API route files, our `GET` routes use the `include` option to perform a SQL JOIN:
...
* If time allows, demonstrate what this looks like by seeding the database, running the app, and using Insomnia to demonstrate:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Sequelize docs on one-to-one relationships](https://sequelize.org/master/manual/assocs.html#one-to-one-relationships), and stick around for Office Hours to ask for help.
...
* Run `npm start` from the command line and use Insomnia to demonstrate the following: 
  ...
  * 🔑 When we perform a GET request to `/api/drivers/1`, we get a response that includes not only the driver's licence data, but also an array of their owned cars:
...
* Open `23-Ins_One-to-Many` in your IDE and demonstrate the following:
  ...
  * 🔑 In `models/Car.js`, we create a model with a column to reference the `id` of the driver that owns it.

  * 🔑 In `models/index.js`, not only to drivers have one license, but they also now have many cars and cars belong to one driver:
  ...
  * 🔑 In `routes/api/driverRoutes.js`, our GET route queries use the `include` option to perform a JOIN to retrieve associated driver's license and cars:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ Are we able to create a new driver's license or car without a driver? 
...
* Use the prompts and talking points below to review the following key (🔑) points:
  ...
  * ✔️ We create a foreign key column in the Book model

  * ✔️ We use `.hasMany()` between Reader and Book

  * ✔️ We use `include` to perform a JOIN

* Open `24-Stu_One-to-Many/Solved` in your IDE and explain the following: 

  * 🔑 In `models/Book.js`, we add a column to store a reference to the reader that owns the book: 
  ...
  * 🔑 In `models/index.js`, we create the relationship between readers and books by declaring that readers have many books and a book belongs to one reader:
  ...
  * 🔑 In `routes/api/readerRoutes.js`, we use the `include` option to perform a JOIN: 
...
* Run `npm start` from the command line and use Insomnia to demonstrate the following: 
  ...
  * 🔑 When we perform a GET request to `/api/drivers/2`, we get a response that includes everything from before, but also a total sum of the mileage of all cars owned by the driver:
...
* Open `25-Ins_Literals` in your IDE and demonstrate the following:
  ...
  * 🔑 In `routes/api/driverRoutes.js`, we update our query to include a new attribute called `totalMileage` that has a value of the output of a SQL function:
  ...
  * 🔑 Since we need to perform a SQL query on our database connection, we import our Sequelize instance at the top of the file:
...
* Open `26-Stu_Literals/Solved/routes/api/readerRoutes.js` in your IDE and explain the following: 
  ...
  * 🔑 For our GET routes, we add the `attributes` option to instruct Sequelize to include or exclude any columns or fields:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ Why does our SQL query have to explicitly mention where `book.reader_id = reader.id`?
...
* Navigate to `27-Evr_Eslint/Unsolved` on the command line and run `npm install` so we can demonstrate the following:
  ...
  * 🔑 If we run `npm run test`, ESLint will check all of our JavaScript files for code styling issues by comparing it to both its internal base rules and what rules are present in `.eslintrc.json`
...
* Run `npm start` from the command line and use Insomnia to demonstrate the following: 
  ...
  * 🔑 When we perform a GET request to `/api/travellers/:id` (pick an `id`), we get back data for a traveller along with the locations they plan on visiting and more information for that trip:
  ...
  * 🔑 Explain this data is spread across three tables, where travellers can visit many locations and locations can have many travellers through trips. A trip can only be created if there is a traveller and a location that already exists.
...
* Open `28-Stu_Mini-Project/Solved` in your IDE and explain the following: 
  ...
  * 🔑 In the `models` folder we create a `Trip` model to be our connecting, or "through", table between locations and travellers, so we create foreign key columns for both:
  ...
  * 🔑 In `models/index.js`, we associate locations and travellers to one another through trips:
  ...
  * 🔑 In our GET routes, we use `include` with the `through` option to perform a many-to-many JOIN:
  ...
  * 🔑 In `config/connection.js`, we use `process.env.JAWSDB_URL` as a placeholder value that will be used in production by Heroku:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ Can we use our local machine's MySQL database instead of the one through Heroku?

  * 🙋 Yes, but that means we'd have to leave our machines on all the time, so we shouldn't! 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We will use Sequelize to build one-to-many and many-to-many relationships and perform CRUD actions on our database using RESTful API endpoints.

  * ☝️ How does this project relate to your career goals?

  * 🙋 It will deepen our knowledge in back end technologies using a common situation in development where we need to establish relationships between data.
