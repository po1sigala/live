...
* Open `21-Ins_One-to-One/models/License.js` in your IDE and explain the following:
  ...
  * 🔑 In the `License` model, however, there is a column created that specifically references the `driver` model:
...
* Open `21-Ins_One-to-One/models/index.js` in your IDE and explain the following:
  ...
  * 🔑 We import both models and instruct Sequelize to create a relationship between them:
...
* Open `22-Stu_One-to-One/Solved/models/LibraryCard.js` in your IDE and explain the following: 
  ...
  * 🔑 We create a foreign key column to reference the library card's `Reader`:
...
* Open `22-Stu_One-to-One/Solved/models/index.js` in your IDE and explain the following: 
  ...
  * 🔑 We import both models, associate them to one another, and export them as an object:
...
* Open `24-Stu_One-to-Many/Solved/models/Book.js` in your IDE and explain the following: 
  ...
  * 🔑 We add a column to store a reference to the reader that owns the book: 
...
* Open `24-Stu_One-to-Many/Solved/models/index.js` in your IDE and explain the following: 
  ...
  * 🔑 We create the relationship between readers and books by declaring that readers have many books and a book belongs to one reader:
...
* Open `24-Stu_One-to-Many/Solved/routes/api/readerRoutes.js` in your IDE and explain the following: 
  ...
  * 🔑 We use the `include` option to perform a JOIN: 
...
* Open `25-Ins_Literals/routes/api/driverRoutes.js` in your IDE and demonstrate the following:
  ...
  * 🔑 We update our query to include a new attribute called `totalMileage` that has a value of the output of a SQL function:
...
* Open `27-Evr_EsLint/Unsolved` in your IDE to demonstrate the following:
  ...
  * The `npm run lint` script uses `npx` to temporarily download ESLint as an executable command and run it on all files that match the extension of `.js`. We use `exit 0` at the end to make it so Node doesn't throw an error when the linting doesn't pass:
...
* Open `28-Stu_Mini-Project/Solved/models/Trip.js` in your IDE and explain the following: 
  ...
  * 🔑 We create a `Trip` model to be our joining, or "through" (in Sequelize terms), table between locations and travellers, so we create foreign key columns for both:
...
* Open `28-Stu_Mini-Project/Solved/models/index.js` in your IDE and explain the following: 
  ...
  * 🔑 We associate locations and travellers to one another through trips:
...
* Open `28-Stu_Mini-Project/Solved/config/connection.js` in your IDE and explain the following: 
  ...
  * 🔑 We use `process.env.JAWSDB_URL` as a placeholder value that will be used in production by Heroku:
