...
In this class, students will be re-introduced to JavaScript. They will break down how JavaScript executes an application including so they can understand how the various components work together to run a program. Students will also use the browser to understand the role of Web APIs and asynchronous behavior.
...
* Break students into pairs that will work together on this activity.
  ...
  * Refer to the documentation: [MDN Web Docs on the event loop](https://developer.mozilla.org/en-US/docs/Web/JavaScript/EventLoop)

  * Due to the visual nature of this exercise, it may be helpful to sketch the execution context and allocation of variables.
...
* Break your students into pairs that will work together on this activity.
  ...
  * When we run the unit tests, the defined tests do not pass.
  ...
  2. In the command line, run `npm test` to run the unit test.
...
* Open `05-Ins_Stacks/__tests__/index.test.js` in your IDE to demonstrate the following:
  ...
  * The third test description tells us that we can add an item to the back of the array.

  * The fourth test description tells us that we can remove an item from the back of the array.
...
* Open `07-Ins_Queues/__tests__/index.test.js` in your IDE to demonstrate the following:
  ...
  * The fourth test description tells us that we can remove an item from the **front** of the array.

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How is this data structure different from a stack?

  * 🙋 Stacks only add and remove elements from one side of the structure, whereas queues add from one side and remove from the other.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What is causing the delay for the statement "Are you listening?" because the timer is set to zero?
...
* Open `10-Stu_Callback-Queues/Solved/assets/js/script.js` in your IDE and explain the following: 
  ...
  * Best practice is to send processes that use a lot of resources to the server to free up the call stack on the client to handle the UI.
