...
* The `Everyone Do: Interview Prep - Algorithms` is an OPTIONAL activity meant to introduce students to the tools needed to study algorithms on their own and the amount of time spent on the activity is flexible. If time remains at the end of class, please complete this activity during that time. If you do not have time during class, feel free to use this activity during Office Hours as a supplemental activity.  
...
* Answer any questions before moving on to the next **optional** activity **if time allows. Otherwise, end class.

### 18. Everyone Do: Algorithm Practive (OPTIONAL)

* > **Important:** This activity is optional and aimed at helping students gain confidence in pseudocoding and solving interview algorithms on their own. 

* Open [Leetcode](https://leetcode.com/) in your browser and explain the following:

  * Leetcode is one of the more popular platforms that allow developers to practice algorithms similar to those they may face in an interview setting. 
  ...
  * After logging in to Leetcode, we click on `Problems` to see a range of algorithms for you to solve. Each algorithm is ranked from `Easy` to `Hard`. When starting out, it is a good idea to start with the `Easy` algorithms to practice the steps needed to solve algorithms and gain confidence. We will be working together on the Square Root Algorithm. 
...
* Open [Leetcode Square Root Algorithm](https://leetcode.com/problems/sqrtx/) in your browser and explain the following:
  ...
  * The output is the square root of `x`. However, the result must be truncated, meaning if the result is 2.8, the algorithm should be returned as a whole integer, or 2.

  * Once we understand the required input and output, we are ready to pseudocode our algorithm. In an interview, it is important to talk through your solution as you code and not just work in silence! Pseudocode is a great way to help organize your thoughts and provide quick talking points as you work through the solution.
  ...
  * Next let's expand the pseudocode by thinking about how we might find the square root of the integer. There are multiple ways to mathematically find the square root of an integer, but in this case we are going to look for perfect squares to figure out if an integer is a perfect  square and if not, we can find nearest smaller perfect square to return the right value.
  ...
  * Now that we have thought through the process with psuedocode, we can start to think of the JavaScriptneeded to make our algorithm work. The first line is simple, we can return x if the value of x is 0 or 1.

    ```js
    if(x === 1 || x=== 0) return x;
    ```

  * Next, lets add some variables we can use to find our solution. We use `squared` to hold our perfect square and `i` as a counter.
  ...
  * Since we want to test for perfect squares, we use a while loop that will find the perfect square of an increasing number, in our case `i`.  If we find a match, we can return `i`, since that will be the square root. If not, we keep going until the perfect square is larger than the value of `x`. When that happens, we know, the value we want to return is `i - 1`. 
  ...
  * In an interview setting, talking through your process as you code is as important as getting the right answer, so don't forget to talk through your process, even when practicing at home. It can also help to explain your solution to others.

* If time remains, have students work in pairs to solve a Leetcode algorithm of their choice or have them practice explaining the solution to each other. 
