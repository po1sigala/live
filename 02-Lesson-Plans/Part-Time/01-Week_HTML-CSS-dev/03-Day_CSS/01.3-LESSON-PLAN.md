...
* Navigate to `22-Stu_Mini-Project/Main/index.html` in your browser and demonstrate the following:
  ...
  * Navigate to the `Styles` tab on the `Elements` panel and note the CSS listed on the page.
  
  * Hover over a CSS property and click to uncheck the box. Note the change on the webpage.
  
  * Change the `background-color` property in the body to `purple` to demonstrate how you can manually change a value.
  
  * Explain that changing CSS right in the browaer is a great way to experiment with style properties.
  ...
  * Click on the pointer icon and then hover over an element. A box should appear that indicates the elements accessibility including the role as read by an accessiblity device and contrast.
  
  * Note that having enough contrast between colors is important to make sure your page is viewable to a range of users. 
  
  * Encourage the students to experiment with Chrome DevTools when they build their own projects later in the class.
