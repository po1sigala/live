...
* Open [GitHub](https://github.com/) in your browser and demonstrate the following:
  ...
  * We click in the `new` button to create a new repo.

  * We give the repo the name `git-guide-sample` by adding it in the textbox under `Repository Name`.

  * 🔑 We click on the `Add a README file` checkbox.

  * We click `Create Repository` to create a repository. For now, it will just contain a README file.

  * On the repository page, we click on the green `Code` button and select the `HTTPS` option. We copy the value provided.

* Open your command line and demonstrate the following:

  * 🔑 In the terminal, we navigate to the Desktop folder. Then, we use `git clone` plus the value copied to downlodd the repo on to our local machine.
  ...
  * We use `touch` to add a new file to our repo

  ```bash
  touch index.html
  ```

  * We use `mkdir` to add a new directory to our repo.

  ```bash
  mkdir assets
  ```

  * To add a file to our new directory, we use `cd` to navigate to our new directory and `touch` to add a `style.css` file.
  ...
  * 🔑 To commit our changes we use `git commit` and add a message describing the files changed.
...
* Open your command line and demonstrate the following:
  ...
  * Using `git clone` is a great way to create a repo and download it from local use but you can also use `git init` to create a version control project locally. 
