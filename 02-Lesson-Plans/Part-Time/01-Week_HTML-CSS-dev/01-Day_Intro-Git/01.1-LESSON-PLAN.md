# 01.1 Part-Time Lesson Plan: HTML, CSS, and Git
...
* Explain that in addition to navigating between files and directories, we can use terminal commands to create and remove directories and files. 
  ...
  * 🔑 Since we want to create our new folder inside the `Desktop` directory, we navigate down one layer into the `Desktop` directory using `cd`.
  ...
  * 🔑 We use `touch` to create a new file called `index.html` inside of `demo-folder`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the MDN Web Docs on [Basic Built-in Terminal Commands](https://developer.mozilla.org/en-US/docs/Learn/Tools_and_testing/Understanding_client-side_tools/Command_line#Basic_built-in_terminal_commands) and [Bash/Terminal Commands](bash-terminal-commands-cheatsheet.md), and stick around for office hours to ask for help.
...
* Open up a terminal on your machine. 
  ...
  * 🔑 We use the terminal command `cd` to navigate to the directory where we want the repository located. In our case, we'll use the `Desktop` again.
  ...
  * 🔑 Let's add an HTML file to our local repo using `touch` command.
  ...
  * 🔑 To commit our changes, we use the command `git commit -m` with a commit message, "First commit". 

    ```bash
    git commit -m "First commit"
    ```

  * 🔑 To push our changes back up to the repo we created on GitHub, we use the command `git push`. The `git push` command sends any local commits to a remote location. In our case, the location is `origin` (GitHub), and we want to update the origin's `main` branch. For now, you only have one version of your codebase, and it's called `main` by default.

    ```bash
    git push origin main
    ```

  * Historically, the most common name for the main body of a codebase has been `master`. Recently, however, `main` has been gaining in popularity. In fact, GitHub now uses `main` as the default name for its repositories&mdash;as do the projects in this course. Be aware that you might see instances of both throughout your development career, or hear experienced coders use the term "master branch" out of habit.

  * 🔑 It is important that we keep our local repo up-to-date, especially when collaborating with others. Other collaborators may make changes to the repo that you don't have on your local repo yet.
  
  * 🔑 To pull down any changes from the repo on GitHub and update the repo on our local machine, we use the command `git pull`. 
