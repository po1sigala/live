...
* Open up a terminal on your machine. 
  ...
  * 🔑 We use the terminal command `cd` to navigate to the directory where we want the repository located. In our case, we'll use the `Desktop` again. `@TODO DO NOT USE DESKTOP`
  ...
  * You will be using this `git pull` command before every class to download that day's activities from the class repository.
