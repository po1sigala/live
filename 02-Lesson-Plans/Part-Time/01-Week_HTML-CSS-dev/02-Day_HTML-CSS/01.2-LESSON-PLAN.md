...
* Open `07-Ins_Attributes/index.html` in your IDE and demonstrate the following:
  ...
  * We also use an `href` attribute with `<link>` element. This `<link>` element connects our HTML page to an external CSS stylesheet. The `href` specifies the relative path. This is how we link our stylesheet to our HTML. 
