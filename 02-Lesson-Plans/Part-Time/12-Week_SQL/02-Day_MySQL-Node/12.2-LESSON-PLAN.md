...
* Open the `test_db-master/employees.sql` directory in your IDE to demonstrate the following:
...
* Navigate to `test_db-master` directory in your command line  to demonstrate the following:
  ...
  * To check to make sure our employees database is now in use, we enter `SELECT DATABASES()`.
...
* As developer's, it is important that we build our databases to easily scale to work quickly and easily whether the tables contain ten rows or 300,000. In today's class, we will focus on the tools needed to build databases, provide our own practice data for development and connect our databases to a MySQL server. 

### 2. Instructor Demo: Connect to Node (5 min) 

* Navigate to `11-Ins_Connect-Node` in your command line and enter `mysql -uroot -p` to enter the MySQL Shell.

* Run `SOURCE db/schema.sql;` and `SOURCE db/seeds.sql;` to create and seed the database. 

* Run `npm install` to demonstrate the following:
...
* Navigate to `12-Stu_Connect-Node/Solved` in your command line.
...
* Open `14-Stu_Data-Types/Solved/schema.sql` in your IDE and explain the following: 
...
* Navigate to `15-Ins_Schema` in your command line and enter `mysql -uroot -p` to enter the MySQL Shell to demonstrate the following:
...
* Navigate to `16-Stu_Schema/Solved` in your command line and enter `mysql -uroot -p` to enter the MySQL Shell to demonstrate the following:
...
* Navigate to `17-Ins_Seeds` in your command line and enter `mysql -uroot -p` to enter the MySQL Shell to demonstrate the following:

  * After we initialize the MySQL Shell, we set up the database by typing `source db/schema.sql`.
...
* Navigate to `16-Stu_Seeds/Solved` in your command line and enter `mysql -uroot -p` to enter the MySQL Shell to demonstrate the following:
...
* Open `19-Ins_Foreign-Primary-Key/db/schema.sql` in your browser and demonstrate the following:
...
* Navigate to `20-Stu_Foreign-Primary-Key/Solved` in your command line and enter `mysql -uroot -p` to enter the MySQL Shell to demonstrate the following:
