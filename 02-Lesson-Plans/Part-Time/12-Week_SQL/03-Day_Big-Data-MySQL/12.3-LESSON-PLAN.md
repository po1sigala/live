...
* Open `21-Ins_Prepared-Statements/server.js` in your IDE and explain the following: 
  ...
  * 🔑 Hardcoding works if we only want to delete a particular row. To make the query more durable, we can use a prepared statement. 

  * 🔑 The prepared statement is a parameterized query uses a `?` as a placeholder for the id's value: 

    ```sql
    DELETE FROM books WHERE id = ?
    ```

  * 🔑 For the `DELETE` to work, we then provide the value we want inserted in place of the `?` placeholder as the second parameter of the `query()` function. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 A prepared statement is a way to make our queries more durable. In place of hardcoding a value, we use a `?` placeholder. Then, we provide the `?` a value as a parameter to the `query()` function. By doing this, we can use a single query to handle multiple points of data just by reassigning the `?`. 
...
* Navigate to `22-Stu_Prepared-Statements/Solved/db` in your command line and run `SOURCE schema.sql;` and `SOURCE seeds.sql;` in the MySQL Shell.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MySQL documentation on prepared statements](https://dev.mysql.com/doc/refman/8.0/en/sql-prepared-statements.html), and stick around for office hours to ask for help.

* Answer any questions before proceeding to the next activity.

### 5. Instructor Demo: Aggregate Functions (5 min) 

* Navigate to `23-Ins_COUNT-GROUPBY/db` in your command line and enter `mysql -uroot -p` to start the MySQL Shell to demonstrate the following: 
...
* Open `23-Ins_COUNT-GROUPBY/db/queries.sql` in your IDE and demonstrate the following:
  ...
  * We can count the numbers of data in a group of data by adding the `GROUP BY` clause. This will provide the total count for each department.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Aggregate functions can provide summary data for the entire table or a group of data from the table. 
...
* Open `24-Stu_COUNT-GROUPBY/Solved/server.js` in your IDE and demonstrate the following:
  ...
  * 🔑 To count all the rows of the table, we use the aggregate function `COUNT()` to count each `id` value. We use `AS` to provide an alias column name for our counts. 

    ```sql
    SELECT COUNT(id) AS total_count 
    ```

  * 🔑 Since we want to count the number of books in-stock and out of stock, we add a `GROUP BY`. This will give us the total count of books with an in-stock value of `true` and the total count of books with the in-stock value of `false`. 

    ```sql
    GROUP BY in_stock
    ```

  * 🔑 To get a sum of all of the quantities of books in each department, use the `SUM()` aggregate function to tally the quantity column. 

    ```sql
    SUM(quantity)
    ```

   * 🔑 To get the maximum quantity of all books in each department, use the `MAX()` aggregate function.

    ```sql
    MAX(quantity)
    ```

  * 🔑 To get the minimum quantity of all books in each department, use the `MIN()` aggregate function.

    ```sql
    MAX(quantity)
    ```

   * 🔑 To get the average quantity of all books in each department, use the `AVG()` aggregate function.
...
* Navigate to `24-Stu_COUNT-GROUPBY/Solved/db` in your command line and enter `mysql -uroot -p` to start the MySQL Shell.
  ...
  * We can then exit the shell and navigate to `24-Stu_COUNT-GROUPBY/Solved/`. Enter `npm install` and `node server` to spin up our Express server. The results of our queries print to the command line showing the aggregate values.

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How can we use aggregate functions to get summary data for our tables? 

  * 🙋 We use the aggregate functions to operate on sets of values. Some of the aggregate functions we use are COUNT(), SUM(), MIN(), MAX() and AVG().

  * ☝️ What can we do if we don't completely understand this?

  * 🙋 We can refer to supplemental material, read the [MySQL documentation on aggregate functions](https://dev.mysql.com/doc/refman/8.0/en/aggregate-functions.html), and stick around for office hours to ask for help.
...
* Navigate to `25-Ins_Join/db` in the command line and demonstrate the following:
  ...
  * 🔑 We set up the database and tables by typing `source schema.sql` in the MySQL Shell.

    ```sql
    SOURCE schema.sql;
    ```

  * 🔑 We seed the database by typing `source seeds.sql`.

    ```sql
    SOURCE seeds.sql;
    ```

  * 🔑 We join the `department` and `course names` data together by executing a query in `query.sql`.
...
* Open `26-Stu_Joins/Solved/db/query.sql` in your IDE and explain the following: 
  ...
  * 🔑 We then join the book_price table using the `JOIN` clause:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to the lesson plan, read the [MySQL documentation on the JOIN clause](https://dev.mysql.com/doc/refman/5.7/en/join.html), and stay for Office Hours to ask for hel
...
* Open your command line and demonstrate the following:
  ...
  * 🔑 Now, we can make a second branch. This is the branch where we are going to cherry-pick from. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 First, we need to create a database. Then, we will use `mysql2` to connect the database to an Express server. Finally, we will build routes in Express to query our data. 
...
* Open `28-Stu_Mini-Project/main/db/seeds.sql` in your IDE and explain the following: 
  ...
  * 🔑 To create our routes, we start by writing a query to access our data. It is a good idea to check our query works using the MySQL Shell.
...
* Open `28-Stu_Mini-Project/main/server.js` in your IDE and explain the following: 
  ...
  * 🔑 Then, we create a connection to our database using `createConnection()`.
 ...
 * 🔑 Now, that we have a connection, we can query the database directly from the Express environment. We use the `query()` method to execute our query on our database. 
  ...
  * 🔑 Finally, we wrap our query in an Express route. The paths define the endpoints at which requests can be made. When a route is requested, the query will be executed. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How can we use a route to execute a query in Express? 

  * 🙋 First, we connect our database using `mysql2`. Then, we use the `query()` method to execute the query and wrap the method in a valid Express route. 

  * ☝️ What can we do if we don't completely understand this?

  * 🙋 We can refer to supplemental material, read the [Express documentation on routing](https://expressjs.com/en/guide/routing.html), and stick around for office hours to ask for help.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We are learning to build a database, perform queries and join tables. 

  * ☝️ How does this project build off or extend previously learned material?

  * 🙋 To successfully complete this project, we will have to use the skills learned this week including creating a schema, seed a database, writing queries, and performing JOINS. 
