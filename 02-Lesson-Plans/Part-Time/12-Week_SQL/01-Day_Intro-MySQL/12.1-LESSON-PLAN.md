...
* Make sure that students can initialize the MySQL command-line prompt using the command `mysql -u root -p`. They will need their MySQL password. Students should also be encouraged to use a MySQL password that is not used for any other personal accounts.
...
* Open `03-Ins_Databases` in your command line and demonstrate the following:
  ...
  * 🔑 To create a new database, first we navigate to the command line and enter `mysql -uroot -p` to open MySQL Shell:
...
* Navigate to `05-Ins_Tables` in your command line and enter `mysql -uroot -p` to open MySQL Shell.
...
* Navigate to `06-Stu_Tables/Solved` in your command line and enter `mysql -uroot -p` to open MySQL Shell.
...
* Navigate to `07-Ins_CRUD-Insert` in your command line and enter `mysql -uroot -p` to open MySQL Shell.
...
* Navigate to `09-Ins_CRUD-Delete` in your command line and enter `mysql -uroot -p` to open MySQL Shell.
  ...
  * We execute the `update.sql` file to run the insert query:
