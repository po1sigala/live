...
* Open the [slide deck](https://docs.google.com/presentation/d/1tL0nVHEJVeR5Bi1C1bDBUAOY2ncW9ySReklGIAzaRms/edit?usp=sharing) and follow these prompts on their corresponding slides:
  ...
  * **Server Side APIs**
    ...
    * This unit is all about server-side APIs, which are APIs that help us more on the data-side of our applications than the functional side.

  * **Where do web applications live?**

    * Web applications live on servers.

    * Web servers are typically nothing more than specialized computers running software with the specific task of waiting for an internet request to come in that asks for data in return. 
  ...
  * **How are these requests made?**
    ...
    * Across all of our internet-connected devices, we make HTTP requests to web servers for different types of data constantly:
    
      * Visiting our deployed applications at `<username>.github.io`

      * Our phone or watch automatically updating the weather forecast

      * Using a media streaming service

      * Even the HTML `<link>` and `<script>` tags we use to incorporate Bootstrap, jQuery, or any other third-party API into our applications 

  * **Can we use data from other servers in our applications?**

    * Just as we’ve used third-party APIs to make our application’s functionality and design easier to maintain, we can use specific functionality to request data over HTTP and use that data in our applications. This data usually comes in the form of a special type JavaScript object known as **JSON** (JavaScript Object Notation).

    * Examples of what we can do with this data in our applications:  
    
      * Retrieve weather data to display in our own application
      
      * Use Google Maps to help create a trip itinerary
      
      * Manage our Spotify or YouTube playlists
  ...
  * **How can we learn to use and implement these types of APIs?**
    ...
    * Like other APIs we’ve used in the past, the implementation of server-side APIs depends on what solution that API provides. Some are very simple, some are complex and powerful, so it’s up to us to determine what parts to use, if any.
  ...
  * **Mini Project**
    ...
    * The mini project for this unit will use everything we've learned so far to build an search application that retrieves data from the Library of Congress.
...
* Open `27-Stu_Mini-Project/Main/index.html` in your browser and demonstrate the following:
  ...
  * Upon form submission, we are directed to a second page where our search results appear.

  * We can also conduct more detailed searches as we please by submitting the form again with a format selected.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Most large applications rely on its data coming from a server. It's how the modern web works! 
