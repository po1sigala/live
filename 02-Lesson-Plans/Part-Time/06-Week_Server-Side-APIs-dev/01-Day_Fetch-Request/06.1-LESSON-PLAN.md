...
* Ask the class the following questions (☝️) and call on students for the answers (🙋).
  ...
  * ☝️ What is the **root endpoin**t in the preceding URL?
  ...
  * 🙋 We can use it to quickly confrim that an API URL is correct or working, without the need for JavaScript.
...
* Open `04-Stu_Create-Fetch/Solved/assets/js/script.js` in your IDE and explain the following:
  ...
  * 🔑 We save the full endpoint we would like to make a request to in a variable called `requestUrl`, replacing `{USERNAME}` with the username we want to search, as shown in the following example:
