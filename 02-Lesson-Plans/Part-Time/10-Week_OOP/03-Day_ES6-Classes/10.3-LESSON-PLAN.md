...
* Open `26-Stu-Multiple-Classes/Solved/index.js` in your IDE and explain the following: 
  ...
  * First, we see that we have a `package.json` in this directory, so let's run `npm install` to get Jest installed.
  
  * If we look at `index.js`, we see that the Rare Toy can only be purchased once before it goes out of stock. We can continue to purchase Rare Toys by using our `replenishStock` method to restock.
