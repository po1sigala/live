...
* Run `node app` from the command line and demonstrate the following:
  ...
  * We are going to build a CLI application that will create a summary of all the employees at our company.

  * This application will take in user input and generate an HTML webpage that displays summaries for each person
  
  * This assignment also includes some tests for us to compare our code against. 
