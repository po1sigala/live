...
* Open `23-Ins_Subclasses/rectangle.js` in your IDE and explain the following:
  ...
  * 🔑 In the `constructor()`, we calculate the area and perimeter and pass them into the `super()` function. `super()` will call the constructor on the parent class, `Shape` and make use of `area` and `perimeter`.
