...
* Open `11-Ins-TDD/tests/arithmetic.test.js` in your IDE and demonstrate the following:
  ...
  * Let's take a look at the first test. We start with a `describe` statement that contains several other `describe` statements. The parent `describe` statement groups together the related tests, which also helps with readability.
  ...
  * Here, we see all of the methods that appeared in the tests in `arithmetic.test.js`. 
  ...
  ```
  * Remember, the first test expected `number`to be initialized with a value,and that is what `Arithmetic` appears to accomplish.
...
* Navigate to `11-Ins-TDD/index.js` in your IDE and explain the following:
  ...
  * When we look at the code in `index.js`, we can see the methods declared in `arithmetic.js` chained together, preforming specific operations one after another and finally, returning the value.
  ...
  * We also see that If we run `node index.js` , we see
