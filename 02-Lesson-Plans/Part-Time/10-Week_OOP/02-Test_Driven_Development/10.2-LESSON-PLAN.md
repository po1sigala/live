...
* Welcome students to class.
  ...
  * Test driven development (TDD) is a process and methodology. In order to prime our brain for learning, let's go over some terms that we will see throughout this unit. We will also discuss a very simple flow of how TDD works.
  ...
  * We are going to focus on the basic TDD workflow and expand on each step in this unit, but for now all you need to know are these three steps:
...
* Open `11-Ins_TDD/index.js` in your IDE and explain the following:
  ...
  * We should see the output in the terminal from our testing framework, `jest`. All the passing tests will appear in green and all failing tests will appear in red.
...
* Open `12-Stu_TDD/Solved/test/algo.test.js` in your IDE and explain the following: 
  ...
  * Next, we define `str` as the starting string and `reversed` as `str` backwards, and `result` as the actual result of the `reverse` function.
...
* Open `15-Ins_Organizing-Tests/test/todo.test.js` in your IDE and demonstrate the following:
  ...
  * 🔑 When writing tests, it's a good idea to have a structure that can be referenced for organizing tests and deciding what kinds of things to test for.
...
* Open `15-Ins_Organizing-Tests/test/todoList.test.js` in your IDE and explain the following:
  ...
  * 🔑  When considering what kinds of things to write tests for, it's a good idea to keep in mind positive, negative, and exception tests.
    ...
    * Positive tests check that an application behaves as expected with positive inputs or not.
    
    * Negative tests check that an application behaves as expected while providing invalid or improper data sets as input. This is to ensure the application can handle invalid data input gracefully.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel with organizing your tests? (Poll via Fist to Five, Slack, or Zoom)
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Jest Docs on spyOn()](https://jestjs.io/docs/en/jest-object#jestspyonobject-methodname), and stick around for office hours to ask for help.
...
* Open `19-Ins_Module-Mock-Demo/movieSearch.js` in your IDE and demonstrate the following:
  ...
  * 🔑 This file exports a `MovieSearch` constructor that can be used to create an object used for searching the OMDB (Open Movie Database) API.
...
* Open `19-Ins_Module-Mock-Demo/test/movieSearch.test.js` in your IDE and demonstrate the following:
  ...
  * 🔑 We require axios and mock it, since we want to prevent actual API requests from happening. This is one way of mocking a node module.
