...
* Open the [slide deck](https://docs.google.com/presentation/d/1Heou5UD956vYb1d4YDCi4GobFCcz4geNifZmz0KISgg/edit?usp=sharing) and follow these prompts on their corresponding slides:
  ...
  * **Everything is an object!**
    ...
    * Nearly everything in Javascript is an object other than primitive data types. That means anything except `null`, `undefined`, strings, numbers, and booleans are objects. In Javascript, everything else boils down to an object, which is essentially a list with key:value pairs.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We are utilizing Object Oriented Programming, ES6 classes and constructor functions to build a command line app.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Constructor functions act as a kind of blueprint for creating many objects of the same type. They indicate the necessary criteria for creating an object and allow instances of that object to be "instantiated" using the `new` keyword.
...
* Open `02-Stu_Constructors/Solved/index.js` in your IDE and explain the following: 
  ...
  * We create a new object to test out the `Developer` constructor. You could have used any name and technology you wanted. For our example, we will use "Rita".

  ```js
  const rita = new Developer('Rita', 'JavaScript');
  ```

  * We call the `introduction()` method on `rita` to print the message in the console.
...
* Open DevTools in Google Chrome and demonstrate the following:
  ...
  * 🔑 Everything in JavaScript is an object! `String`s, `Date`s, `Number`s, `Array`s, and `Object`s ... they are all standard built-in objects in JavaScript. 
  ...
  * The result will be every number in the array logging in the console. We were able to use the `Array.prototype.forEach()` method because `myArray` was derived from the built-in Array object.
  ...
  * Referencing methods via the prototype chain is also an efficient use of memory. Rather than creating multiple copies of the same method in memory, we can create one and point to it.
  ...
  * In DevTools, let's create a constructor function named `Movie`, which will take in two arguments, `title` and `releaseYear`.
  ...
  * One way to check if a property was inherited or not is to use the `hasOwnProperty` method. This will return `true` if it does have the specified property.
  
  *  We can check this by running the following in the developer console
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How can we use the `hasOwnProperty()` method to check if an object's property is its own, or if it was inherited?
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How would we implement method overload in our application?

  * 🙋 We will need to use either a `if` statement or `switch` statement to determine what is returned by the method.
...
* Open `06-Stu-Polymorphism/Solved/index.js` in your IDE and explain the following: 
  ...
  * When we open this file at the top of the file we see the `inRange()` helper function and also a constructor called `Student`.
  ...
  * We also see a method on the `Student` constructor called `displayGrade()`. Our task was to build out this method by checking the data type for the value of `grade`. 
  ...
  * 🔑 We respond differently depending on whether or not the `input` is a string or a number. So first, we will check if the `input` is a number. If so, using a series of `if` statements and the helper function `inRange`, we will set the `response` variable to the letter grade and return it.
  ...
  * 🔑 If the `input` is a string, we set up a `switch` statement to handle different cases of letter grades, set the `response` variable to the range of grades, and return it.
...
* Open `07-Ins-Promises/index.js` in your IDE and explain the following:
  ...
  * 🔑 Let's use the example of a real-life promise. Let's say parents promised their child a Nintendo Switch if the child finished all their chores. If the `choresDone` is `true`, the promise will be resolved. But if not, there will be an error and the promise will be rejected.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can use promises to avoid chaining multiple callbacks together. Chaining callbacks often gets confusing to understand and sometimes hard to read.
...
* Open `08-Stu_Promises/Solved/index.js` in your IDE and explain the following: 
  ...
  * 🔑 Following the same logic as the original `practiceCoding()` function, our promise calls the `reject()` method if `studentDistracted` is set to `true`. 
  ...
  * 🔑 Otherwise, we resolve the promise because the student is coding which is our desired outcome.
...
* Navigate to `09-Ins_Promise-All` and run `node index.js` in your command line to demonstrate the following:
  ...
  * 🔑 We see the statuses of three promises. `p1` resolved right away but the other two are pending.
  ...
  * 🔑 Then after a few seconds we see the returned array from the `Promise.all()`.
...
* Open `09-Ins_Promise-All/index.js` in your IDE and explain the following:
  ...
  * 🔑 We see that `p1` is a promise that resolves right away. 
  ...
  * 🔑 But `p2` and `p3` take a few seconds to resolve. Therefore, when we try to log it in the console, we are logging the output of a promise that hasn't been fully resolved or rejected yet.
  ...
  * 🔑 When all of the promises are resolved (or rejected), we get a response from a `Promise.all()` being invoked. The results of `Promise.all()` include the results of all the promises that we pass in as an array.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How can we utilize `Promise.all()`?
...
* Open `10-Stu_Promise-All/Solved/index.js` in your IDE and explain the following: 
  ...
  * 🔑 The key to any promise is implementing `resolve()` and `reject()`. First we set up a conditional statement to check whether the `duration` is greater than `maxDuration`. If so, we reject the promise.
  ...
  * 🔑 Finally, we call `Promise.all()` with the `promises` array. If all of the promises were resolved, we console log the response by chaining a `.then()`. If anything went wrong, we catch the errors in the `.catch()`.
