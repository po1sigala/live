...
* Open `02-Stu_Constructors/Solved/index.js` in your IDE and explain the following: 
  ...
  * We create a new object to test out the `Developer` constructor. You have used any name and technology you wanted. For our example, we will use "Rita".
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How can we used the `hasOwnProperty()` method to check if an object's property is it's own, or if it's been inherited?

  * 🙋 We can use `hasOwnProperty()` to check this. The `hasOwnProperty` method will return false if it is not a direct method.
...
* Open `09-Ins_Promise-All/index.js` in your IDE and explain the following:
  ...
  * 🔑 But `p2` and `p3` takes a few seconds to resolve. Therefore, when we try to log it in the console, we are logging the output of a promise that hasn't been fully resolved or rejected yet.
