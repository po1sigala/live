...
* Open `07-Ins_Dom-Traversal/index.html` in the browser and open the Elements tab in DevTools to demonstrate the following:
  ...
  * 🔑 In the Elements tab, expand all the elements on the page, especially the `<ul>`, to reveal that the visible elements on the page are children or grandchildren to the `<main id="top">` element.
  
  * Examine the `<script>` elements at the bottom of the `<body>` for jQuery and the `script.js` file.

* Open `07-Ins_Dom-Traversal/assets/js/script.js` in your IDE and notice that statements are commented. Uncomment each line to display how the webpage currently renders. Also keep the console in DevTools open to review any logs. We will continually refresh this webpage to view DOM changes.

* Open `07-Ins_Dom-Traversal/index.html` in your IDE and show the following:
  
  * Note the position of the `<main id="top">` in the markup as the parent element:

      ```html
      <main id='top'>
      ```

 * Note the `<script type="text/javascript" src="./assets/js/script.js"></script>`
