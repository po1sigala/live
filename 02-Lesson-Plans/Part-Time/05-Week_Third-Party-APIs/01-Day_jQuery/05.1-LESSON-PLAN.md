...
* Open `07-Ins_Dom-Traversal/index.html` in the browser and open the Elements tab in DevTools to demonstrate the following:
  ...
  * 🔑 Expand all the elements on the page in the Elements tab, especially the `<ul>`, to reveal that the visible elements on the page are children or grandchildren to the `<main id="top">` element.
  
  * Examine the `<script>` elements at the bottom of the `<body>` for jQuery and the `script.js` file in the Elements tab in DevTools. Note the element hierarchy.

* Open `07-Ins_Dom-Traversal/assets/js/script.js` in your IDE and notice that JavaScript statements are commented. Only uncomment the requests lines to reveal each effect on the webpage one by one.
...
* Refresh `07-Ins_Dom-Traversal/index.html` in your browser to display how the webpage has been modified.
  ...
  * Open the Elements tab in DevTools and select the `<h1>` to view the `.boxy` class selector and its corresponding CSS rules or view `style.css` in the IDE.

  * Notice that although the text color was changed to red in the class rule, it was overwritten by the `css()` method due to its cascading priority. The `css(color, red)` method can be chained to this statement to override the previous `css(color, yellow)`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What is the name of the method to traverse down one level to the select all descendant elements?
...
* Open `08-Stu_Dom-Traversal/Solved/assets/js/script.js` in your IDE and explain the following:
  ...
  * 🔑 To select the background color for each list item or `<li>`, we must select the children of the `<ul>` or the list.
