...
In this class, we will perform DOM manipulations that students are already familiar with, except this time we'll be using jQuery. 
...
* Open the [slide deck](https://docs.google.com/presentation/d/1-TxVbjIGv8jMD4pcEtjonVPs8qItpUNHUCfIGMAErxg/edit?usp=sharing) and follow these prompts on their corresponding slides:
  ...
  * **Is the DOM built into the JavaScript language?**
    ...
    * Nope. The browswer provides it! On its own, JavaScript is very limited. Luckily, the browser provides numerous application programming interfaces (APIs) that we can use in JavaScript code.
...
* Open `01-Ins_jQuery-Elements/assets/js/script.js` in your IDE and demonstrate the following:
    ...
    * We can select the element by attribute like an `id` and create a jQuery object, as follows:
  
      ```js
      var rootEl = $("#root");
      ``` 

    * We can create a DOM element with angle brackets, as follows:
  
      ```js
      var titleEl = $("<h1>");
      ``` 

    * We can loop through an array and create a DOM element for each array element, add text content, and append the DOM element to the page, as follows:
...
* Open `02-Stu_jQuery-Elements/Solved/assets/js/script.js` in your IDE and explain the following: 
    ...
    * We can also create elements by placing the angle brackets around the element, as follows:

      ```js
      var authorEl = $("<p>"); 
      ```
    
    * 🔑 We can add text content with the `text()` method, as follows:

      ```js
      authorEl.text("~ Carol Dweck");
      ```

    * We can create an `<h1>` element for the quote text, as follows:

      ```js
      var quoteEl = $("<h1>"); 
      ```

    * We can assign a class using the `addClass()` method, as follows:

      ```js
      authorEl.addClass("plain");
      ```
    
    *  We can add text content to the quote element with the `text()` method, as follows:

        ```js
        quoteEl.text("Love Challenges, Be Intrigued by Mistakes, Enjoy Effort, and Keep Learning.");
        ```

    * 🔑 We can also add class or any attribute with the `attr()` method, as follows:

      ```js
      quoteEl.attr("class", "fancy");
      ```

    * 🔑 We can append the quote element to the page with  `append()`, as follows:

      ```js
      rootEl.append(quoteEl);
      ```

    * We attach the author element to the quote, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to supplemental material, read the [jQuery API documentation](https://api.jquery.com/), look at examples, and stick around for office hours to ask for help.
...
* Open `03-Ins_Click-Events/assets/js/script.js` in your IDE and show the following statements:
    ...
    * We create references to the DOM elements we will work with first, as follows:
    ...
    * 🔑 We attach the click event to the alert button, as follows:
...
* Research the [jQuery documentation on events listeners](https://api.jquery.com/category/events/) to find the method that will provide the operation we require.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel adding click events with jQuery? (Poll via Fist to Five, Slack, or Zoom)
...
* Open `04-Stu_Click-Events/Solved/assets/js/script.js` in your IDE and explain the following: 
    ...
    * 🔑 Because the `.on()` method will listen for any valid browser event, we can simply change it to say `click`, as follows:
    
      ```js
      passwordBtnEl.on('click', function () {
      ```

    * 🔑 We change the argument passed into `passwordGenerator()` to a value of `15` instead of `2`, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to supplemental material, read the [jQuery documentation on mouse events](https://api.jquery.com/category/events/mouse-events/), and stick around for office hours to ask for help.
...
* Open `05-Ins_Form-Elements/assets/js/script.js` in your IDE and show the following:
    ...
    * We can use `.on('submit', ...)` as the click event for the `<form>` element and pass `handleFormSubmit()` as the callback function, as follows:
 
      ```js
      formEl.on('submit', handleFormSubmit);
      ```
    
    * We use the jQuery `.val()` method on form elements to retrieve the input values, as follows:

      ```js
      firstNameEl.val() 
      ```

    * Because there can be more than one checked item, we need to select the checked elements and store them in an array using the jQuery `$.each()` method, as follows:
    ...
    * We can clear form elements by using CSS selectors and setting an empty string value to them, but the checkbox requires a different type of reset, as follows:
...
* Open `06-Stu_Form-Elements/Solved/assets/js/script.js` in your IDE and explain the following: 
    ...
    * 🔑 We attach the `submit` event to the form element using `.on()`, as follows:

      ```js
      shoppingFormEl.on('submit', handleFormSubmit);
      ```

    * 🔑 We can get the value entered in the form `<input>` by selecting it by its `name` attribute and using `.val()`, as follows:

      ```js
      var shoppingItem = $('input[name="shopping-input"]').val();
      ```
    
    * 🔑 If we click Submit and nothing is in the `<input>`, do nothing, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the supplemental material, read the [jQuery documentation on forms](https://api.jquery.com/category/forms/), and stick around for office hours to ask for help.
...
* Open `07-Ins_Dom-Traversal/index.html` in your IDE and show the following:
    ...
    *  We notice the position of the empty `<div>` in the markup, as follows:
...
* Open `07-Ins_Dom-Traversal/assets/js/script.js` in your IDE and demonstrate the following:
    ...
    * From the empty `<div>`, we can select elements in the DOM tree, as follows:
    ...
    * We can nest selectors inside of jQuery methods, as follows:
...
* Open `08-Stu_Dom-Traversal/Solved/assets/js/script.js` in your IDE and explain the following: 
    ...
    * 🔑 We use the `find()` method to search all descendant elements with a selector. Then we use the `css()` method to change the styling, as follows:

      ```js
      rootEl.find("li").css("background-color", "white");
      ```
    
    * 🔑 We can chain the methods together to combine searches. We can compartmentalize the search by row to make them more legible, as follows:

      ```js
      var rowThree = rootEl.children().last().children();
      ```
    
    * 🔑 We use `first()`, to select the first element of the set of returned elements. We use the `text("O")` to insert the text O, as follows:

      ```js
      rowThree.first().text("O");
      ```
    
    * 🔑 We use `closest("main")`, to select all the ancestor elements that is a `<main>`, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the supplemental material, read the [jQuery documentation on traversing](https://api.jquery.com/category/traversing), and stick around for office hours to ask for help.
...
* Open `09-Ins_Event-Delegation/index.html` in your browser and demonstrate the following: 
    ...
    * 🔑 When we click the Show Letters button, we can create and render the letter elements.

* Open `09-Ins_Event-Delegation/assets/js/script.js` in your IDE and demonstrate the following:  
    
    * 🔑 We must delegate the event listener to the parent element, `<div id="buttons">`, because the letters are not available when the `document` loads, as follows:
...
* Open `10-Stu_Event-Delegation/Solved/assets/js/script.js`in your IDE and explain the following: 
    ...
    * 🔑 We can attach the `on()` event listener to the parent element, `shoppingListEl`, to listen for the click on the shopping list item buttons, as follows:

      ```js
      shoppingListEl.on('click', '.delete-item-btn', handleRemoveItem);
      ```

    * 🔑 In the event handler, we use `$(event.target)` to select the button clicked by the user, as follows:

      ```js
      var btnClicked = $(event.target);
      ```

    * 🔑 We use traversing with `.parent('li')` to select the first parent element that matches a `<li>` and remove it, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
    ...
    * 🙋 We can refer to the supplemental material, read the [jQuery documentation on event delegation](https://learn.jquery.com/events/event-delegation/), and stick around for office hours to ask for help.
