...
* Open `07-Ins_Dom-Traversal/index.html` in the browser and demonstrate the following:
  ...
  * 🔑 We can travel down through the children elements from a parent element.

* Open `07-Ins_Dom-Traversal/index.html` in your IDE and show the following:
  
  * We notice the position of the `<main id="top">` in the markup:
...
* Open `07-Ins_Dom-Traversal/assets/js/script.js` in your IDE and demonstrate the following:
  ...
  * From the parent element, `<main id='top'>`, we can select the children elements in the DOM tree as shown with a red border:
  
    ```js
    $('#top').children().css('border', 'red solid 2px');
    ```

  * We can chain methods together to select nested elements, then alter the elements properties.
  ...
  * We can use `.css()` to highlight what is being currently selected.
