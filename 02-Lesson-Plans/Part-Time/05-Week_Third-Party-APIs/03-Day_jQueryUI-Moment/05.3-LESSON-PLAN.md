...
* Open the newly created `git_revert_demo` directory in your IDE and copy the `27-Evr_Git-Revert/script.js` file into it.
  ...
  * If we look at the [Git documentation on git revert](https://git-scm.com/docs/git-revert#_options), we see various options that we can also use. One useful option is to pass in the `HEAD` which will revert the latest commit, as follows:
