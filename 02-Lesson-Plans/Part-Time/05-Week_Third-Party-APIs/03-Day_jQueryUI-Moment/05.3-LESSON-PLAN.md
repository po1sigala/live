...
* Open the [Git documentation on git revert](https://git-scm.com/docs/git-revert) in your browser and explain the following:
  ...
  * `git revert` isn't exactly an undo operation, but it can invert the changes introduced by a single commit and then append a new commit with the resulting inverse content. This way, you are not losing any history, which is vital to the integrity of your revision history and reliable collaboration with other developers.

  * An example scenario for this is if you were working with your classmates on a project and discovered a bug that was caused by a single commit. You could use `git revert` to fix the bug and commit a new snapshot without undoing any other commits or compromising the revision history of the codebase. 
...
* If necessary, you can review the activity. Open your command line, create a new directory, `cd` into it, and initialize a Git repo there to practice in:
...
* Let students know that we are taking the `script.js` file from the jQuery UI activity. First, we will make the initial commit:
...
* Then we'll add the autocomplete widget to `script.js`:
  ...
  * Now make a commit for the widget addition:
