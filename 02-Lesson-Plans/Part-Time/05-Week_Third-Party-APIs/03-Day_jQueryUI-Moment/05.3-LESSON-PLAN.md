...
* If necessary, you can review the activity with students. 
  ...
  * If we invoke `git log` to display the commit history, we see all four commits. But let's say the sortable interaction was a mistake and was causing bugs. We can use `git revert` to revert back to before we made that commit.
  ...
  * When we check the `script.js` file, we see that the sortable interaction is indeed gone. And we have a record of all the commits in the project history.
