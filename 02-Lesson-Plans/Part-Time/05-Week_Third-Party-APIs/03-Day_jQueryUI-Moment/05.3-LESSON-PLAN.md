...
* If necessary, you can review the activity with students. 
  ...
  * If we invoke `git log` to display the commit history, we see all four commits. But let's say the sortable interaction was a mistake and is causing bugs. We can use `git revert` to revert back to before we made that commit.
