...
In the class, you will introduce students to Mongoose, a node package that provides a schema-based solution to model your Node application data. Students will build upon their knowledge of MongoDB and see how Mongoose makes it easier to handle the data from the database.
...
* As we continue to work with MongoDB, we'll be adding Mongoose as the focus of today's activities. But reassure students that Mongoose is simply a "wrapper" around MongoDB that makes it easier to manage relationships between data. It's similar to what Sequelize was for MySQL.
...
* The topic of today's lesson is Mongoose, an Object Data Mapper (ODM) for MongoDB. Using the prompts provided, have a discussion about the usefulness of data modeling and ODMs.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 When we used Sequelize, an object-relational mapper (ORM), with our MySQL database. Mongoose is similar to Sequelize in that aspect, but for MongoDB. Mongoose is an Object Data Mapper (ODM) for MongoDB. Think of it like a wrapper around MongoDB that makes it easier to manage relationships between data.

  * ☝️ What are the benefits to using Mongoose with MongoDB?
...
* Open `13-Ins_CRUD-Mongoose/server.js` in your IDE and demonstrate the following:
  ...
  * 🔑 Another helper function we can use for CRUD operations is the `.findOneAndDelete()`. This will find the document and delete it from the database. This time instead of hardcoding the name, we will use the URL params.
