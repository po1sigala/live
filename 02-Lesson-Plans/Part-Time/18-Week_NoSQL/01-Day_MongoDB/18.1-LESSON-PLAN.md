...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 MongoDB is a popular NoSQL database solution for companies large and small. MongoDB is also the "M" in two popular technology stacks -- MERN and MEAN -- that are used by developers to create easily scalable full-stack apps. We will learn more about the MERN stack in the weeks ahead and will use MongoDB to store data for our final projects.
...
* In the open MongoDB shell, do the following:
  ...
  * 🔑 Data in MongoDB databases is stored in one or more collections. Just like when we create a database, we do not have to use a special command to create the collection. Instead, we can simply give the new collection a name and insert data in a single step.
    ...
    * We start our command with `db.` This `db` refers to the current database in use. We then use dot notation to enter the name of the collection we want to use. Because `petCollection` doesn't exist yet, it will be automatically created when our command is executed.
...
* Click Refresh at the top of the Compass screen to demonstrate the following:
  ...
  * When we click Refresh at the top of the Compass screen, the `inventoryDB` database that we just created should appear on the list of available databases.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MongoDB docs on introduction to MongoDB](https://docs.mongodb.com/manual/introduction/), and stay for Office Hours to ask for help.
...
* Open `04-Stu_Create-Read/Solved/server.js` in your IDE and explain the following:
  ...
  * 🔑 To integrate MongoDB into an existing Express server, we must install the `mongodb` package using the command `npm install mongodb` and require `mongodb` and the `MongoClient` class in our `server.js` file.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MongoDB docs on insert methods](https://docs.mongodb.com/manual/reference/insert-methods/) and the [MongoDOB docs on query methods](https://docs.mongodb.com/manual/tutorial/query-documents/), and stay for Office Hours to ask for help.
...
* In the open MongoDB shell, do the following:
  ...
  * 🔑 To delete the document we just updated, we use the `deleteOne()` method.
    ...
    * It is important that you provide filter information so that only the document we want deleted is deleted, and not all the documents in the collection!
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MongoDB docs on delete methods](https://docs.mongodb.com/manual/reference/delete-methods/), and stay for Office Hours to ask for help.
...
* Open `10-Stu_Cursor-Methods/Solved/server.js` in your IDE and explain the following:
  ...
  * 🔑 Next, we add `skip()`. Because we want to skip over the first five results, we set the parameter to 5.
  ...
  * 🔑 Finally, we want to return a maximum of five documents, so we use `limit()`.
  ...
  * Our completed `GET` route will return sorted documents, that skip the first five documents and have a maximum limit of five. While sort, skip, and limit can be used alone, these three methods are often used together to constrain results to tailor the returned results to only those we need.
...
* In your command line, navigate to `10-Stu_Cursor-Methods/Solved` and run `npm install` and `node server` to demonstrate the following in Insomnia:
  ...
  * When we test the `GET` request with the route `localhost:3001/read`, we see that the results are sorted by the `number` field in descending order, five results are skipped, and the results are limited to five.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [MongoDB docs on cursor methods](https://docs.mongodb.com/manual/reference/method/js-cursor/), and stay for Office Hours to ask for help.
