...
* Open the [slide deck](https://docs.google.com/presentation/d/1tsxce_ukQswTyiPlxV9BfzshLkwquUHWdxNrFQYlHvM/edit#slide=id.g3fd172e150_2_21) and follow these prompts on their corresponding slides:
  ...
  * **MongoDB uses documents and objects instead of tables and rows**: MongoDB stores data in objects stored in a format called BSON, or Binary JSON. BSON looks and acts just like JSON, but is optimized for faster parsing.
  ...
  * **SQL vs MongoDB**: So let’s review how data is stored in SQL and MongoDB databases. SQL relationships are relational and SQL enforces a rigid schema and uses rows tables. MongoDB is NoSQL and does not enforce a rigid schema and uses documents and collections. Finally, SQL relies on joins to relate data while MongoDB uses embedded documents.  
...
* Navigate to `28-Stu_Mini-Project/Main` in your command line and run `npm install` demonstrate the following:
  ...
  * In the console, we can see all the data that has been populated and will be added to our database. We can query this data using a `GET` request.

  * Next we run `npm start` to start the app on port 3001.

* Open Insomnia on your local machine to demonstrate the following:

  * We use the `GET` request and the route `localhost:3001/api/students` to return a list of all of the student data as well as each students' associated grades.
  ...
  * In today's class, we will be learning how to write our own routes to create, read, update and delete these document objects in a MongoDB database.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 MongoDB is a popular, NoSQL database solution for companies large and small. MongoDB is also the `M` in two popular technology stacks -- `MERN` and `MEAN` -- that are used by developers to create easily scalable, fullstack apps. We will learn more about the `MERN` stack in the weeks ahead and will also use MongoDB to store data for our final projects. 
...
* Open Insomnia on your local machine to demonstrate the following:
  ...
  * To fetch the new document we just created from our database, we make a `GET` request and add the route `localhost:3001/read` and click `Send`.
...
* Open `04-Stu_Create-Read/Solved/server.js` in your IDE and explain the following: 
  ...
  * 🔑 To integrate MongoDB into an existing Express server, we must install the mongodb package with `npm install mongodb` and require mongodb and MongoClient class in our `server.js` file.
  ...
  * 🔑 Once MongoDB is integrated into the server, we can then use MongoDB methods to perform CRUD operations -- such as create and read -- directly in a route. This allows us to interact with the underlying MongoDB in our apps. 
  ...
  * Inside the request, we add our query. We use `collection()` to create or select an instance of a collection and `insertOne()` to add a single document to the selected collection. We also add handling for both and errors and the results.
  ...
  * To create a `GET` request to fetch data, we use the `find()` method. Since the filter contains no data, all documents will be returned. We also use the `toArray()` method so multiple documents are returned in an array. Since the filter contains no data, all documents will be returned.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How do insert multiple documents using a single MongoDB method?

  * 🙋 We use the `insertMany()` method and pass in an array containing all the documents we want to add. 

  * ☝️ What can we do if we don't completely understand this?

  * 🙋 We can refer to supplemental material, read the [MongoDB docs on insert methods](https://docs.mongodb.com/manual/reference/insert-methods/), [MongoDOB docs on query methods](https://docs.mongodb.com/manual/tutorial/query-documents/) and stick around for office hours to ask for help.
...
* Direct students to the activity instructions found in `06-Stu_Update-Delete/README.md`.
...
* Open `06-Stu_Update-Delete/Solved/server.js` in your IDE and explain the following: 
  ...
  * 🔑 In this route, we will use the `deleteOne()` method to delete a single document by id. Don't forget the numerical id string must be wrapped with `ObjectID()` for now. One of the advantages of Mongoose, which we we learn about later in this unit, is that it makes deleting by id easier and we won't have to manually do this step.

    ```js
    db.collection('bookCollection').deleteOne(...)
    ```

  * 🔑 When using a delete method, it is very important that we specify a filter so that only the documents we want deleted are removed. This filter will match the id we provide in the request body and only delete that document, not all documents! 
...
* Navigate to `06-Stu_Update-Delete/Solved/` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia: 
  ...
  * We click `SEND` and copy the id string for our new document from the returned data  We will use that to delete the document we just created by id. 

  * To delete our document, we use a DELETE request with the route `localhost:3001/delete`. In the request body, we add the string of our new document we copied wrapped in `ObjectID()`. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 When using a delete method, it is important that you only remove the documents you are targeting, not all the documents. Using a filter allows us to target only the documents we want to delete. In addition, by filtering by id, we know we are only targeting one document, since each id is unique. 
...
* Open `07-Ins_Embedded-Documents/models/data.js` in your IDE and demonstrate the following:
  ...
  * We can also model relationships much easier using embedded documents. An embedded document is simply a document inside another relationship. Using embedded documents reduces the number of read operations needed to retrieve data and makes it simple to determine relationships between documents. 

  * To model a one-to-one relationship, we embed one related document as a subdocument. In this instance, the parent document has one related child promotion document.
  ...
  * To model a one-to-many relationship, we add many related embedded document. In this instance, the parent document has multiple related child documents.
...
* Open `08-Ins_Sort-Skip/server.js` in your IDE and explain the following: 
  ...
  * 🔑 To skip a certain number of documents and not include them in the results, we  use `skip()`. The number added as a parameter equals the number of documents we want to skip. Using `skip()` all the documents after the last one skipped, will be returned.  
...
* Open `10-Stu_Sort-Skip/Solved/server.js` in your IDE and explain the following: 
  ...
  * 🔑 To sort the data by the `number` field in descending order, we chain a `sort()` method after the `find()`. Order matters here! We always want to fetch the data using `find()` before applying any cursor methods to constrain the data. 
