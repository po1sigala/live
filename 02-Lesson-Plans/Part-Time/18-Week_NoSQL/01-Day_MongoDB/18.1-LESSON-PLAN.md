...
* Open the MongoDB shell to do the following: 
  ...
  * When a document has been successfully created, an object is returned. If we have not explicitly included a value for the `_id` field, the inserted ID that MongoDB creates one for us is included in this object.
  ...
  * 🔑 We can fetch our new document in various ways. One way is to simply use the `find()` method. This will return all the documents currently in our collection.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We use the MongoDB methods `insertOne()` and `find()` to create a new document and read all all the documents in a collection, respectively.
