...
* Open `03-Ins_Create-Read/` in your IDE to demonstrate the following:
  ...
  * The `POST` request we tested with Insomnia uses `insertOne()` to add a single document with a `name` and a `breed` field to the `petCollection`.
...
* Navigate to `06-Stu_Update-Delete/Solved/` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia: 
  ...
  * To test our route, we first use a GET request with the route `localhost:3001/create` to insert a new document with a title `Hello World` into our database. We add the data object into the request body as JSON. 
  ...
  * We click `SEND` and copy the id string returned for our new document. We will use that to delete the document we just created. 

  * To delete our document, we use a DELETE request with the route `localhost:3001/delete`. In the request body, we add the string wrapped in `ObjectID()`. 
...
* Navigate to `08-Stu_Embedded-Document/Solved` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia:
  ...
  * When we test the `GET` request with the route `localhost:3001/price-less-than-10`, all documents that have a an embedded document with a `price` field that is less than 10 are returned.

  * When we test the `GET` request with the route `localhost:3001/featured-authors`, all documents that have a an embedded document with a `featured` field that is `true` are returned.
...
* Open `08-Ins_Sort-Skip/server.js` in your IDE and explain the following: 
  ...
  * In the `GET` route, we see a number of methods that are working to constrain the data taht is actually returned. 
  ...
  * 🔑 To skip a certain number of documents and not include them in the results, we can use `skip()`. The number added as a parameter is the number of documents skipped. 

    ```js
    .skip(1)
    ```

  * 🔑 Finally, set a maximum number of results to return, we use `limit()` and set a number as the parameter. This will limit the results to a maximum of 10. 
...
* Open `10-Stu_Sort-Skip/Solved/server.js` in your IDE and explain the following: 
  ...
  * 🔑 To sort the data by the `number` field in descending order, we chain a `sort()` method after the `find()`. Order matters here!
  ...
  * 🔑 Next, we add `skip()`. Since, we want to skip over the first five results, we set the parameter to 5.  
  ...
  * Our completed `GET` route will return sorted documents, that skip five and have a maximum limit of 5.  While sort, skip and limit can be used alone, these three methods are often used together to constrain results to just those we want or need. 
...
* Navigate to `10-Stu_Sort-Skip/Solved` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia:
  ...
  * When we test the `GET` request with the route `localhost:3001/read`, we see the results are sorted by the `number` field in descending order, five results are skipped and the results are limited to 5. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ What is the difference between `limit()` and `skip()` cursor methods?

  * 🙋 The `limit()` cursor methods sets a maximum number of documents that will be returned while `skip()` method excludes a certain number of documents and returns only the documents that come after them. Both methods are frequently used to constrain the data returned in a MongoDB query. 

  * ☝️ What can we do if we don't completely understand this?

  * 🙋 We can refer to supplemental material, read the [MongoDB docs on cursor methods](https://docs.mongodb.com/manual/reference/method/js-cursor/), and stick around for office hours to ask for help.
