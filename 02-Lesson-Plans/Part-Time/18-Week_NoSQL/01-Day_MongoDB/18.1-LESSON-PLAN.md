...
In this lesson, you will introduce students to NoSQL databases and executing CRUD functions using MongoDB commands. Embedded documents and MongoDB skip, sort, and limit methods will also be explored.
...
* Navigate to `28-Stu_Mini-Project/Main` in your command line and run `npm install` demonstrate the following:
  ...
  * In the console, we can see all the data that has been populated. We can query this data in our database using a `GET` request. 

  * Next we run `npm start` to start the app on port 3001.

* Open Insomnia on your local machine to demonstrate the following:

  * We use the `GET` request and the route `localhost:3001/api/students` to return a list of all of the student data in database as well as their associated grades.
...
* Open `04-Stu_Create-Read/Solved/server.js` in your IDE and explain the following: 
  ...
  * To insert multiple documents, we use `insertMany()` and place the documents inside an array.
  ...
  * To create a `GET` request, we use the `find()` method and use the `toArray()` method so multiple methods are returned in an array. Since the filter contains no data, all documents will be returned. 
...
* Open `06-Stu_Update-Delete/Solved/server.js` in your IDE and explain the following: 
  ...
  * To use an Express route to query a MongoDB database, we first have to to require and connect to the MongoDB database. In this code, that work has already been done for us.
  ...
  * Next, we create a `DELETE` route that we will use to delete data.

    ```js
    app.delete('/delete', (req, res) => {...}
    ```

  * 🔑 In this route, we will use the `deleteOne` method to delete a single document by id. Don't forget the numerical id string must be wrapped with `ObjectID()` for now. One of the advantages of Mongoose, which we we learn about later in this unit, is that it makes deleting by id easier.

    ```js
    db.collection('bookCollection').deleteOne(...)
    ```

  * 🔑 When using a delete method, it is very important that we specify a filter so that only the documents we want deleted are removed. This filter will delete only the id we provide in the request body. 

    ```js
    db.collection('bookCollection').deleteOne({ _id: req.body.id },...)
    ```

  * 🔑 We also add logic to handle the error and the response.
...
* Navigate to `06-Stu_Update-Delete/Solved/` in your command line and run `npm install` and `node server` to demonstrate the following: 

  * To test our route, we first use a GET route with the url `localhost:3001/create` to insert a new document with a title `Hello World` into our database. We add the data we want to add into the request body as JSON. 
  ...
  * We copy the id string returned for our new document. We will use that to delete the document we just created. 

  * To delete our document, we use a DELETE route with the url `localhost:3001/delete`. In the request body, we add the string wrapped in Object.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 When using a delete method, it is important that you only remove the documents you are targeting, not all the documents. Using a filter allows us to target a unique document. In addition, by using an id, we know we are only targeting one document, since each id is unique to one document. 

  * ☝️ What can we do if we don't completely understand this?

  * 🙋 We can refer to supplemental material, read the [MongoDB docs on delete methods](https://docs.mongodb.com/manual/reference/delete-methods/), and stick around for office hours to ask for help.
...
* Open `07-Ins_Embedded-Documents/models/data.js` in your IDE and demonstrate the following:
  ...
  * To create a data relationship in MongoDB, we can use a reference to establish a relationship between two documents. This is similar to SQL databases primary and foreign keys. In this example, the `promotion_id` refers to the promotion object below.
  ...
  * We can also model relationships easily using embedded documents. An embedded document is simply a document inside another relationship. Using embedded documents reduces the number of read operations needed to retrieve data and makes it simple to determine the relationship between documents. 

  * To model a one-to-one relationship, we add one related embedded document. In this instance, one department has one related promotion document.
  ...
  * To model a one-to-many relationship, we add many related embedded document. In this instance, one department has multiple related promotion documents.
...
* Open `07-Ins_Embedded-Documents/server.js` in your IDE and demonstrate the following:
  ...
  * Just like any other object, to access the data in an embedded object, we use dot notation. This will find any document in the `groceryList` collection that has, inside the `promotion` document a `percentage_discount` that is greater than 30. 
...
* Navigate to `07-Ins_Embedded-Document/` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia: 
  ...
  * When we test the `GET` route with the URL `localhost:3001/sale-over-30`, all documents that have a matching percentage discount are returned. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How do we model a one-to-one or one-to-many relationship with MongoDB? 

  * 🙋 We can embed one or more related subdocuments and access them using dot notation. 
...
* Direct students to the activity instructions found in `08-Stu_Embedded-Document/README.md`.
...
* Open `08-Stu_Embedded-Documents/Solved/server.js` in your IDE and explain the following: 
  ...
  * When we look at the data, we see each book has one set of information. This is a one-to-one relationship. We also see that the information is an embedded document. 
  ...
  * When we look at the data, we see each book can have multiple authors. This is a one-to-many relationship.
...
* Navigate to `08-Stu_Embedded-Document/Solved` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia: 
  ...
  * When we test the `GET` route with the URL `localhost:3001/price-less-than-10`, all documents that have a an embedded document with a `price` field that is less than 10 are returned.

  * When we test the `GET` route with the URL `localhost:3001/featured-authors`, all documents that have a an embedded document with a `featured` field that is `true` are returned.

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How do we access fields in an embedded document? 

  * 🙋 Since we are working with objects, we use dot notation. 

  * ☝️ What can we do if we don't completely understand this?

  * 🙋 We can refer to supplemental material, read the [MongoDB docs on modeling one-to-one relationships with embedded documents](https://docs.mongodb.com/manual/tutorial/model-embedded-one-to-one-relationships-between-documents/), [MongoDB docs on modeling one-to-one relationships with embedded documents](https://docs.mongodb.com/manual/tutorial/model-embedded-one-to-many-relationships-between-documents/)and stick around for office hours to ask for help.

* Answer any questions before proceeding to the next activity.

### 15. Instructor Demo: Sort-Skip (5 min) 

* Navigate to `08-Ins_Sort-Skip` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia: 
...
* Open `08-Ins_Sort-Skip/server.js` in your IDE and explain the following: 

  * In the `GET` route, we see a number of methods at work. 

  * `find()` is used to first return all the documents.

    ```js
    .find()
    ```

  * 🔑 Then, chained to `find()`, we add `sort()` to sort by a field in ascending or descending order. We use 1 to sort in ascending order and -1 to sort in descending order.

    ```js
    .sort({ letter: 1 })
    ```

  * 🔑 To skip a certain number of documents and not include them in the results, we can use `skip()`. The number added as a parameter will be the number of documents skipped. 

    ```js
    .skip(1)
    ```

  * 🔑 Finally, set a maximum number of results to return, we use `limit()`.
...
* In preparation for the activity, ask TAs to start directing students to the activity instructions found in `10-Stu_Sort-Skip/README.md`.

### 16. Student Do: Sort-Skip (15 min) 

* Direct students to the activity instructions found in `10-Stu_Sort-Skip/README.md`.
...
### 17. Instructor Review: Sort-Skip (15 min)
...
* Open `10-Stu_Sort-Skip/Solved/server.js` in your IDE and explain the following: 
  ...
  * 🔑 To sort the data in descending method, we chain a `sort()` method after the `find()`. Order matters here!
  ...
  * 🔑 Next, we add `skip()`. Since, we want to skip over the first five results, we use 5. 
  ...
  * 🔑 Finally, we want to return a maximum of 5 documents, so we use `limit()`. While sort, skip and limit can be used alone, these three methods are often used together to constrain results to just those we want or need. 
  ...
  * Our completed `GET` route will return sorted documents, that skip five and have a maximum limit of 5. 
...
* Navigate to `10-Stu_Sort-Skip/Solved` in your command line and run `npm install` and `node server` to demonstrate the following in Insomnia: 

  * When we test the `GET` route with the URL `localhost:3001/read`, we see the results are in descending order, five results are skipped and the results are limited to 5. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ What is the difference between `limit()` and `skip()` cursor methods?

  * 🙋 The `limit()` cursor methods sets a maximum number of documents that will be returned. The `skip()` method excludes a certain number of documents and returns only the documents that come after them. 

  * ☝️ What can we do if we don't completely understand this?

  * 🙋 @TODO We can refer to supplemental material, read the [MongoDB docs on cursor methods](https://docs.mongodb.com/manual/reference/method/js-cursor/), and stick around for office hours to ask for help.
