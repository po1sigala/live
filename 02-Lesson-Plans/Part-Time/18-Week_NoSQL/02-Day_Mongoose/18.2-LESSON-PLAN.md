...
In this class, you will introduce students to Mongoose, a node package that provides a schema-based solution to model your Node application data. Students will build upon their knowledge of MongoDB and see how Mongoose makes it easier to handle the data from the database.
...
* As we continue to work with MongoDB, we'll be adding Mongoose as the focus of today's activities. But reassure students that Mongoose is simply a "wrapper" around MongoDB that uses a schema to map to a MongoDB collection and makes it easier to manage relationships between data. It's similar to what Sequelize was for MySQL.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What is data modeling? Or more specifically, object-based data models?

  * 🙋 Generally speaking, object data modeling is focused on how the data is represented. It isn't so much over what the data is, but more about how it is visualized and connected. Data modeling can help us handle large amounts of data efficiently and get the right data back quickly.

  * ☝️ Where have we seen something similar to this before?

  * 🙋 When we used Sequelize, an object-relational mapper (ORM), with our MySQL database. Mongoose is similar to Sequelize in that aspect, but for MongoDB. Mongoose is an Object Data Modeling library, also known as an Object Data Mapper, for MongoDB. Think of it like a wrapper around MongoDB that makes it easier to manage relationships between data.

  * ☝️ What are the benefits of using Mongoose with MongoDB?

  * 🙋 Mongoose makes it easy to use MongoDB with its built-in structure and methods. The structure of Mongoose contains business logic that helps you organize your data. The built-in methods help automate common tasks and queries which help you work more quickly and efficiently.

  * ☝️ Do we have to use Mongoose when working with MongoDB?

  * 🙋 Not at all! It is important to remember that Mongoose is simply a helpful tool for working with MongoDB databases, especially when compared to ORMs like Sequelize which were more essential for relational databases. MongoDB has plenty of built-in features for helping us organize, analyze, and keep track of our data on its own. However, Mongoose will help us handle the data more easily.
...
* Open `11-Ins_Models-Schemas/config/connection.js` in your IDE and demonstrate the following:
  ...
  * You'll notice some options we are using in our `connect()` method. Those are not crucial to understand but if you want to know more about it, you can check out the [Mongoose docs on Connections](https://mongoosejs.com/docs/connections.html#options). They fix the deprecation warnings regarding the MongoDB Node.js driver to make sure the connection works. You can read more about this in the [Mongoose docs on deprecation warnings](https://mongoosejs.com/docs/deprecations.html).
...
* Open `12-Stu_Models-Schemas/Solved/models/Book.js` in your IDE and explain the following:
  ...
  * This is a good example of the flexibility you have with NoSQL databases and how validators can come in handy when specifying the type of data you want to have in the database. This will give you better control of the quality of data that gets saved in your database.
...
* Run `npm install` and `npm start` to start the application. Open Insomnia and demonstrate the following:
  ...
  * When we make a GET request to `localhost:3001/all-books`, we see all of our books returned in JSON format. Every book instance has a title since it was required, but the other properties were optional.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Mongoose docs on Schemas](https://mongoosejs.com/docs/guide.html#definition), and stay for Office Hours to ask for help.
...
* Open `13-Ins_CRUD-Mongoose/server.js` in your IDE and demonstrate the following:
  ...
  * 🔑 We know we can use the `.find()` method to query the database to find all documents that are instances of that model, but Mongoose also provides static helper functions for MongoDB CRUD operations. If we want to find one specific document, we can use the `.findOne()` method.
  ...
  * 🔑 Another helper function we can use for MongoDB CRUD operations is the `.findOneAndDelete()`. This will find the document and delete it from the database. This time instead of hardcoding the name, we will use the URL params.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Mongoose docs on queries](https://mongoosejs.com/docs/queries.html), and stay for Office Hours to ask for help.
...
* Open `15-Ins_Models-Instance-Methods/models/Department.js` in your IDE and demonstrate the following:
  ...
  * 🔑 Before we use the custom instance method, let's first use a built-in instance method `.get()` to get the `totalStock` of `produce`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We will need to define the schema, create a custom instance method, and run the custom method on an instance of the model.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Mongoose docs on Instance methods](https://mongoosejs.com/docs/guide.html#methods), and stay for Office Hours to ask for help.
...
* Open `17-Ins_Subdocuments/models/Department.js` in your IDE and demonstrate the following:
  ...
  * 🔑 In Mongoose, subdocuments are documents embedded in other documents, otherwise known as "embedded documents" in MongoDB. This means we can nest schemas in parent schemas. In Mongoose, we can do this in two ways: an array of subdocuments and a single nested subdocument.
  ...
  * In the preceding code block, we are including the `manager` as a single nested subdocument and the `employees` as an array of subdocuments.

  * Let's create the model and instances of the model to see how this will look! Notice the `managerData` is a single object, but the `employeeData` is an array of objects.
...
* Run `npm install` and `npm start` from the command line to demonstrate the following:
  ...
  * When we run the application, we see the `Shoes` department returned with one manager and an array of two employees.
    ...
    ```js
    {
      ...
      name: 'Shoes',
      manager: { _id: 611d528eed231427e2d386e4, name: 'Lewis', salary: 80000 },
      employees: [
...
* Direct students to the activity instructions found in `18-Stu_Subdocuments/README.md`.
...
* Open `18-Stu_Subdocuments/Solved/models/Library.js` in your IDE and explain the following:
  ...
  * 🔑 Finally, we create the array of books that will use the `bookSchema` when we create a new instance of `Library`. You could've used any titles of books and prices.
...
* Run `npm install` and `npm start` in your command line to demonstrate the following:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Mongoose docs on Subdocuments](https://mongoosejs.com/docs/subdocs.html), and stay for Office Hours to ask for help.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Mongoose docs on Aggregate](https://mongoosejs.com/docs/api/aggregate.html), and stay for Office Hours to ask for help.
