...
* Make a POST request to `http://localhost:3001/api/users` using the following JSON:
...
* Make a POST request to `https://localhost:3001/api/posts` using the following JSON:
  ...
  * Confirm that the post was created with the response as shown below. Be sure to take node of the `_id` property.

  * This will create a post for us to assocatiate with a user. The the posts array will be populated with a few are considered subdocuments on the `posts` schema.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel with populating subdocuments { TOPIC }? (Poll via Fist to Five, Slack, or Zoom)
