...
* Open `24-Stu_Subdoc-Population/Solved/controllers/tagController.js` in your IDE to demonstrate the following:
  ...
  * In this controller, we also have logic to create a new tag, and subsquently update a post with that tag.
  ...
  * It is important handle errors in this method. If the `createTag` method fails, we want to return a 500 error to the user.
  ...
  * Lets make a POST request to add a new tag to a post, but first let's make a get request and pick one of our existing posts to work with.
  ...
  * Next, make a POST request to `http://localhost:3000/api/tags/` with the following body:

    ```json
    {
      "postId": "6116dd5c38283bccf74a2353", // Make sure to replace this with the actual _id of a post
      "tagName": "new tag",
      "color": "red"
    }
    ```
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Mongoose documentation on Subdocuments](https://mongoosejs.com/docs/subdocs.html), and stick around for office hours to ask for help.

* Answer any questions before proceeding to the next activity.

### 8. Instructor Demo: Mongoose CRUD (5 min)

* Navigate to `25-Ins-Mongoose-CRUD/` in your terminal and run `npm run seed && npm start` to seed the database with some data and start the server.
  ...
  * Before we test out the application, let's first make a GET request to `http://localhost:3000/api/users/` to see if the server is up and running.
  ...
  * To set up an API like this, we would first begin by creating our controller for Users, Videos, and Responses. Once all of those are created we can import them into a single file and begin to create the routes for our API.
...
* In preparation for the activity, ask TAs to start directing students to the activity instructions found in `26-Stu_Mongoose-CRUD/README.md`.

### 9. Student Do: Mongoose CRUD (15 min)

* Direct students to the activity instructions found in `26-Stu_Mongoose-CRUD/README.md`.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How comfortable do you feel with Mongoose CRUD operations(Poll via Fist to Five, Slack, or Zoom)?
...
* Open `26-Stu_Mongoose-CRUD/Unsolved/controllers/appController.js` in your IDE and explain the following:
  ...
  * `find()` happens to be the first methods listed in the `appController.js` file. We use the `.find()` method to find all of the applications in our database, as shown in the following code:
  ...
  * Finally, it is important that we use `.catch()` to handle any errors that may occur.
  ...
  * Once the application is created, we return another promise that will handle updating the user who's application is being created with the `findOneAndUpdate` method and a mongodb operator called `$addToSet`. This operator will add the new application to the user's list of applications.
  ...
  * After the user has been updated, we return a message to user depending on whether the application was created successfully or not.
  ...
  * Next, we are going to take a look at the update and delete methods for the `applications` collection. These are titled `updateApplication` and `deleteApplication` respectively.
  ...
  * Inside the the `update` object, we use the `$set` operator to update the entire body of a given `application`, that is selected using the `req.params.applicationId` passed in from the URL.
  ...
  * After the application has been removed, we return another promise that will handle updating the user who's application is being deleted with the `findOneAndUpdate` method and a mongodb operator called `$pull`. This operator will remove the application from the user's list of applications.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We can refer to supplemental material, read the [Mongoose documentation on Models](https://mongoosejs.com/docs/api/model.html), and stick around for office hours to ask for help.
...
* Open [GitHub Packages Getting Started](https://docs.github.com/en/packages) in your browser and explain the following:
  ...
  * This guide will walk you though the steps needed to create a GitHub package, and how to publish it. At the end of the guide you will be able to create a GitHub package and publish it so that it can be installed when running `npm install`.
...
<!-- !TODO: Ask if this repo should be published to coding-boot-camp similar to our git-forking tutorial -->
<!-- !TODO: If the former is true, then we should provide the instructor steps to clone the repo and demonstrate publishing the npm package -->
...
* Open `28-Stu_Mini-Project/Main/controllers/studentController.js` in your IDE and explain the following:
  ...
  * In order to get the number of students enrolled, we were tasked with creating a function that executes an aggregate pipeline that counts the number of students in the database.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ What is required in order to deploy a backend application and connect it to a MongoDB Atlas database?
  ...
  * 🙋 We can refer to supplemental material, read the [Full-Stack Blog](https://coding-boot-camp.github.io/full-stack/mongodb/deploy-with-heroku-and-mongodb-atlas), and stick around for office hours to ask for help.
...
* Open `01-Class-Content/18-NoSQL-dev/02-Homework/README.md` in your IDE and demonstrate the following:
  ...
  * Optionally, you can import a time/date management library to make adding time stamps to your data easier. You also have the option of using the vanilla `Date` object if you so choose.

  * The app use the following models: `User`, `Thought`, and `Reactions`.
  ...
  * The `Thought` model will have `thoughText`, `userName`, `reactions` (aka replies), and `createdAt` fields. This model will also need a virtual field called `reactCount` that returns the number of reactions that the `Thought` has.
...
* Ask TAs to direct students to the Homework Requirements found in `01-Class-Content/18-NoSQL-dev/02-Homework/README.md`.
