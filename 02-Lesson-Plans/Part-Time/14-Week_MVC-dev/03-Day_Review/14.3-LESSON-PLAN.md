...
* Open `22-Stu_MVC-Review/Solved/views/homepage.handlebars` in your IDE and explain the following:
  ...
  * 🔑 We use a Handlebars.js `#each` helper to loop over the users and render a `<div>` element for each one.
...
* Open `24-Stu_Auth-Review/Solved/views/layouts/main.handlebars` in your IDE and explain the following:
  ...
  * We use the `#if` helper to conditionally render elements for logged in users.
...
* Open `25-Ins_Lint-Review/package.json` in your IDE and explain the following:
  ...
  * 🔑 We've added `eslint` as a dependency to this project.
...
* Open `25-Ins_Lint-Review/.eslintrc.json` in your IDE and explain the following:
  ...
  * In the config file, we specified that ESLint shouldn't look at parent folders for other configurations.

    ```json
    "root": true
    ```

  * 🔑 We added an `env` property to specify the different environments and features we want ESLint to support, including ES6 syntax.
...
* Open `26-Stu_Lint-Review/Solved/.eslintrc.json` in your IDE and explain the following: 
  ...
  * 🔑 The exponentiation operator is an ES7 feature, so we added `es2017` as an environment we want ESLint to support.

    ```json
    "es2017": true
    ```

  * 🔑 We added `no-undef-init` to the list of rules to disallow assigning variables to `undefined`.
...
* In preparation for the activity, ask TAs to start directing students to the activity instructions found in `27-Evr_Prettier`.
