...
* You will use MySQL for a few of today's activities. Be sure to have your MySQL server up and running. You can create the `dish_db` database ahead of time, or you can demonstrate it live in class. 
...
* Use data serialization to simplify objects to work with necessary properties only.
...
| 9:00PM | 16  | Student Do: Handlebars FE Logic             | 0:15     |
| 9:15PM | 17  | Instructor Review: Handlebars FE Logic      | 0:15     |
| 9:30PM | 18  | END                                        | 0:00     |
...
* Open the [slide deck](https://docs.google.com/presentation/d/1rb2QnbKkUfmwqfUEpdQjV8x6S18ShB1BRdLggIVq6yU/edit?usp=sharing) and follow these prompts on their corresponding slides:
  ...
  * **What is the Separation of Concerns Principle?**
    ...
    * In a restaurant, the chef’s concern is to cook the food, the server’s concern is to take orders and serve food, and a customer’s concern is to order and eat.
  ...
  * **What is MVC?**
    ...
    * Model&mdash;Models are in charge of storing data and data-related logic.

    * View&mdash;Views are in charge of UI/UX concerns, or what a user will see and interact with.

    * Controller&mdash;Controllers are the interface between Models and Views. They process requests from the View, use the Model to manipulate data, and send data to the View to render.
  ...
  * **What are advantages of using a template engine?**
    ...
    * They offer tools that cut down on repetition in the code.
  ...
  * **What is Handlebars.js?**
    ...
    * It is a pure rendering engine&mdash;that means it has no built-in support for event-handling, accessing back-end services, or incremental DOM updates

  * **Why are we learning Handlebars.js?**
  
    * It gives us a great introduction to template engines because it is easy to use, but offers a ton of functionality. It isn’t the only one out there, but it is a great place to start!
...
* Click on the `logout` button on the page to demonstrate that a user can log out of their account.
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We will use Sequelize, MYSQL, and object-oriented programming. This project will also include a front end and a back end.

  * ☝️ How does this project relate to your career goals?

  * 🙋 We will learn how to use template engines and explain the MVC framework, a popular arichitectural pattern that you will encounter as a developer. This work will also prepare us for using heavier frameworks, like React.js.
...
* Open `01-Ins_MVC-Handlebars-Intro/controllers/dish-routes.js` in your IDE and demonstrate the following:
  ...
  * 🔑 MVC is an architectural pattern that is used in most modern web applications.
...
* Open `01-Ins_MVC-Handlebars-Intro/models/Dish.js` in your IDE and demonstrate the following: 
  ...
  * We learned how to set up models in the previous unit on ORMs. Remember, the data is hardcoded in `index.js`, so `Dish.js` is set up, but isn't actually doing anything at the moment.
...
* [npm documentation on Express Handlebars.js](https://www.npmjs.com/package/express-handlebars)
...
* Open `02-Stu_MVC-Handlebars-Intro/Solved/views/layouts/main.handlebars` in your IDE and explain the following: 
  ...
  * The file looks like standard HTML, except for `{{{ body}}}`, which is a Handlebars.js placeholder. Whatever the Controller directs the View to render will be plugged in here, as shown in the following example:
...
* Open `03-Ins_Handlebars-Expressions/controllers/dish-routes.js` in your IDE and demonstrate the following:
  ...
  * Each object includes `dish_name` and `description` keys and value pairs, like in the following example:
  ...
  * The Controller includes a GET route for one dish, that passes the static data for one dish into a Handlebars.js template, as follows:
...
* Navigate to `03-Ins_Handlebars-Expressions/views/dish.handlebars` in your IDE and demonstrate the following:
  ...
  * 🔑  The template for one dish uses a Handlebars.js expression to render the name of a dish. Inside the double curly brackets, we include the `dish_name` key so that its value will be rendered&mdash;as shown in the following example:
...
* Navigate to <http://local:host3001/dish/2> and demonstarte the following: 
  ...
  * 🔑  Now, when we view the page for another dish, we see that the template is the same, but the data has changed. 
...
* Open `04-Stu_Handlebars-Expressions/Solved/controllers/dish-routes.js` in your IDE and explain the following: 
  ...
  * We render the `dish` template by passing it in as an argument. Then, we use params to select the correct dish to render in the template, based on the index of the dish. Now, we have access to a dish description through a dish object, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 `{{ data }}`, or data wrapped in double curly brackets.
...
* Open `05-Ins_Built-In-Helpers/controllers/dish-routes.js` in your IDE and explain the following: 
  ...
  * 🔑 We can use Handlebars.js built-in helpers to conditionally render blocks of code, iterate over lists, etc. Because the Controller is passing `dishes` to the View through `render()`, we can use a built-in helper to iterate over it in the template.
...
* Open `05-Ins_Built-In-Helpers/views/all.handlebars` in your IDE and demonstrate the following:
  ...
  * `dishes` represents the `dishes` array that the Controller passed to the template. `dish` represents one dish object in that array.

  * 🔑 After `{{#each dishes as |dish|}}`, is the block of code to be rendered for each dish object. We could also use the syntax, `{{#each dish}}`, and then use `this.id` and `this.dish_name` in the block of code.
...
* Open `06_Stu-Built-in-Helpers/Solved/controllers/dish-routes.js` in your IDE and explain the following: 
  ...
  * When we look at the dish data, we see that it includes a Boolean for `has_nuts` that is set to `true` for the dishes that contain nuts, as shown in the following example:
...
* Open `06-Stu_Built-In-Helpers/Solved/views/dish.handlebars` in your IDE and explain the following: 
  ...
  * 🔑 We want to make sure that the message "Contains Nuts" is only rendered for dish objects that actually contain nuts. To do this, we need to conditionally render a block of code with the `#if` built-in helper.

  * 🔑 We provide `has_nuts` as the condition when we open the `#if` helper. Then we include the block of code that we wish to conditionally render. Finally, we close `#if`, as follows: 
...
* Open `07-Ins_Data-Serialization/controllers/dish-routes.js` in your IDE. 
...
* For the purpose of this Demo, the Controller includes two GET routes for one dish. The first does not serialize data, the second one (currently commented out) does serialize the data.
  ...
  * 🔑 The Controller has changed now that the Model is set up. Now we are using `findByPk()` to locate a dish in the database. Once `dishData` is found, we are passing it to the `dish` template in the View to be rendered, as shown in the following example:
...
* Run `npm install`, `npm start`, navigate to <http://local:host3001/dish/1> in your browser and demonstrate the following: 
  ...
  * 🔑 However, the object will be returned as a Sequelize object that can be difficult to parse through on the template side. 
 
  * 🔑 When we look at the browser, the template is there, but the data is missing. 

  * 🔑 When we look at the command prompt, we see that we are logging the Sequelize object. Look at all of the extra data in the following code:
...
* In `07-Ins_Data-Serialization/controllers/dish-routes.js`, comment out the first GET route and uncomment the second GET route to demonstrate the following:
  ...
  * 🔑 Let's look at the second GET route in the Controller. This route is serializing the Sequalize object.

  * 🔑 We accomplish this by attaching `.get({ plain: true })` to the returned object `dishData`, and then we pass the serialized data as `dish` through to the `'dish'` template, like in the following example:
...
* Restart your server and navigate to <http://local:host3001/dish/1> in your browser and demonstrate the following:.
  ...
  * Here we see that the View for one dish is now rendering both the template and the data. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ How would we make sure that the templates in the View can parse the data sent by the Controller?
...
* How can you change your route so that the View can display the menu of food items?
...
* Open `08-Stu_Data-Serialization/Solved/controllers/dish-routes.js` in your IDE and explain the following: 
  ...
  * To render a full menu for party guests using a GET route, we must first query the database for all dish objects using the Sequelize method, `findAll()`.

  * 🔑 Instead of recieving one object, we recieve an array of dish objects that need to be serialized before being rendered. We are using `map()` to return a new array of serialized objects by adding `.get({ plain: true})` to each `dish` object.
 ...
 * 🔑 The MVC framework follows the separation of concerns principle. For these purposes, this means that content should be separate from presentation and that data processing should also be separate from content. This helps us write clear and maintainable code.

  * 🔑 We will explore MVC and the separation of concerns principle further in this lesson by examining a form to add a dish to the database.
...
* Open `09-Ins_Handlebars-FE-Logic/public/js/add-dish.js` in your IDE and demonstrate the following:
  ...
  * 🔑 `fetch()` passes the form input to the Controller in the body of the request with the POST method, as follows:
