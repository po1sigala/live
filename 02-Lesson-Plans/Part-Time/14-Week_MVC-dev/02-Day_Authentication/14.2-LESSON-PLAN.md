...
* One of the topics for today is cookies. Using the following prompts, have a discussion about how sessions and cookies work hand-in-hand.

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * Today we will talk about cookies! Not the cookies that we eat, but the HTTP cookies that are saved whenever we visit a website. 
  ...
  * 🙋 The most common example is a shopping cart. When we visit an online store and select some items to add to your shopping cart, the cookie will remember those items. When you are ready to check out, the items will still be in your shopping cart.
  ...
  * 🙋 We will use cookies to remember our logged-in status so that we can navigate from page to page and access all the content.
...
* Open `11-Ins_Partials/views/partials/gallery-details.handlebars` in your IDE and explain the following:
  ...
  * 🔑 We create a new folder in the `views` directory called `partials` and put the partials there.
...
* Open `12-Stu_Partials/Solved/views/gallery.handlebars` in your IDE and explain the following: 
  ...
  * This is the gallery page that displays the paintings that belong to this gallery. There are two to three paintings for each gallery, so we use the built-in helper function `{{#each}}` to iterate through each painting, as shown in the following example:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 That will be handled in the Controllers! As part of the MVC framework, the templates (View) only focus on the rendering of content. The routes (Controller) are responsible for getting the data from the database (Model) and handle all the logic behind which data is needed to be rendered.
...
* Open `13-Ins_Custom-Helpers/views/gallery.handlebars` in your IDE and explain the following:
  ...
  * We see how the Opening Hour uses the gallery's `starting_date` as is, as shown in the following example:

    ```handlebars
    <h4>Opening Hour: {{gallery.starting_date}}</h4>
    ```

  * 🔑 But we see how Closing Hour uses a custom helper function called `format_time` to display only the time of `gallery.ending_date` in a specific format, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How would we use custom helpers to format the painting's end date in the museum application?
...
* Open `14-Stu_Custom-Helpers/Solved/utils/helpers.js` in your IDE and explain the following: 
  ...
  * 🔑 To format the date, we use template literals! First we need to get the month, but notice how `.getMonth()` will return the month as a zero-based value. For example, January will return `0`. To format it correctly, we need to add `1` to the value returned, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 To keep the templates logic-free and only concerned with rendering data, we can separate this kind of logic and place them in helper functions instead.
...
* Navigate to `15-Ins_Sessions` in your command line and run `npm install` to demonstrate the following:
  ...
  * Sessions allow the Express.js server to keep track of which user is making a request, and store useful data about them in memory.
...
* Open `15-Ins_Sessions/server.js` in your IDE and demonstrate the following:
  ...
  * The `saveUninitialized` option forces a session that is "uninitialized" to be saved to the store. A session is uninitialized when it is new but not modified. Choosing `false`, as shown in the following example, is useful for implementing login sessions: 
...
* Open `15-Ins_Sessions/controllers/home-routes.js` in your IDE and explain the following:
  ...
  * We will set up a session variable to count how many times we visit the homepage, and **only** the homepage. We can visit the gallery and painting pages, but that will not get counted.
  ...
  * We want to make sure the session is created before we send the response back, so we're wrapping the variables in a callback. The `req.session.save()` method, shown in the following example, will initiate the creation of the session and then run the callback function once complete: 
...
* Run `npm start` and open <http://localhost:3001/> in your browser and demonstrate the following:
  ...
  * If we restart the server, the count is reset! That's because the session is lost once the connection to the server is lost or restarted.
...
![On a museum page, under the gallery name and operating hours, there is a message to log in.](./Images/01-Unauthorized-Gallery.jpg)
...
* Open `16-Stu_Sessions/Solved/controllers/api/user-routes.js` in your IDE and explain the following: 
  ...
  * 🔑 When a new user is created, we want them to be logged in right away. So we set up a session variable `loggedIn` and set it to `true` as soon a user is created, as shown in the following example:
  ...
  * This isn't something you had to code, but notice how we destroy the session once the user logs out, as follows:
...
* Open `16-Stu_Sessions/Solved/views/gallery.handlebars` in your IDE and explain the following: 
  ...
  * 🔑 We want to prevent unauthorized access to the paintings in the gallery page. So on the `gallery.handlebars` template, we use the built-in helper method `{{#if}}` to determine if the session variable `loggedIn` is set to `true`. Only then can they access the `painting-details`. See the following code for an example:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 On the `main.handlebars` template, once the user logged in, the link to log in was replaced by a button to logout in the navbar!
...
[npm documentation on the connect session store using Sequelize](https://www.npmjs.com/package/connect-session-sequelize)
...
* Open `18-Stu_Cookies/Solved/server.js` in your IDE and explain the following: 
  ...
  * 🔑 Lastly, we set `store` to a new Sequelize store and we connect it to the database, as follows:
...
* Open `19-Ins_Middleware/controllers/home-routes.js` in your IDE and explain the following:
  ...
  * 🔑 Conditional access is when we only allow users who are authorized to see certain content. We did something similar in the Sessions activity when we checked if the user was logged in before allowing them to see the paintings. But we checked that in the templates.

  * 🔑 In line with the separation of concerns principle and the MVC framework, the correct way to accomplish this is directly in the routes. If we look at the route for a gallery, we use the same logic where if they are not logged in (meaning the `loggedIn` session variable does not exist), then the user is redirected to the login page&mdash;as shown in the following example:
  ...
  * We can create custom middleware to check if the user is authenticated before allowing them to access the route. That's what the next activity is all about!
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * ☝️ How would we use custom middleware for the museum application?
...
* Open `20-Stu_Middleware/Solved/utils/auth.js` in your IDE and explain the following: 
  ...
  * 🔑 So, to create this custom middleware, `withAuth`, we take the exact same logic that was in the routes and put it in the function, as follows:
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 Writing custom middleware helps us handle common functionality in applications. For example, it keeps us from repeating the same code at the beginning of a route handler.
