...
* Navigate to [GitHub.com](https://github.com) in your browser and demonstrate the following:
  ...
  * Now we can add an item to our board by clicking on `+ Add item` in the bottom left to add an existing issue as a card to our project or create a new draft issue. Note that the board will allow you to add existing issues from any repository you own, so make sure you are selecting the appropriate repo.

  * When the status of a card changes, we update the board by dragging the card into its appropriate column.

  * Remember you can also create an issue by navigating to the `Issues` tab. If you decide to add it to a project, you can select which project the issue can be added to by selecting the project to associate the issue with within the issue card.
