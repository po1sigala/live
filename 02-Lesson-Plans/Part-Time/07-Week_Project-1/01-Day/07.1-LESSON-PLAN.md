...
* Navigate to [GitHub.com](https://github.com) in your browser and demonstrate the following:
  ...
  * When prompted to select a template, click `Team Backlog`. Then, click Create.

  * Here, we'll give the project board a name by clicking on the ellipsis on the right hand side of the page and then clicking on `Settings`. Here, we can rename the project and give it a description.

  * Now that we've created a project board we can add a repository to it by navigating to any project repository and selecting the `Projects` tab. From here click the `Add project` button and select the project board from the dropdown. The project board will now appear in the `Quick Access` box below!

  * Returning to the project board, we can click on `+ Add item` in the bottom left to add an existing issue as a card to our project or to create a new draft issue. Note that the board will allow you to add existing issues from any repository you own, so make sure you are selecting the appropriate repo.

  * When the status of a card changes, we update the board by dragging the card into its appropriate column.

  * Remember you can also create an issue by navigating to the `Issues` tab. If you decide to add it to a project, you can select which project the issue can be added by selecting the project to associate the issue with within the issue card.
...
3. Create a card in the "Backlog" column, then move it to "In progress"
