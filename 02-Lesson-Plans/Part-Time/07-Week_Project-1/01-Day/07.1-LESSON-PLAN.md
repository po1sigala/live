...
* Navigate to [GitHub.com](https://github.com) in your browser and demonstrate the following:
  ...
  * Returning to the main backlog page, we can click on `+ Add item` in the bottom left to add an existing issue as a card to our project or to create a new draft issue. Note that the board will allow you to add existing issues from any repository you own, so make sure you are selecting the appropriate repo.
