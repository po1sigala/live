...
* Open `01-Ins_Window-Object/index.html` in your browser and navigate to the console to demonstrate the following:
  ...
  * 🔑 When we open up the window object, we see properties and methods. We already used the window methods `alert()`, `confirm()` and `prompt()` in our Rock, Paper, Scissors mini-project. 

  * 🔑 The document represents the web page loaded in the browser. When we log `document.defaultView` the window object associated wth the document is returned.

  * 🔑 We log `document.body` to return what is contained in the `<body>` of the document. 

* Ask the class the following questions (☝️) and call on students for the answers (🙋):

  * ☝️ What does the window object represent?

  * 🙋 An open window in the browser.
...
* Open `02-Stu_Window-Object/script.js` in your IDE and explain the following: 
  ...
  * 🔑 The document also serves as the entry point to access a web page's content. We use `document.documentElement` to log the root element or the `<HTML>`.
