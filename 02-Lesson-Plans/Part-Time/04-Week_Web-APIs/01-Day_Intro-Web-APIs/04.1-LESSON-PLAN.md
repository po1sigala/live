...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋 We previously used loops to iterate over a list of students' names. Now, we are using a loop to iterate over a list of HTML elements. Loops are widely used in JavaScript when we want to perform the same task over and over again and helps us write DRY-er code. 
