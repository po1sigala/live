...
* Open `03-Ins_Models/.env.EXAMPLE`, add your credentials, and demonstrate the following:
  ...
  * All of the activities provide a `.env.EXAMPLE` file that we must remember to change each time before we begin. Let's rename the file to `.env` and add our credentials.
...
* Open `05-Ins_Create/routes/api/index.js` in your IDE and explain the following:
  ...
  * We created an `api/` directory to hold all of our API endpoints, which will help keep them separate from any HTML routes we might eventually build.
