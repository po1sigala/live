...
* Open `22-Stu_One-to-One/Solved/models/index.js` in your IDE and explain the following:
  ...
  * 🔑 We import both models, associate them to one another, and export them as an object, like in the following example:
    ...
    * 🔑 To associate the `Reader` model with the `LibraryCard` model in a one-to-one relationship, we use the `hasOne()` method. The foreign key will be defined on the `Reader` model. 
...
* Ask the class the following questions (☝️) and call on students for the answers (🙋):
  ...
  * 🙋  In Sequelize, to form an association we use the `hasOne()` method to establish that a model has one of a second model. Then, we use the `belongsTo` method to establish a relationship between the two models.
...
* Open `24-Stu_One-to-Many/Solved/models/index.js` in your IDE and explain the following:
  ...
  * 🔑 We create the relationship between readers and books by using `hasMany()` to declare readers have many books, as shown in the following example:
  ...
  * 🔑 We pair the `hasMany()` declaration with a `belongsTo()` declaration to show that while each reader has many books, a book belongs to a single reader. This forms the one-to-many relationship:
